...
class NativeWorkflowTests(unittest.TestCase):
    ...
    def test_lease_counts_descendants_and_retains_unknown_or_dead_capacity(self):
        with mock.patch.object(router, 'native_process', side_effect=lambda *a: dict(self.info)), \
                mock.patch.object(router, 'host_processes', return_value=({}, {})):
            router.native_lease(self.root, ROOT, 100, 8)
            self.assertEqual(router.occupancy(self.root), ([8, 0], [1, 0]))
            self.assertEqual(router.admission_limits(self.root, [1, 0])[0][0], 11)
            with mock.patch.object(router, 'process_identity', return_value='123'):
                with self.assertRaises(ValueError):
                    router.native_lease(self.root, ROOT, 100, 8, release=True)
            self.assertEqual(router.native_leases(self.root)[ROOT]['slots'], 8)
    ...
    def test_release_requires_the_original_root_identity(self):
        with mock.patch.object(router, 'native_process', side_effect=lambda *a: dict(self.info)), \
            ...
            with self.assertRaisesRegex(ValueError, 'release identity'):
                router.native_lease(self.root, ROOT, 999, 8, release=True)
            with self.assertRaisesRegex(ValueError, 'release identity'):
                router.native_lease(self.root, ROOT, 100, 7, release=True)
