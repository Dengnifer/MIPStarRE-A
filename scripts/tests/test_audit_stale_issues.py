...
def _make_fake_repo(root: Path) -> None:
    """Create a minimal PaperLib/ tree used by audit tests."""
    mod = root / "PaperLib" / "Core" / "Fake"
...
class ExtractFileCitationsTests(unittest.TestCase):
    def test_extracts_path_with_colon_line(self) -> None:
        body = "See `PaperLib/Core/Foo/Bar.lean:141` for details."
        self.assertEqual(
            extract_file_citations(body),
            [FileCitation(path="PaperLib/Core/Foo/Bar.lean", line=141)],
        )

    def test_attaches_nearby_line_word_to_sole_path_on_line(self) -> None:
        body = "at PaperLib/Core/Foo/Bar.lean on line 42."
        self.assertEqual(
            extract_file_citations(body),
            [FileCitation(path="PaperLib/Core/Foo/Bar.lean", line=42)],
        )

    def test_does_not_cross_attribute_when_multiple_paths_on_line(self) -> None:
        body = (
            "PaperLib/A.lean and PaperLib/B.lean both matter; also line 99."
        )
        self.assertEqual(
            extract_file_citations(body),
            [
                FileCitation(path="PaperLib/A.lean", line=None),
                FileCitation(path="PaperLib/B.lean", line=None),
            ],
        )

    def test_deduplicates_repeated_citations(self) -> None:
        body = (
            "See PaperLib/X.lean:10.\n"
            "Again PaperLib/X.lean:10.\n"
            "And PaperLib/X.lean without a line."
        )
        self.assertEqual(
            extract_file_citations(body),
            [
                FileCitation(path="PaperLib/X.lean", line=10),
                FileCitation(path="PaperLib/X.lean", line=None),
            ],
        )

    def test_extracts_path_from_github_blob_url(self) -> None:
        body = (
            "See https://github.com/OWNER/REPO/blob/main/"
            "PaperLib/Core/Foo/Bar.lean#L141 for details."
        )
        self.assertEqual(
            extract_file_citations(body),
            [FileCitation(path="PaperLib/Core/Foo/Bar.lean", line=141)],
    ...
    def test_extracts_path_from_blob_url_with_slash_in_ref(self) -> None:
        body = (
            "See https://github.com/OWNER/REPO/blob/feature/foo/"
            "PaperLib/Core/Foo/Bar.lean#L13 for details."
        )
        self.assertEqual(
            extract_file_citations(body),
            [FileCitation(path="PaperLib/Core/Foo/Bar.lean", line=13)],
        )
...
class BuildDeclIndexTests(unittest.TestCase):
    def test_collects_short_and_qualified_names(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            index = build_decl_index(root / "PaperLib")
...
class AuditIssueTests(unittest.TestCase):
    def test_flags_missing_file_missing_decl_and_resolved_sorry(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            _make_fake_repo(root)
            decl_index = build_decl_index(root / "PaperLib")
            ...
            issue = {
                ...
                "body": (
                    "Stale file: `PaperLib/Core/Fake/Gone.lean:10`\n"
                    "Resolved line: `PaperLib/Core/Fake/Live.lean:6`\n"
            ...
            self.assertEqual(
                report.missing_files, ["PaperLib/Core/Fake/Gone.lean:10"]
            )
            self.assertEqual(
                report.non_sorry_lines,
                [("PaperLib/Core/Fake/Live.lean", 6)],
            )
    ...
    def test_flags_out_of_range_line_as_stale(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            issue = {
                ...
                "url": "",
                "body": "Out of range: `PaperLib/Core/Fake/Live.lean:9999`.",
            }
            report = audit_issue(issue, root, decl_index)
            self.assertTrue(report.is_flagged)
            self.assertEqual(
                report.non_sorry_lines,
                [("PaperLib/Core/Fake/Live.lean", 9999)],
            )
    ...
    def test_flags_file_citation_that_escapes_repo_root(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            issue = {
                ...
                "title": "path escape",
                "url": "",
                "body": "Escapes checkout: `PaperLib/../../secret.lean:2`.",
            }
            report = audit_issue(issue, root, decl_index)
            self.assertTrue(report.is_flagged)
            self.assertEqual(
                report.missing_files,
                ["PaperLib/../../secret.lean:2"],
            )
    ...
    def test_not_flagged_when_all_citations_resolve(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            issue = {
                ...
                "url": "",
                "body": (
                    "Line `PaperLib/Core/Fake/Live.lean:16` is still a sorry "
    ...
    def test_text_and_json_renderers_include_flagged_issue(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(
                ...
                ["PaperLib/Core/Fake/Gone.lean:3"],
            )
