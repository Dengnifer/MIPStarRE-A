---
title: "Historical issue 515 combined-line consistency audit"
date: 2026-09-18
purpose: >
  Preserve the 150-line audit from PR #534 verbatim below a current-status
  qualification. Its validation and source-completion language is historical.
status: historical
track: qpbt
kind: preservation
...
## Preservation status (2026-09-18)

The text beginning with `# Issue 515` below is the unchanged 150-line audit
from PR #534 at `7042717ee89edf398ea40da3a19fe5bd4dbcd5c2`, SHA-256
`4a88778ef648c7e5f8b4700fd64b74f1c9bf2f1cac54947071bc1ebfd9ae2fb0`.
Its build, CI, blueprint, axiom, and review statements describe that historical
branch, not validation of this preservation commit. The old proof is already
represented on the published main baseline by `combined_line_measurement_consistency`
in `Lines/Construction.lean`; the source-labelled `lem:qld-xz-lines` remains
`\notready` in the current blueprint. The historical claim of exact source
preservation does not settle issue #515: the Lean `IsPolyErr₂` error contract
uses a sum of powers, while the source shorthand uses a product form, as
documented in `docs/paper-gaps/qpbt_pasting-product-error.tex`. Neither the
source theorem nor its original error-scale interpretation is certified here.
...
The earlier issue #118/B8 checkpoint records 13 attempts and 26,509 working
seconds. Five separately identified later records add 14,637 seconds; the old
prover-515 episode and review history are additional. Unknown model usage is
not zero, and this preservation supplies no new mathematical attempt budget.
