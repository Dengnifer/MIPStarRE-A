...
`Extraction/Consistency.lean`, the source-facing obligation in
`Extraction/Construction.lean`, its canonical re-export in `MIPStarRE/QPBT.lean`,
the supplied-witness documentation in
...
The source-facing `exists_pulled_apart_consistency` in
`Extraction/Construction.lean` quantifies universal
...
Focused checks pass; `Consistency.lean` reports the four retained holes,
and `Construction.lean` contains the one documented realignment hole.
After regenerating the affected modules' branch-private artifacts,
...
`.lake/issue519-checks/Axioms.lean` and `axioms-followup.log`.
...
1446 references in the normalized declaration list.
The primary `check_oversized_lean_files.py` passes with no oversized files:
`Consistency.lean` has 1000 lines and `Construction.lean` has 71.
The source docstring, statement, and proof were moved verbatim from commit
`db915b556a0bf042adf3fb856c307080a1ccd599`; no additional proof hole was added.
Both modules and the canonical `MIPStarRE/QPBT.lean` re-export type-check.
The source lemma and all three conditional blueprint links still have no
statement or proof completion markers.
