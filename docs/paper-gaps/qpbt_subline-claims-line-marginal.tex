...
source transport is separate from the proved directly indexed estimate.
\textbf{Estimated weight.} The scalar proof uses a finite-sum complex
Cauchy--Schwarz lemma and the existing restricted-distribution deficit bound.
The pasting consistency obligation remains open.
...
The concrete measurement domain is retained. The proved scalar estimate
...
source law $D$. Thus it is a formalization-only result, linked from
...
The directly indexed complex X-factor estimate is proved in \ghissue{514}.
The distribution transport and the other complex-overlap obligations under
\ghissue{118} remain open; their historical budget is unchanged.
...
The proof of \leanid{subline\_remove\_X\_factor\_direct} applies complex
weighted Cauchy--Schwarz to bound $|A(T)-B(T)|$ by the square root of the
X-overlap deficit, then uses
\leanid{exists\_concreteXPointOverlap\_deficit\_le}. For each sample, the
positive line effect defines a sesquilinear form. Cauchy--Schwarz for this
form bounds the complex correlation by the product of the square roots
of its diagonal values. Summing over outcomes and then averaging uses the
ordinary finite-sum Cauchy--Schwarz inequality. Completeness bounds the
Z diagonal sum by one; projectivity turns the complementary X diagonal
sum into the deficit. The existing deficit estimate
...
The retained direct proof hole is
\leanid{combined\_line\_measurement\_consistency}.
...
scalar conclusion is proved for the directly indexed auxiliary theorem;
