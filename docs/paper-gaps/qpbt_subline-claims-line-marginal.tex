...
real-part and complex-modulus estimates on the directly indexed subline law
are formalized. The source distribution correspondence remains open,
...
The original real-part theorem \leanid{subline\_remove\_X\_factor}
and its proof are also retained, with auxiliary blueprint node
\path{lem:claim-17-2-direct-real}. The compatibility names
\leanid{subline\_replace\_by\_ordered\_product} and
\leanid{subline\_Z\_term\_near\_one} denote the corresponding real-part
estimates under their explicitly qualified names above.
...
give the deficit bound, and Cauchy--Schwarz closes both the real-part and complex-modulus estimates.
...
The first scalar statement compares real parts, whereas the source bounds a
complex modulus. The second now also has a proved complex-modulus version. The ordered product $M_ZM_X$ need not be
...
and the stated distance bounds, without a reality hypothesis. For the second
claim, \leanid{norm\_overlap\_gap\_le\_sqrt\_one\_sub\_of\_isProj}
in \path{MIPStarRE/QPBT/Combining/ComplexOverlapGap.lean} supplies the
complex weighted estimate from positivity, completeness, and projectivity.
...
The blueprint records the three proved real-part auxiliary estimates and
the complex second estimate separately and
...
The separate retained proof obligation
\leanid{combined\_line\_measurement\_consistency} must establish
consistency of the unconditioned X-Z-X measurement with the supplied
polynomially controlled joint point family on the original product of
line-point distributions, in every opposite placement. Its discharge by
the source point-to-line comparison and pasting argument is described in
...
The Lean theorem \leanid{subline\_remove\_X\_factor\_direct} retains
its existing projective-setting and directly indexed subline hypotheses,
the defined measurement, and the same complex-modulus conclusion and error
scale. Its proof introduces no public assumptions. The original real-part
theorem and the first and third real-part estimates retain their statements.
...
conclusion for the second estimate is now proved on that domain.
...
scalar conclusion is proved for the directly indexed auxiliary law;
