...
Pauli-basis check bound its evaluated inconsistency with the given marginal.
...
Completeness and positivity then bound the non-encoding mass by the full
polynomial inconsistency. No support estimate is inserted as an extraction
hypothesis.
...
explicit \path{IsEncoding} hypothesis.  The complementary estimate
\path{nonencodingMarginalMass_le} is also proved, in issue~\#517, with its
original assumptions and conclusion. Its proof depends only on the standard
Lean axioms \path{propext}, \path{Classical.choice}, and \path{Quot.sound}.
The auxiliary module \path{MIPStarRE/QPBT/Extraction/NonencodingSupport.lean}
provides the evaluated reference comparison and the support reduction.
...
source's measurement-consistency conclusion uses the separate
non-encoding-mass estimate, now proved for a supplied global polynomial-pair
witness. Constructing that witness and completing the remaining extraction
