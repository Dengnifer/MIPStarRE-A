...
evaluations coincide.  The two player-specific estimates needed for the
pulled-apart point comparisons are now proved from these ingredients.  The
single theorem packaging the estimate uniformly over either player remains an
open proof obligation; the bound is not inserted as an extraction hypothesis.
...
results to one constructed global witness.  The more general packaged theorem
\path{nonencodingMarginalMass_le} remains open, but it is not used by that
source-facing composition.  Issue~\#19 owns the Stage~4.2 statements and this
discrepancy record; \ghissue{47} owns the general non-encoding support theorem.
...
conclusion are proved and used.  The single theorem packaging the same estimate
uniformly over either player remains an open proof obligation.
