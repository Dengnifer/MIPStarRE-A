...
\section{The domain of the paired-line helper}

The prerequisite paired-line lemma, \path{lem:qld-xz-lines}, concerns two
lines $\ell_X,\ell_Z$ in $\F_q^m$.  It constructs the POVM
$\{T^{\ell_X,\ell_Z}_{f_X,f_Z}\}$ with degree-$md$ polynomial outcomes,
of degree at most $d$ on an axis-parallel component, and asserts
\[
  \big(T^{\ell_X,\ell_Z}_{[f_X(x)=a,\,f_Z(z)=b]}\big)_{\mathsf A\mathsf A'}
  \simeq_{\delta_P}
  \big(\hat Q^{x,z}_{a,b}\big)_{\mathsf B\mathsf A''},
  \qquad \delta_P=\poly(\eps,md/q),
\]
on the expanded state, averaged over two independently sampled line-point
pairs, together with all symmetric equivalents.  The point measurements here
are those of \path{lem:qld-4-10}, with error
$\delta_Q(\eps)=\poly(\eps)$.%
\footnote{Both statements occur in
\path{references/qpbt-paper/14_analysis_of_the_pauli_basis_test.tex}:
\path{lem:qld-xz-lines} at lines~882--894 and \path{lem:qld-4-10} at
lines~689--709.  Their blueprint nodes have the same labels in
\path{blueprint/src/chapter/ch15_qpbt_combining.tex}.}
This prerequisite is distinct from the extended-line lemma
\path{lem:qld-4-13}, which constructs $\hat Q^\ell$ for a single line in
$\F_q^{2m+2}$.  The domain repair below concerns the paired-line helper;
it neither changes nor resolves the extended-line error discrepancy.

The paired-line source statement does not take an arbitrary joint point
measurement as an input.  Consequently,
...
The verdict for this prerequisite is therefore a correction of the conditional
helper's domain, with no change to the paired-line source assertion or its
error form $\poly(\eps,md/q)$.
