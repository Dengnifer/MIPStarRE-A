...
the source theorem with its constructed points. Several finite parts now have
kernel-checked proofs in
\path{MIPStarRE/QPBT/Combining/QuadraticPointObstruction.lean}.
The deterministic quadratic family satisfies all point-witness fields at some
scalar error, and its completed coarse-graining is exactly
$\operatorname{some}(\alpha x^2)$. For any normalized state and any POVM
supported on affine polynomials, the formal consistency defect is at least
$1-2/q$ when $\alpha\ne0$, also after completing the answer alphabet.
Averaging over uniform $\alpha$ gives $(1-1/q)(1-2/q)$; the canonical
eight-element field gives the explicit bound $21/32$.
The proof uses the polynomial root bound followed by positivity and
normalization of the outcome probabilities, without any completeness admission.
The finite-error witness lemma is reused from
\path{CombinedPointsWitness.exists_error_of_projective}, proved for issue~\ghissue{509}.
These results are not a kernel-checked refutation of the full auxiliary:
the formal perfect-strategy construction and transport of this scalar average
to the full directly indexed line law, including its mixture mass, remain
to be supplied.
