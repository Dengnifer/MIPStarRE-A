...
\paragraph{Corrected error form (adopted).}
The blueprint adopts
$\delta_{\mathrm{combine}} = m \cdot \poly(\eps, md/q)$ --- concretely,
the first-route bound --- as the operative error form.  The only consumer is
the polynomial-pair lemma, whose imported error
...
The remaining obligation is formal: prove the three chained estimates and the
consumer bound with explicit constants.
...
The blueprint \cite{MIPStarREBlueprint} corrects the combined-lines lemma to
the established error form $m \cdot \poly(\eps, md/q)$.  Its proof derives the
first route and uses that bound in the sequel.  The
...
\path{blueprint/src/chapter/ch15_qpbt_combining.tex}.  The corrected lemma
links to \path{ExtendedLinesWitness} and
\path{exists_extendedLinesWitness_established}.}

The latter Lean theorem has the explicit form
$C m\,\poly(\eps,md/q)$ for a universal $C>0$, which agrees with the
blueprint convention that constants are absorbed into $\poly$.  Its proof
remains open.  The older declaration \path{exists_extendedLinesWitness}
retains the unsupported printed form $\poly(m^2\eps,md/q)$ and is no longer
linked to the corrected blueprint node.
...
uses the corrected first-route form and cites this note; strengthening the
sub-line lemma to license the second route is optional, and no route to the
printed form is currently known.  We do not assert that the printed form is
false.
