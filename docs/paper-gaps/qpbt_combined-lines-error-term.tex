...
not remove this obstruction. The source instead uses the family constructed
by its point lemma, and defines the scalar answer by
$c=\alpha a+\beta b$ at lines~993--1018 of the Pauli basis analysis.

Take $m=d=1$, $\eps=0$, and let $q$ tend to infinity through admissible field
sizes. The Pauli basis test has a perfect projective strategy by
\path{lem:pauli-completeness}.\footnote{See
\path{references/qpbt-paper/08_classical_and_quantum_low_degree_tests.tex},
lines~1232--1255, for the statement and its projective Pauli measurements.}
On its expanded local spaces, choose the supplied joint point measurement on
both sides to have the deterministic answer $(x^2,0)$: the effect for this
answer is the identity, and every other effect is zero. This is a complete
projective measurement. Its self-consistency defect is zero. The ordered-product
distances are finite real numbers on the finitely many placements, so a common
scalar $\delta_Q$ bounds all of them. Thus this family satisfies the supplied
witness hypotheses. No estimate of this scalar in terms of $\eps$ is required.
...
Consequently the unrestricted supplied-point auxiliary is false. This argument
does not refute the existential point-witness declaration or the source
...
mathematical argument above, not a Lean counterexample theorem.}
The assigned signature and its unfinished proof are preserved, with this
obstruction tracked by \ghissue{509}. A future realignment must retain the
polynomial point-error dependency or use the existentially constructed points.
Even after that domain repair, the separate printed-error estimate discussed
below remains open.
