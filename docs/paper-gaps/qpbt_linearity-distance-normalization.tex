...
Fourier--Naimark construction minimizes the error for each input. The
normalization row remains pending independent review and main adoption; the
separate state-preserving padding question remains open.
...
they are enumerated, together with the source applications and exact Lean
caller set, in the accompanying audit.%
...
$2$, but does not make the construction optimal for each input. This is a
proved correction proposed for independent review and main adoption; the
register remains pending, and state-preserving padding remains open.
