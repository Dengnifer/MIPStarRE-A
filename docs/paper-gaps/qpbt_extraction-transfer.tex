...
This completion does not establish Lemma~\path{lem:qld-unitary}. No declaration
yet composes \path{exists_globalPairWitness} with
\path{exists_extractionWitness_ofGlobalPairWitness} to state the source lemma
without a witness premise. The general range-projection and ancilla results
above do not yet instantiate the concrete swap operators or prove the final
isometry comparison for the source soundness theorem.

Issue~\#19 owns this concrete Stage~4.2 interface and discrepancy record.
\ghissue{123} records the source-facing composition work: the pulled-apart
consistency composition is complete, while the source-unitary declaration is
still absent.  \ghissue{47} tracks the separate range-projection transfer needed
...
results; it does not discharge the remaining extraction or soundness steps.
...
source-facing theorem, including at zero error.  The subsequent extraction
witness theorem remains conditional on a supplied global measurement, and no
source-facing declaration yet establishes Lemma~\path{lem:qld-unitary}; the
final concrete isometry transfer remains separate and open.
