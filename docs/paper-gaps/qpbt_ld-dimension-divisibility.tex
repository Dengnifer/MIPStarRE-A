...
\begin{itemize}
  ...
  only $m \mid q$. The directly indexed reformulation and its soundness
  proof are established for $k=1$; their use in the combining subtree
  remains to be verified. Second, the source's import of the classical
  ...
  \item \textbf{Formal status:} direct index sampling, parsing pullback,
  value-preserving correlated-register dilation, and the compression
  argument are proved. They yield all three soundness conclusions with
  error $\delta_{\mathrm{ld}}$ for $k=1$. The proof applies the low
  individual degree theorem directly and avoids the two obligations of
  the printed tensor-code import. The arbitrary-$k$ combining reduction
  and the downstream use at dimension $2m+2$ remain open.
  \item \textbf{Estimated weight:} the completed $k=1$ transport requires
  no further definitions or supporting lemmas. Remaining work concerns
  the arbitrary-$k$ combining reduction and its downstream application,
  rather than the seed-fiber transport proved here.
...
\begin{enumerate}
  ...
  2^{-bm'd})$. This form is proved for $k=1$, as is its transport to the
  seed-indexed game when $m'\mid q$. The proof uses the low individual
  degree theorem directly, rather than the tensor-code import of
  Section~\ref{sec:import-obligations}. Only the extension to arbitrary
  $k$ remains open in this soundness assertion.
\end{enumerate}
...
open for the printed tensor-code route. Both are independent of the
divisibility question and would arise if that route were used for the
directly indexed variant as well. The established $k=1$ proof instead
uses the low individual degree theorem directly, as described below.
...
difficulty described below: averaging preserves the line-versus-point
branches exactly but not the synchronicity branches. The correlated-register
construction avoids this loss in the direct reduction; it does not establish
the printed identification with the tensor-code test.
...
\section{Direct repair and padding alternative}

\paragraph{Direct index sampling (established for $k=1$).}
The formalization defines, for arbitrary $m' \ge 1$, the line-point distribution over
...
$i-1$ coordinates, directly, and the \emph{directly indexed game} with
...
$s \in \F_q$. Soundness for this game (form~4 above) is proved for $k=1$
by a reduction to the low individual degree theorem with failure bound
$3\eps$ and auxiliary parameter $2560000(m')^3d$. Thus the proof avoids
both obligations of the printed tensor-code import. The extension to
arbitrary $k$ remains the combining reduction, not an index-sampling issue.
...
steps. The distribution and pullback identities are proved; independent
seed averaging is inadequate, and the proved transport instead uses a
correlated register as described below.
\begin{itemize}
  ...
  other in general. The proved construction avoids independent averaging:
  a maximally correlated register of dimension $q/m'$ selects the same
  residue for both players. Block-diagonal measurements reconstruct the
  seed from its index and residue. This preserves projectivity and the
  value of every branch, including synchronicity, exactly
  (\leanid{ldStrategyToDirect\_value\_eq}).
  \item \emph{Recovery on the original Hilbert spaces.} Apply direct
  soundness to this extended strategy, then average the diagonal blocks
  of each polynomial measurement. The resulting POVMs satisfy the two
  point-versus-polynomial consistency bounds exactly. Compression need
  not preserve global polynomial consistency, since it removes the shared
  residue correlation. Instead, the point-versus-point branch gives a
  $9\eps$ bound on the original state. Chaining the point relations and
  applying the polynomial collision estimate with loss $m'd/q$ recovers
  global polynomial consistency there. For $k=1$, absorption into
  $\delta_{\mathrm{ld}}$ completes the round trip, as proved in
...
a hypothesis. The transport identities are established for general $k$;
the soundness round trip is complete for $k=1$, while simultaneous
polynomial measurements for arbitrary $k$ still require the combining reduction.
...
repair, direct index sampling is established for the simultaneity parameter $k = 1$,
with dimension padding remaining the fallback confined to the present game
definition.

Concretely: the imported soundness theorem is restated for the directly
indexed game at arbitrary dimension (\leanid{exists\_direct\_ld\_soundness}
in \doclink{MIPStarRE/QPBT/Combining/DirectLowDegree/Soundness.lean}), whose
question law is defined for every admissible tuple, so the divisibility
obstruction described above does not arise there; the
distribution and parsing-pullback lemmas are proved in
\doclink{MIPStarRE/QPBT/Combining/DirectLowDegree/Transport/Correspondence.lean};
and the de-seeding round trip is settled by dilating the seed-indexed
strategy with a maximally correlated register indexed by the seed fibre,
which preserves the value exactly
(\leanid{ldStrategyToDirect\_value\_eq}), and compressing that register
afterwards.  The compression is exact for the two point-versus-polynomial
relations and not for the global one; the global relation is instead
recovered on the original Hilbert spaces from the point-agreement branch of the
game, as described in
\doclink{MIPStarRE/QPBT/Combining/DirectLowDegree/SeedIndexedSoundness.lean}.
For $k = 1$ the round trip therefore does yield the source-shaped soundness
statement for the seed-indexed game, as
\leanid{exists\_ld\_soundness\_of\_k\_eq\_one}, and its directly indexed form
as \leanid{exists\_direct\_ld\_soundness\_of\_k\_eq\_one}.

Neither of those two theorems relies on the asserted identity with the
two-prover tensor-code test, and neither instantiates Theorem~4.7 of
\cite{Ji2021Quantum}: both apply the low individual degree theorem
\leanid{MIPStarRE.LDT.Test.mainFormal} directly, at the auxiliary sampling
parameter $2560000\,m^3 d$, which satisfies that theorem's own large-sampling
hypothesis for every admissible tuple.  The two import obligations of
Section~\ref{sec:import-obligations} are thus avoided rather than
discharged: they remain the open content of the source's printed route, and
the arbitrary-$k$ statements keep that route and their tracked proof debt.
What remains open for the directly indexed route itself is the extension
from $k = 1$ to arbitrary $k$, which is the combining reduction analysed in
\cite{gap:qpbt_ld-simultaneous-sandwich} and not a divisibility question,
and the rereading of the combining subtree against the directly indexed
game.
