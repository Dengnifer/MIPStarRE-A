...
\begin{itemize}
  ...
  seed-indexed soundness theorem on its original domain, and the final
  ...
  comparisons are not thereby proved.
...
the outcome tuple. The Lean theorem proves all three relations for every
positive $k$, with no tensor-code correspondence or parameter-bound
hypothesis. Its point families post-process answers of the wrong type to
the zero tuple; thus the displayed comparison uses complete POVMs, with
that convention explicit.\footnote{\leanid{exists\_ld\_soundness},
...
This established theorem must be distinguished from its use outside that
game's parameter domain. The extended-dimension comparison has four forms.
\begin{enumerate}
...
soundness for every $k$ on its original domain $m'\mid q$, with possible
...
provider remark likewise distinguishes the proved seed-indexed theorem
from the printed import. These remarks record a documented deviation;
...
without $2m+2\mid q$. The seed-indexed classical soundness theorem is also
proved, for every positive simultaneity parameter, on its unchanged domain
$m'\mid q$. These proofs use the low individual degree theorem, not the
printed tensor-code import.
