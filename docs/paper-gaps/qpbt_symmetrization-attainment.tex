...
\begin{itemize}
  ...
  \cite[Section~5]{Ji2020MIPStar}.  The blueprint correction is audited in
  \ghissue{173}.
\end{itemize}
...
and its proof at lines~101--132.  The printed source statement is recorded next
to the corrected \path{lem:symmetric-strat} in
...
the given-strategy form.  The blueprint records that form as part of the games
vocabulary and has no downstream dependency on it; the two source uses above
belong to transformations outside the scope of the present development.
Consequently, no consumer of the lemma --- in the source or here --- requires
the attainment form.

\section{Comparison with the blueprint}

The blueprint \cite{MIPStarREBlueprint} now states the given-strategy form and
cites this note at the correction.  Its proof is exactly the value-preserving
block construction above; an accompanying remark records the stronger printed
attainment form and explains why it does not follow.  The linked declaration
\leanid{exists\_symmetric\_projective\_strategy\_of\_strategy} has the same
hypothesis, exact value equality, and projective symmetric conclusion.  Its
proof remains open.  The separate source-shaped declaration
\leanid{exists\_symmetric\_projective\_strategy} remains an unproved record of
the unsupported attainment claim and is not advertised by the corrected node.
...
attained.  The blueprint uses the valid given-strategy form and displays the
source discrepancy explicitly.  This correction is harmless downstream,
because every source use proceeds either from a given strategy or from a
strict lower bound on the value.  The source-shaped attainment declaration
remains separate and unproved rather than serving as the blueprint theorem.
