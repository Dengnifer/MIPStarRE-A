...
at the identity follows from exact linearity as above.  Finally, the provider
prints the bound $\delta$ for squared operator distance, whereas its proof gives
the bound $2\delta$, or equivalently $\delta$ for the associated binary-
...
operator bound.  No Lean declaration yet supplies the finite common-extension
construction for the family indexed by $(x,z)$.
...
The quotation has two independent discrepancies: it changes averaged closeness
to pointwise closeness, and the provider's displayed operator-distance bound
misses a factor of two.  The single-family theorem is now proved with mean
squared operator error at most $2\delta$, equivalently mean squared binary-
measurement error at most $\delta$, without an upper bound on $\delta$.
Its use for the family indexed by $(x,z)$ still requires an explicit finite
common-extension argument; that is the remaining open formal reduction.
