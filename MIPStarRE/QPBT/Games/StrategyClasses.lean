...
The source proof of `lem:symmetric-strat`
(`references/qpbt-paper/06_nonlocal_games_and_mipstar.tex:101-132`) chooses,
for every `ε' > ε`, a strategy of value at least `1 - ε'` from the supremum
defining `Game.value`, and symmetrizes it.  The declarations below formalize
exactly that passage: the supremum property of `Game.value` over the
nonempty-answer domain, composed with the value-preserving construction
`exists_symmetric_projective_strategy_of_strategy`.  The remaining attainment
step at `ε' = ε` is the open source gap recorded in
`docs/paper-gaps/qpbt_symmetrization-attainment.tex`; it is not proved here. -/
