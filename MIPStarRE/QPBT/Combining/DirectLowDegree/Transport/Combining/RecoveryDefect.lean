...
* `consistencyDefect_le_of_recoveryBound` — the transport of a consistency
  relation along a relabelling which is correct off a rare event.
...
/-- Transport of a consistency relation along the recovery relabelling.  The
first measurement is complete at every question, so the total Born weight of an
outcome of the second measurement does not depend on the question; the bound on
the discrepancy event may therefore be averaged over the question against that
...
theorem consistencyDefect_le_of_recoveryBound
  ...
  have hkey : avgOver (uniformDistribution X)
    ...
    have hstep : ∀ x : X,
        avgOver (uniformDistribution Y) (AgrH x) ≤
          AgrG x + ∑ g : Γ, W g * F x g := by
      intro x
      have hexp : avgOver (uniformDistribution Y) (AgrH x) =
      ...
      have hsum : (∑ b : β, ∑ g : Γ,
          ...
          AgrG x + ∑ g : Γ, (∑ b : β, w x b g) * F x g := by
        rw [hAgrG]
        simp only
        calc (∑ b : β, ∑ g : Γ,
              ((if f x g = b then w x b g else 0) + w x b g * F x g))
