...
/-- Coordinatewise coding identifies direct points with mature LDT points. -/
noncomputable def directPointEquiv (D : DirectLdParams) :
    (Fin D.m → DirectScalarQ D) ≃ Point D.toLDTParameters :=
  Equiv.piCongrRight fun _ => directScalarEquiv D

@[simp] theorem directPointEquiv_apply (D : DirectLdParams)
    (u : Fin D.m → DirectScalarQ D) (i : Fin D.m) :
    directPointEquiv D u i = directScalarEquiv D (u i) :=
  rfl

@[simp] theorem directPointEquiv_symm_apply (D : DirectLdParams)
    (u : Point D.toLDTParameters) (i : Fin D.m) :
    (directPointEquiv D).symm u i = (directScalarEquiv D).symm (u i) :=
  rfl
...
private theorem distribution_eq_of_support_eq_of_toPMF_eq
    {alpha : Type*} (mu nu : Distribution alpha)
    (hmu : mu.IsProbability) (hnu : nu.IsProbability)
    (hsupport : mu.support = nu.support)
...
private theorem uniformDistribution_map_fst_of_equiv
  ...
  · calc
      ...
      _ = (PMF.uniformOfFintype alpha).map (Prod.fst ∘ e) := by
        exact congrArg
