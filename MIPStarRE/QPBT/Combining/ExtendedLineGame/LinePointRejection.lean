...
This module identifies the axis- and diagonal-line/point branch rejections of
the strategy constructed from an `ExtendedLinesWitness` with the corresponding
completed line-point defects.  These are finite postprocessing and
state-transport steps in the first paragraph of the proof of `lem:qld-4-7`;
they do not establish a passing-value bound or construct the supplied witness.

## References

- `references/qpbt-paper/14_analysis_of_the_pauli_basis_test.tex:1279-1288`
- Blueprint `lem:qld-4-7`.
- Issues #302, #305, and #307.
-/
