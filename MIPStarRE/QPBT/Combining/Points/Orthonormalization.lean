...
This is the application of `lem:ortho` in the proof of `lem:qld-4-10`, paper
...
`AA'`, `BA''`.  Both the pointwise and the averaged bound are on squared
distances. -/
...
`BA''`, `AA'`.  Both the pointwise and the averaged bound are on squared
distances. -/
