...
The source nodes are `def:introparams` and `lem:delta-bound` in
...
/-- The admissibility predicate for a raw `(q, m, d)` tuple. This Lean-only
view is used to separate the numerical proof from the structure constructor. -/
def IsAdmissibleTuple (t : ℕ × ℕ × ℕ) : Prop :=
  IsAdmissibleSize t.1 ∧ 1 ≤ t.2.1 ∧ 1 ≤ t.2.2 ∧ t.2.1 ∣ t.1

/-- Convert a raw admissible tuple to the parameter structure used by the
Pauli basis game. -/
...
/-- The raw canonical `(q, m, d)` tuple of `def:introparams`. -/
def introParamsTuple (c R : ℕ) : ℕ × ℕ × ℕ :=
  (2 ^ (c * Nat.clog 2 (Nat.clog 2 R) + 1),
    2 ^ Nat.log 2 (c * Nat.clog 2 R + 1), 1)

/-- The canonical raw tuple is admissible. This is the numerical
...
/-- The canonical admissible parameter package of `def:introparams`,
...
field exponent and `m`; see `rem:delta-bound-exponent-comparison`. -/
