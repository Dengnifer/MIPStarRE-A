#!/usr/bin/env python3
"""Per-subagent token usage across both worker pools.
...
Sessions that bypass dispatch.sh (the operating session's own helpers, an external
  prover): results/telemetry/owner-sessions.jsonl, field `tokens` = the total its harness
  reported for that subagent,
...
other = load('results/telemetry/owner-sessions.jsonl')
by = collections.defaultdict(lambda: collections.Counter())
rows = []
for s in codex:
    u = s.get('usage') or {}
    tot = (u.get('input') or 0) + (u.get('output') or 0)
    key = ('codex:' + str(s.get('dispatcher') or s.get('model') or 'unknown'), s.get('role') or '?')
...
# Non-dispatch sessions: prefer end records (status != running); drop a running stub if any other record shares its issue+role+model and has tokens
ended = [s for s in other if s.get('status') != 'running']
for s in ended:
    key = ('other:' + str(s.get('model')), s.get('role') or '?')
    by[key]['sessions'] += 1; by[key]['total'] += s.get('tokens') or 0; by[key]['wall_s'] += s.get('wall_s') or 0
    by[key]['tool_uses'] += s.get('tool_uses') or 0
    rows.append(('other', s.get('name'), s.get('role'), s.get('issue'), (s.get('start') or '')[:16], s.get('tokens') or 0, s.get('wall_s')))
still = [s for s in other if s.get('status') == 'running']
if '--md' in sys.argv:
    print('# Subagent token usage (generated %s)\n' % datetime.datetime.utcnow().strftime('%Y-%m-%dT%H:%MZ'))
    print('Codex totals count input + output tokens (cached input shown separately); other totals are the harness-reported subagent totals.\n')
    ...
    print(f"\nRecords still marked running (no end record yet): {len(still)}")
else:
