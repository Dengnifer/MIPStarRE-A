...
if tmux capture-pane -p -t "$S" | grep -q "gpt-5.6-sol"; then
  # wait (up to 15 min) for the turn to end, then hard-exit with double Ctrl-C
  for _ in $(seq 1 90); do tmux capture-pane -p -t "$S" | grep -q "esc to interrupt" || break; sleep 10; done
  tmux capture-pane -p -t "$S" | grep -q "esc to interrupt" && echo "still working after 15 min; interrupting anyway"
