...
A reporting tool for a project that runs more than one model: it groups the
recorded sessions by role and model and prints the medians side by side.  The
assignment of models to tasks is never random, so nothing it prints is causal.
Standard library only.  Re-run with:
...
CACHE_ROOT = os.path.expanduser(os.environ.get("MIPSTARRE_CACHE_ROOT")
                                or "~/.cache/mipstarre-dev")
LANES = os.path.join(CACHE_ROOT, "watchdog/lanes")
PULLS_CACHE = os.path.join(CACHE_ROOT, "model-comparison/pulls.json")
REPO = "OWNER/REPO"
...
def derive_model(row, lanes):
    ...
    # No registry field, no stream, no lane line: say so.  (The origin project
    # had a dated "before/after the switch" rule here; that is its history, not
    # a rule any other project can apply.)
    return "unknown", "none"
...
def main():
    ...
    parts = ["# codex model comparison by role", "",
             "Generated by `results/telemetry/model-comparison/compare.py` at %s." % now,
             "The assignment of models to tasks is not random, so nothing here",
             "is causal.", "",
