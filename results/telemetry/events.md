...
## 2026-09-09 - PR491 failed refresh conflict repair

- The daemon refresh stopped at 08:09:47Z on three conflict blocks in
  `Combining/Points/Consistency.lean`. Both sides replace the former local
  operator-product helper with `DistanceCalculus.applyOperatorToState_mul`;
  the resolution retains the branch's explicit namespace qualification.
- The immutable parents are `4c836c5441b584883707b3991bc31c6655d52b67` and
  `a45258248d39dccc43d3a4a5d451ed4a1cfbc335`. All 3365 original stage-zero
  entries were verified unchanged before this incident append. The pending
  index, staged patch, and conflicted source were preserved in
  `~/.cache/mipstarre-dev/recovery/pr491-orc-490-20260909-01/`.
- The canonical merge-loss guard and the focused consistency-file check pass.
  Statement integrity: the assumptions, conclusions, and proof steps from both
  parents are preserved; the cited sandwich argument is in
  `references/qpbt-paper/14_analysis_of_the_pauli_basis_test.tex:743-790`.
  No new mathematical proof attempt, proof hole, or budget reset is involved.
  Exact-head CI and independent review remain publication requirements.
- The first pre-commit run passed 615 tests but its routine-model fixture
  inherited this session's `MIPSTARRE_HARDNESS_REASON`, causing the model-policy
  preflight to reject the routine classification. Validation subprocesses
  clear only that inherited justification; the model policy, actual session
  identity, admission controls, and cumulative proof budget remain unchanged.
- Clearing only the justification exposed four fixtures that also inherited
  `MIPSTARRE_JOB_CLASS=source_semantic`. Clearing both classification and
  justification in validation subprocesses passes all 31 focused dispatch and
  model-policy tests. This is test-environment isolation, not a workflow or
  model-policy amendment; no repository scripts were changed.
