...
- 2026-09-09: PR469 checked publication at `ed064873` stopped before transport:
  the reverse blueprint coverage check's `git diff --merge-base` found three
  best merge bases with `origin/main`. No gate was bypassed. The issue branch
  merged verified published main `1c297a4b`, preserving incoming changes and
  both sides of the append-only event log conflict. The primary checkout was
  not changed. The merge-loss guard is used before publication is retried.
