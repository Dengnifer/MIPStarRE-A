...
### 2026-09-09 - PR449 failed-refresh recovery in orc-445-20260909-01

- The deferred refresh exited without model admission. This session began at
  2026-09-09T21:39:03+0800 with a 45-minute working limit. Earlier session costs,
  failures, review findings and preserved telemetry remain historical records;
  no budget or attempt count is restarted. The task is completed in this
  session without another model session or an independent-review claim.
- The pending merge has immutable parents
  `62989f46d068b6faabb931e562846a8149aeefe8` and
  `0dbd4a9cd97f292931a9eca3164e7152b47bc75f`. Resolve the five Lean conflicts by
  retaining both import sets, the explicit shared operator rewrites, the
  measurement contraction proofs, the corrected Parseval column description,
  the real parts in overlap formulas, and the incoming paper-gap citations.
  Prior replacements of private placement helpers by the shared API remain.
- Statement integrity: the paper assumptions are the projective strategy and
  the expanded-measurement commutation and consistency bounds of
  `lem:qld-comm-cons`. The sandwich calculation in
  `references/qpbt-paper/14_analysis_of_the_pauli_basis_test.tex:731-770`
  transports each factor using projectivity, completeness and consistency.
  Lean retains the same `ProjectiveSetting` hypotheses and opposite placements,
  and the same sandwich and ordered-product squared-distance conclusions with
  universal constants. The separate supplied-marginal auxiliary at
  `eq:pasting-q1` (paper lines 936-941) retains both exact bounds
  `8 * deltaQ + C * (epsilon + deltaLine epsilon)`. Verdict: unchanged faithful
  encoding, with the existing documented direct field-valued construction;
  no added assumptions, weakened conclusions or new proof holes.
- The pending-index merge-loss guard passed. All 96 public-header comparisons
  in the five resolved files agree with their respective immutable parents.
  Both parents' session, build and stage rows are retained with multiplicity.
  The direct Sandwich Lean check passed. Publication, canonical CI and the
  exact-head independent-review handoff are recorded in this session's final
  receipt; failed-refresh markers remain until repaired publication is verified.
