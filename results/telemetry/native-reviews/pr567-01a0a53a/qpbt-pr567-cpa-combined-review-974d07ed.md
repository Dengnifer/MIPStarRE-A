<!-- mipstarre-review pr=567 head=974d07ed4466736c87f63777f640fa9a9653361b -->
# Independent Two-Pass Review: PR567
...
Code pass: APPROVED. Blueprint/prose pass: APPROVED. Worst pass verdict: APPROVED.

Prepared for MAIN validation and publication; not published by this reviewer.
...
No unresolved in-scope code findings.

### Proof And Dependencies

`MIPStarRE/QPBT/Extraction/Construction.lean:46` retains its public statement. Lines 68-80 obtain the universal constants before fixing the parameters and strategy, then construct a single global polynomial-pair witness. Lines 95-101 apply both point comparisons and the fixed-coordinate observable comparison to that same witness. The witness is not an input, and it cannot vary with the Pauli basis or basis coordinate.

The maximum `C = max (max C_AB C_BA) C_obs` is a valid common coefficient. The proof obtains `1 <= C` from the first coefficient's lower bound and proves each coefficient is at most `C`. It establishes nonnegativity of `deltaQld` and of `B = deltaG + sqrt epsilon + md/q`, then uses `mul_le_mul_of_nonneg_right` to enlarge each estimate. No strict positivity of epsilon, division by epsilon, or zero-error exception is introduced.

`MIPStarRE/QPBT/Combining/Apply.lean:253` constructs the witness over the full strategy-error domain, using the established directly indexed construction and the rounded polynomial-pair estimate. Its statement has no witness, divisibility-at-dimension-2m+2, or residual input. The original seed-indexed proof's separately documented obligations are not assumptions of this construction. `GlobalPairWitness` at `Combining/Witnesses.lean:294` records actual complete projective measurements and the two expanded point-consistency conclusions, not the pulled-apart conclusions being proved.

The two point proofs at `SuppliedPointConsistency.lean:107` and `:226` genuinely bound the non-encoding mass: their local estimates at lines 131 and 250 use the encoding-supported reference comparison and Schwartz--Zippel, then add the witness error. They do not invoke the open uniform theorem `nonencodingMarginalMass_le`. The observable proof at `ObservableConsistency.lean:49` uses the evaluated difference-polynomial comparison, collision estimate, and character conversion. It does not invoke the open complement-absorption theorem.

The three changed helper docstrings now say `Conditional` and name the completed construction that supplies their premise. Removing their former `Unfaithful` paragraphs is justified specifically by the new source-facing composition. This does not justify deleting the downstream markers in `Extraction/Unitary.lean:89`, `:126`, or `:150`, or the corresponding state-extraction markers; the PR leaves them in place.

No new declaration, structure field, axiom, arbitrary implication hypothesis, default witness, or changed mathematical definition is introduced. The imports select the actual construction and estimate modules; the previous definitions remain available transitively. The short scalar proof uses existing order and real-power APIs, with no new global attributes, linter suppression, or unbounded tactic options.

### Previous Code Finding

The completed prior review 5209322228's code finding is fixed at the current head. `docs/paper-gaps/qpbt_extraction-transfer.tex:144` now identifies the proved witness construction and pulled-apart composition, while line 153 separates the still-unformalized source-unitary composition and later range-projection transfer. The register's corresponding entries at `docs/paper-gaps/qpbt-gap-register.md:19` and `:21` agree. `qpbt_decoding-identity.tex:144` distinguishes the two proved specialized mass estimates from the open uniform theorem at line 151. All three modified helper docstrings make the same distinction.

The final correction commit does not modify `Construction.lean` at all; its Lean changes in the two other files are docstrings only. Thus the claim that it corrects records without changing proofs or statements is substantiated by the commit diff, not inherited from the prior verdict.

### Remaining Mathematical Work

`nonencodingMarginalMass_le` and `marginalPoly_sub_pointMeas_approx_zero` retain their tracked proof holes in `Consistency.lean:806` and `:758`. Their axiom closures still contain `sorryAx`; neither is in the completed theorem's axiom closure. `exists_extractionWitness_ofGlobalPairWitness` remains proved only at its explicitly conditional statement (`Unitary.lean:171`), and there is no new source-unitary or final isometry-transfer declaration in this patch. These are existing tracked obligations, not findings deferred by this review.

Code pass verdict: APPROVED.
...
Blueprint/prose pass verdict: APPROVED.
...
| Reviewed path | SHA256 |
|---|---|
| `MIPStarRE/QPBT/Extraction/Construction.lean` | `f8b6f9365e92d58a9becf1c396fd42bc4127162f74bb0bb3ed26c239d21877f3` |
| `MIPStarRE/QPBT/Extraction/ObservableConsistency.lean` | `17326cd07e5b1e4b8742ff2fd5379408e37d456fa16db9f8b34ffe2f05b53687` |
| `MIPStarRE/QPBT/Extraction/SuppliedPointConsistency.lean` | `8e3eca94f0765602353c07801a386714a3399fa6407e20854803d82f093a6591` |
| `blueprint/src/chapter/ch16_qpbt_extraction.tex` | `4cb83d0e03f1ba9a12e619991e1ae7f076847734fb624df6a7e843da2ddbf091` |
| `docs/paper-gaps/qpbt-gap-register.md` | `943d80219ead29b2890225b1ac808506900c08f167c0ffa02e2ed4f01aa4b897` |
| `docs/paper-gaps/qpbt_decoding-identity.tex` | `6166c2c7ba8756216678ca0a300479fb45b8ff6625dc992800686720ab37e8a6` |
| `docs/paper-gaps/qpbt_extraction-transfer.tex` | `cd675f491dad04051f6cb18dea7b0c430ab774ec9223ff5c371396f9a1505a3b` |
...
The final delivery supplies the report hashes and post-report head/clean recheck. No finding has been silently carried forward, deferred, or adjudicated. The failed transport attempts remain failures without substantive verdicts; this is a fresh review of the pinned head, not their reinterpretation.
