...
this readout. In Lean, the Boolean values false and true select $XZ$ and
$ZX$, respectively. The outcome sum is over every singleton polynomial
tuple, without a support restriction.
\end{definition}
...
\begin{proof}
\leanok
\uses{lem:qld-opposite-ordered-products, def:qld-extended-polynomial-ordered-error}
...
The first two calculations are shared with the recovered polynomial-image proof.
\end{proof}
...
The Boolean parameter false selects the first order and true the second.
...
$\combine_{g_X,g_Z}$ as a formal polynomial.
\end{theorem}
\begin{proof}
\leanok
...
There is a nonzero nonconstant coefficient $c(z)$ whose zero set has
probability at most $C/|F|$. For every $z$ outside this one set and every
...
the existing bounded combining map $(g_X,g_Z)\mapsto\combine_{g_X,g_Z}$.
\end{definition}
...
Theorem~\ref{thm:qld-rounded-polynomial-ordered}, the actual rounded
measurements satisfy both ordered estimates with $E=4\eta+8\delta_Q$.
They also satisfy both wrong-variable bounds $2E+2(2md+2)/q$ and the
scalar-nonlinear bound of Theorem~\ref{thm:qld-rounded-scalar-linearity}.
For the same measurements on both heterogeneous placements,
\[
...
\begin{definition}[Restriction and completion on polynomial pairs]
...
\leanok
For admissible parameters, write $j(g_X,g_Z)=\combine_{g_X,g_Z}$.
Substituting the scalar coordinates $(1,0)$ and $(0,1)$ and setting the
unused base block to zero recovers the two formal polynomials, so $j$ is
injective without any field-size restriction. Evaluation satisfies
$j(g_X,g_Z)(x,z,\alpha,\beta)=\alpha g_X(x)+\beta g_Z(z)$.
Given a complete measurement $R$ on bounded extended polynomials, define
$U_{g_X,g_Z}=R_{j(g_X,g_Z)}$. Its missing operator is
$D=I-\sum U=\sum_{g\notin\operatorname{range}j}R_g$.
For a fixed pair $p_0$, relabel each combined outcome by its pair and each
remaining outcome by $p_0$. The resulting complete measurement has effects
$C_p=U_p+1_{p=p_0}D$. If $R$ is projective, both $U$ and $C$ are projective.
These are the algebraic constructions in the completion paragraph of the
proof of Lemma~\ref{lem:qld-4-7} in \cite{Ji2020MIPStar}.
\end{definition}
...
polynomials over $\F_q$. Support inclusion transports the individual-degree
certificate in both directions. Put $j_{\rm dir}=e^{-1}\circ j$ and relabel
the actual rounded measurement by $e$; its effect at $h$ is exactly
$R_{e^{-1}(h)}$. Apply Definition~\ref{def:qld-pair-restriction-completion}
to obtain $C$. This preserves projectivity and every outcome.
Membership in the range of $j_{\rm dir}$ is precisely equality, after field
transport, to $\combine_{g_X,g_Z}$ for a bounded pair. The actual direct
readout of $j_{\rm dir}(g_X,g_Z)$ is
...
\leanok
\uses{thm:qld-retained-point-overlap, def:qld-pair-restriction-completion}
...
\]
More generally, completion has defect at most $G+K_W$, hence at most
$G+2E^W+4/q$, on each opposite placement.
This auxiliary explicitly assumes the two displayed witnesses. The source
Lemma~\ref{lem:qld-4-7} remains open pending their construction from its
original hypotheses and absorption of this actual error into $\delta_{\rm qld}$.
\end{theorem}
\begin{proof}
\leanok
Completion only increases the retained overlap, because its added operator
and the opposite point effect are commuting positive operators. Combine
the preceding retained bounds with $G\leq6E+(12md+4d+10)/q$.
