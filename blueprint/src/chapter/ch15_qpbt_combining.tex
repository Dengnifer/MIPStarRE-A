...
\begin{remark}[Divisibility obstruction in the application of the classical
	...
	divides $q$. An admissible field size
	...
	every admissible $q$ except $q = 2$. Thus the extended seed-indexed
	construction is defined exactly in the exceptional range $m=1$,
	$q\ge8$. The source proof in \cite{Ji2020MIPStar} performs this
	instantiation without addressing its parameter domain.

	The formalized proof of Lemma~\ref{lem:qld-4-7} instead uses the
	directly indexed auxiliary game: its axis and diagonal prefix indices
	are sampled uniformly from $\{1,\ldots,2m+2\}$, and line descriptions
	carry those indices rather than field seeds. Its soundness follows from
	Theorem~\ref{thm:main-formal} and the simultaneous combining reduction
	of Proposition~\ref{prop:ld-simultaneous-general-k}. The direct subline
	distribution of Lemma~\ref{lem:qld-sublines-direct} and the extended
	line measurements of Lemma~\ref{lem:qld-4-13-established} supply the
	proved polynomial-pair conclusion for every admissible tuple. No
	additional divisibility hypothesis is imposed on that conclusion.

	This is a documented deviation of the intermediate construction, as
	explained in \cite{gap:qpbt_ld-dimension-divisibility}. The direct law
	is not identified with the source law: where the seed law is defined,
	the source subline comparison still requires the appropriate line and
	separate projected-point marginals and evaluation transports. The
	source-shaped Lemmas~\ref{lem:qld-sublines} and~\ref{lem:qld-4-13}
	are retained without certifying those comparisons. Separately,
	Theorem~\ref{lem:ld-soundness-formalized} proves seed-indexed soundness
	for completed point POVMs on its original divisible-dimension domain
	by correlated seed dilation, compression, and polynomial agreement.
	The prescribed-effect comparison is distinguished in
	Remark~\ref{rem:ld-soundness-provider}. Neither this theorem nor the
	direct proof establishes the printed tensor-code game correspondence
	or the parameter substitution discussed in
	Remark~\ref{rem:ld-soundness-provider}.
\end{remark}
