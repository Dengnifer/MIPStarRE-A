...
paper's construction of the combined point measurement proceeds in three steps: the
...
assembled by Fourier averaging. The formal proof of Lemma~\ref{lem:qld-4-10}
instead forms the field-valued sandwich directly and applies orthonormalization
on the existing expanded spaces. The linearity theorem and its import
obligations below describe the source's route.
...
\begin{proof}
	...
	The obligations attached to the source's application of this theorem --- the
	...
	Remark~\ref{rem:linearity-import}. The direct proof of
	Lemma~\ref{lem:qld-4-10} below avoids this application.
\end{proof}
...
\begin{remark}[Obligations of the linearity-theorem import]
	...
	load-bearing for the source's proof of Lemma~\ref{lem:qld-4-10}: the error is
	...
	conversion steps in the source's argument for
	...
	definition. For that route, one obligation remains open: the
	ancilla register $\hilb'$ and its state
	...
	Lemma~\ref{lem:qld-4-10} below does not use this import or require this padding.
\end{remark}
...
\begin{lemma}[Field-valued point commutation]
	\label{lem:field-point-commutation}
	\lean{MIPStarRE.QPBT.ProjectiveSetting.expPoint_comm}
	\leanok
	\uses{def:expanded-point-measurement, def:symmetric-equivalents, def:povm-distance}
	There is a universal $C\ge1$ such that, in every projective setting,
	for every placement $p$, writing
	$X_a=\hat M^{(\mathrm{Point},X),x}_a$ and
	$Z_b=\hat M^{(\mathrm{Point},Z),z}_b$ on its local space,
	\[
		\E_{x,z}\sum_{a,b\in\F_q}
		\bigl\|(X_aZ_b-Z_bX_a)_p\ket{\hat\psi}\bigr\|^2
		\le C\sqrt{\eps}.
	\]
	This is a formalization-only field-valued version of the commutation
	estimate used in Lemma~\ref{lem:qld-comm-cons}.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:qld-win-implications-obs, def:expanded-observables,
		prop:fourier-fact-scalar, def:expanded-state}
	Expand both point effects by Fourier inversion. Character
	orthogonality identifies the summed squared commutators with the
	average squared commutator of the expanded observables. The ancillary
	Pauli factor is an isometry, leaving the original-state twisted
	commutator estimate. Parseval's identity introduces no field-size loss.
\end{proof}

\begin{definition}[Field-valued point sandwich POVM]
	\label{def:field-point-sandwich}
	...
	For either player and $x,z\in\F_q^m$, set
	$R^{x,z}_{a,b}=Z_bX_aZ_b$, with $X_a,Z_b$ as above.
	This is a POVM: $Z_bX_aZ_b=(X_aZ_b)^\dagger(X_aZ_b)\ge0$,
	and summing over $a,b$ gives the identity by completeness and
	projectivity. This formalization-only variant of the binary sandwich
	in the proof of Lemma~\ref{lem:qld-4-10} has outcomes in $\F_q^2$.
\end{definition}

\begin{lemma}[Exact overlap of two sandwiches]
	\label{lem:point-sandwich-overlap}
	\lean{MIPStarRE.QPBT.sandwich_overlap_identity}
	\leanok
	\uses{def:povm-conventions}
	Let $X_1,X_2$ and $Z_1,Z_2$ be projective measurements on a common
	finite-dimensional space, with finite outcome sets $\mathcal A$ and
	$\mathcal B$, respectively. Assume, for every $a,b$,
	$[X_1(a),Z_2(b)]=[Z_1(b),X_2(a)]=[Z_1(b),Z_2(b)]=0$.
	Write $R_i(a,b)=Z_i(b)X_i(a)Z_i(b)$ and $W_b=Z_1(b)Z_2(b)$.
	For any vector $\psi$,
	\[
		\begin{aligned}
		\sum_{a,b}\Re\langle\psi,R_1(a,b)R_2(a,b)\psi\rangle
		&=\|\psi\|^2-\frac12\sum_b\|(Z_1(b)-Z_2(b))\psi\|^2\\
		&\quad-\frac12\sum_{a,b}\|(X_1(a)-X_2(a))W_b\psi\|^2.
		\end{aligned}
	\]
	This formalization-only identity replaces the overlap calculation in
	the proof of Lemma~\ref{lem:qld-4-10}.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:distance-projective-point-distance, lem:distance-point-defect}
	Cross commutation gives
	$R_1(a,b)R_2(a,b)=W_b^\dagger X_1(a)X_2(a)W_b$.
	Apply the projective distance-defect identity first to $X_1,X_2$
	in $W_b\psi$, then to $Z_1,Z_2$ in $\psi$; here $W_b$ is a projection.
\end{proof}

\begin{lemma}[Consistency and ordered-product estimates for the point sandwich]
	\label{lem:point-sandwich-estimates}
	...
	\leanok
	\uses{def:field-point-sandwich, def:symmetric-equivalents, def:povm-distance}
	For opposite placements $p_1,p_2$ and a fixed point pair, put
	$X_i(a)=(X_a)_{p_i}$, $Z_i(b)=(Z_b)_{p_i}$, and let $D_X,D_Z$
	be their summed squared differences on $\hat\psi$. Let
	$K_i=\sum_{a,b}\|[X_i(a),Z_i(b)]\hat\psi\|^2$ and
	$B=\frac12D_Z+\frac32(D_X+K_1+K_2)$.
	The off-diagonal consistency defect of $R_{p_1},R_{p_2}$ at this point
	pair is at most $B$, and $\E_{x,z}B\le C_0(\eps+\sqrt{\eps})$.
	Moreover,
	\[
		\begin{gathered}
		\E_{x,z}\sum_{a,b}\|(R_{a,b}-Z_bX_a)_p\hat\psi\|^2
		\le C_1\sqrt{\eps},\\
		\E_{x,z}\sum_{a,b}\|((Z_bX_a)_{p_1}-(X_aZ_b)_{p_2})\hat\psi\|^2
		\le C_2\eps,
		\end{gathered}
	\]
	for universal constants $C_0,C_1,C_2\ge1$, in every projective setting
	and for every indicated placement. These are formalization-only
	estimates for the direct field-valued construction.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:point-sandwich-overlap, lem:field-point-commutation,
		lem:qld-comm-cons, fact:add-a-proj, lem:sandwich-povm-square-sum}
	In the overlap identity, expand
	$(X_1-X_2)Z_1Z_2$ as
	$Z_1Z_2(X_1-X_2)+Z_2[X_1,Z_1]-Z_1[X_2,Z_2]$.
	The three-term squared-norm inequality and projector contractions give
	the bound $B$. Average using point self-consistency and field-valued
	commutation. The remaining bounds follow from
	$R-ZX=Z[X,Z]$ and by transporting the two factors across opposite
...
\begin{proof}
	\leanok
	\uses{lem:field-point-commutation, def:field-point-sandwich,
		lem:point-sandwich-estimates, lem:ortho-explicit-constant,
		lem:epr-cross-swap-invariance, fact:triangle,
		lem:sandwich-povm-square-sum}
	We use a direct construction with outcomes in $\F_q^2$ on the original
	expanded local spaces, as explained in
	\cite{gap:qpbt_linearity-theorem-quotation}.
	Form $R^{x,z}_{a,b}=Z_bX_aZ_b$ as in
	Definition~\ref{def:field-point-sandwich}. Its mean consistency defect
	between opposite placements is at most $C_0(\eps+\sqrt{\eps})$ by
	Lemma~\ref{lem:point-sandwich-estimates}.
	For each $(x,z)$, apply the orthonormalization theorem with explicit
	constant (Lemma~\ref{lem:ortho-explicit-constant}) to this POVM on
	$\mathsf A\mathsf A'$ and its partner on $\mathsf B\mathsf A''$,
	including the unused EPR registers in the second tensor factor.
	This produces a projective measurement on the same first local space.
	Apply the theorem again with the two parties reversed to obtain Bob's
	projective measurement. Concavity of $t^{1/4}$ and finite Jensen's
	inequality bound each mean squared distance to the sandwich by
	$220\bigl(C_0(\eps+\sqrt{\eps})\bigr)^{1/4}$.
	Lemma~\ref{lem:epr-cross-swap-invariance} transfers these estimates to
	the other two placements.

	The triangle inequality now follows the comparisons
	$Q_1\approx R_1\approx (ZX)_1\approx (XZ)_2$ from
	Lemma~\ref{lem:point-sandwich-estimates}. Commuting the last two factors
	using Lemma~\ref{lem:field-point-commutation} gives the second ordered
	product, and the comparison $(ZX)_2\approx R_2\approx Q_2$ gives
	self-consistency. This argument applies to every directed pair of
	opposite placements. For $0\le\eps\le1$, all errors are bounded by
	$K\eps^{1/8}$ for a universal $K$. For $\eps\ge1$, the squared distance
	between the relevant families is at most $4$: POVMs satisfy the
	square-summability bound of Lemma~\ref{lem:sandwich-povm-square-sum},
	and the ordered products of two projective measurements satisfy the
	corresponding equality. Enlarging $K$ covers this range as well.
	Thus $\delta_Q(\eps)=K\eps^{1/8}$ gives all three conclusions.
...
\begin{definition}[Paired-line sandwich POVM]
	\label{def:combined-line-sandwich}
	...
	\leanok
	\uses{def:expanded-line-measurement, lem:qld-comm-line-cons,
		lem:pasting-measurement}
	For either player and lines $\ell_X,\ell_Z$, write
	$M_X(f_X)=\hat M^{(\mathrm{Line},X),\ell_X}_{f_X}$ and
	$M_Z(f_Z)=\hat M^{(\mathrm{Line},Z),\ell_Z}_{f_Z}$, and define
	\[
		T^{\ell_X,\ell_Z}_{f_X,f_Z}=M_X(f_X)M_Z(f_Z)M_X(f_X).
	\]
	This is the POVM constructed in the proof of
	Lemma~\ref{lem:qld-xz-lines}; its outcomes are polynomial pairs of
	degree at most $md$. It is the one-sided pasted measurement with
	$G_1=M_Z$, $G_2=M_X$, and the two answers exchanged. Positivity follows
	by conjugating the positive effect $M_Z(f_Z)$ with $M_X(f_X)$;
	completeness follows by summing first over $f_Z$ and then $f_X$.
\end{definition}

\begin{lemma}[Axis-degree support of the paired-line POVM]
	\label{lem:combined-line-axis-support}
	...
	\leanok
	\uses{def:combined-line-sandwich, def:ideg-deg-polynomials}
	If $\ell_X$ is axis-parallel and $f_X$ has degree greater than $d$,
	then $T^{\ell_X,\ell_Z}_{f_X,f_Z}=0$ for every $f_Z$.
	The same conclusion holds if $\ell_Z$ is axis-parallel and $f_Z$
	has degree greater than $d$, for every $f_X$.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:qld-comm-line-cons}
	The corresponding expanded line effect vanishes by its axis-degree
	support, so the sandwich product vanishes. This is the support argument
	in the proof of Lemma~\ref{lem:qld-xz-lines}.
\end{proof}

\begin{lemma}[Exact X marginal of the paired-line POVM]
	\label{lem:combined-line-x-marginal}
	\lean{MIPStarRE.QPBT.ProjectiveSetting.combinedLineMeasurement_sum_Z}
	\leanok
	\uses{def:combined-line-sandwich}
	For either player, every pair of lines, and every $f_X$,
	\[
		\sum_{f_Z}T^{\ell_X,\ell_Z}_{f_X,f_Z}=M_X(f_X).
	\]
	This is a formalization-only identity for the X marginal of the
	measurement constructed in Lemma~\ref{lem:qld-xz-lines}.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:qld-comm-line-cons}
	Completeness gives $\sum_{f_Z}M_Z(f_Z)=\Id$, and projectivity gives
	$M_X(f_X)^2=M_X(f_X)$.
\end{proof}

The construction, support, and marginal identity above do not establish
the consistency with combined point measurements asserted in
...
\begin{lemma}[Formalization support for coefficient polynomial agreement]
	...
	\lean{MIPStarRE.QPBT.linePolynomialOfCoefficients_eval}
	\lean{MIPStarRE.QPBT.linePolynomialOfCoefficients_injective}
	\lean{MIPStarRE.QPBT.linePolynomialOfCoefficients_natDegree_le}
	\leanok
...
\end{lemma}
\begin{proof}
	\uses{def:combined-line-sandwich, lem:combined-line-axis-support,
		lem:qld-4-10, fact:add-a-proj,
