...
\begin{lemma}[Formalization support for affine evaluation on a nondegenerate line]
	\label{lem:nondegenerate-affine-evaluation}
	\lean{MIPStarRE.QPBT.evalOpt_affine_parameter_of_direction_ne_zero}
	\lean{MIPStarRE.QPBT.polynomial_ofFn_eval_eq_evalCoefficient}
	\leanok
	\uses{def:line, def:ideg-deg-polynomials}
	Let $\ell \subseteq \F_q^m$ be a line with base point $b$ and direction
	$v \ne 0$, and let $f$ be a polynomial of degree at most $c$ given by its
	coefficient vector. For every $t \in \F_q$ the evaluation of $f$ at the
	point $b + tv$ of $\ell$ is defined and equals
	$\sum_{i=0}^{c} f_i t^i$. The same coefficient convention agrees with
	evaluation of the univariate polynomial having those coefficients.
	This is a formalization-only auxiliary for the Schwartz--Zippel step in
	the proof of Lemma~\ref{lem:qld-xz-lines}
	(\texttt{14\_analysis\_of\_the\_pauli\_basis\_test.tex:950--955}).
\end{lemma}
\begin{proof}
	\leanok
	A nonzero direction determines the affine parameter of a point of $\ell$
	uniquely: if $b + tv = b + sv$ then $(t-s)v = 0$, hence $t = s$. Therefore
	the evaluation at $b + tv$ is the value of the coefficient polynomial at
	$t$. The second identity expands the polynomial as a sum of monomials.
\end{proof}

\begin{lemma}[Formalization support for affine parameter resampling]
	\label{lem:line-point-parameter-resampling}
	...
	Let $F$ be a real-valued function of a line and a point of $\F_q^m$, and
	write $b_\ell$ and $v_\ell$ for the base point and the direction of a line
	$\ell$. Then
	\[
		\E_{(\ell,x)} F(\ell,x)
		= \E_{(\ell,x)} \E_{t} F(\ell, b_\ell + t v_\ell),
	\]
	where $(\ell,x)$ is drawn from the line-point distribution
	(Definition~\ref{def:line-point-dist}) and $t$ is uniform on $\F_q$. The
	identity holds separately for the axis-parallel and the diagonal component
	of that mixture, with no restriction on the direction.
\end{lemma}
\begin{proof}
	\leanok
	For a uniformly distributed point $x$ and a fixed direction $v$, the
	canonical representative of the coset $x + \F_q v$ is unchanged by
	translation along $v$, and $x$ is uniform on that coset. Hence replacing
	$x$ by the representative shifted by an independent uniform $t$ leaves the
	joint law of the pair unchanged; this gives the axis-parallel and the
	diagonal case, and for a zero direction both sides coincide. The
	line-point identity is the corresponding convex combination of the two.
\end{proof}

\begin{lemma}[Formalization support for weighted collision mass on nondegenerate lines]
	\label{lem:nondegenerate-weighted-line-collision}
	\lean{MIPStarRE.QPBT.evalOpt_uniform_parameter_collision_le}
	\lean{MIPStarRE.QPBT.linePointDist_nondegenerate_weighted_collision_le}
	\leanok
	\uses{def:line-point-dist, def:ideg-deg-polynomials}
	Let $f \ne g$ be polynomials of degree at most $c$ on a line $\ell$ with
	direction $v \ne 0$. Under a uniform affine parameter $t$ on $\F_q$,
	\[
		\Pr_{t}\bigl[f(b_\ell + t v) = g(b_\ell + t v)\bigr] \le \frac{c}{q}.
	\]
	Consequently, for every weight $w$ on lines with $w(\ell) \ge 0$ for all
	$\ell$,
	\[
		\E_{(\ell,x)}\bigl[\bone{v_\ell \ne 0}\, w(\ell)\,
			\bone{f(x) = g(x)}\bigr]
		\le \frac{c}{q}\,
			\E_{(\ell,x)}\bigl[\bone{v_\ell \ne 0}\, w(\ell)\bigr],
	\]
	the expectation being over the line-point distribution
	(Definition~\ref{def:line-point-dist}). The restriction to nonzero
	directions is part of the assertion: the corresponding unrestricted
	inequality is false for the degenerate fibers of that mixture.
	This is a formalization-only auxiliary for the Schwartz--Zippel step in
	the proof of Lemma~\ref{lem:qld-xz-lines}
	(\texttt{14\_analysis\_of\_the\_pauli\_basis\_test.tex:950--955}).
\end{lemma}
\begin{proof}
	...
	\uses{lem:coefficient-polynomial-agreement,
		lem:nondegenerate-affine-evaluation,
		lem:line-point-parameter-resampling}
	By Lemma~\ref{lem:nondegenerate-affine-evaluation} the two evaluations
	along $\ell$ are the values of the coefficient polynomials of $f$ and $g$
	at $t$, and by Lemma~\ref{lem:coefficient-polynomial-agreement} these
	agree for at most $c$ of the $q$ values of $t$, which gives the first
	bound. For the second, resample the point by an independent uniform affine
	parameter using Lemma~\ref{lem:line-point-parameter-resampling}, and apply
	the first bound fiberwise, multiplied by the nonnegative weight; on
	fibers with zero direction both integrands vanish.
\end{proof}

\begin{lemma}[Formalization support for collision mass on a fixed nondegenerate line]
	...
	the conditional collision probability at most $c/q$ that
	Lemma~\ref{lem:pasting} takes as its hypothesis with $\eta = c/q$. The
	nonzero-direction hypothesis on $\ell_0$ is part of the assertion.
	This is a formalization-only auxiliary for the Schwartz--Zippel step in
	the proof of Lemma~\ref{lem:qld-xz-lines}
	(\texttt{14\_analysis\_of\_the\_pauli\_basis\_test.tex:950--955}); the
	transport to the product law and to the register presentation of the
	pasting lemma is separate.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:nondegenerate-weighted-line-collision}
	Apply Lemma~\ref{lem:nondegenerate-weighted-line-collision} with the
	nonnegative weight $w(\ell) = \bone{\ell = \ell_0}$. Since
	$v_{\ell_0} \ne 0$, the indicator $\bone{v_\ell \ne 0}$ is redundant
	against $\bone{\ell = \ell_0}$ on both sides, which turns the weighted
	inequality into the stated one.
\end{proof}

\begin{lemma}[Formalization support: axis lines have nonzero direction]
	\label{lem:axis-line-nonzero-direction}
	\lean{MIPStarRE.QPBT.aLinePointDist_zero_direction_mass}
	\leanok
	\uses{def:line-point-dist}
	The axis-parallel component of the line-point distribution
	(Definition~\ref{def:line-point-dist}) gives mass $0$ to the event
	$v_\ell = 0$. This is a formalization-only auxiliary; the paper uses the
	same fact implicitly when it applies Schwartz--Zippel along a sampled
	axis-parallel line.
\end{lemma}
\begin{proof}
	\leanok
	\uses{def:line-point-dist}
	The axis-parallel sampler returns the coordinate direction indexed by the
	sampled seed, whose entry at that coordinate is $1$, hence the direction
	is nonzero and the indicator of $v_\ell = 0$ vanishes pointwise.
\end{proof}

\begin{definition}[Formalization support: the nondegenerate line-pasting mass and law]
	\label{def:nondegenerate-line-pasting-law}
	\lean{MIPStarRE.QPBT.nondegenerateLinePastingMass}
	\lean{MIPStarRE.QPBT.nondegenerateLinePastingDist}
	\leanok
	\uses{def:line-point-dist, lem:pasting}
	Draw two independent pairs $(\ell_1,x_1)$ and $(\ell_2,x_2)$ from the
	line-point distribution (Definition~\ref{def:line-point-dist}). The
	\emph{nondegenerate pasting mass} is the mass of the event
	$v_{\ell_1}\ne0$ under that product law. The \emph{nondegenerate
	line-pasting law} is the product law conditioned on that event and
	relabelled as
	$\bigl((\ell_1,\ell_2),(\ell_2,x_2)\bigr),(\ell_1,x_1)$, which is the
	question--answer shape required by Lemma~\ref{lem:pasting}. Both objects
	are proof-only devices of the formalization; neither occurs in
	\cite{Ji2020MIPStar}.
\end{definition}

\begin{lemma}[Formalization support: the nondegenerate line-pasting law is normalized]
	\label{lem:nondegenerate-line-pasting-normalized}
	...
	\leanok
	\uses{def:nondegenerate-line-pasting-law,
		lem:axis-line-nonzero-direction, def:line-point-dist}
	The nondegenerate pasting mass of
	Definition~\ref{def:nondegenerate-line-pasting-law} lies between $1/2$
	and $1$. In particular it is positive, so the conditioning is defined and
	the nondegenerate line-pasting law is a probability distribution.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:axis-line-nonzero-direction, def:line-point-dist}
	The event constrains only the first sample, so its product-law mass is its
	mass under the line-point distribution. That distribution is the equal
	mixture of the axis-parallel and the diagonal component; the axis-parallel
	one has no zero-direction mass by
	Lemma~\ref{lem:axis-line-nonzero-direction} and so contributes $1/2$,
	the diagonal one contributes a nonnegative amount, and the indicator is
	bounded by $1$. Normalizing a positive-mass restriction and pushing it
	forward along a relabelling preserve normalization.
\end{proof}

\begin{lemma}[Formalization support: conditional collision from fiber averages]
	\label{lem:collision-bound-from-fiber-averages}
	\lean{MIPStarRE.QPBT.collision_bound_of_fiber_averages}
	\leanok
	\uses{lem:pasting}
	Let a distribution on pairs consisting of a first question and a second
	question be given, together with an evaluation of the second answer
	index by a second question. Suppose that for every value of the first
	coordinate and every two distinct second answer indices the unnormalized
	collision mass carried by that fiber is at most $\eta$ times the mass of
	the fiber itself. Then the conditional collision hypothesis that
	Lemma~\ref{lem:pasting} takes with parameter $\eta$ holds. This is a
	formalization-only normalization adapter; it converts unnormalized fiber
	estimates into the normalized predicate and asserts nothing new about the
	game.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:pasting}
	The mass of a fiber is the weight the first marginal gives to the
	corresponding first question, so dividing the assumed inequality by that
	weight, which the predicate assumes positive, gives the normalized bound.
\end{proof}

\begin{lemma}[Formalization support: weighted collision for two independent line-point samples]
	\label{lem:prod-nondegenerate-weighted-line-collision}
	\lean{MIPStarRE.QPBT.prod_linePointDist_nondegenerate_weighted_collision_le}
	\leanok
	\uses{lem:nondegenerate-weighted-line-collision, def:line-point-dist,
		def:ideg-deg-polynomials}
	Let $f\ne g$ be polynomials of degree at most $c$ and let $w\ge0$ be a
	weight depending on the first sampled line and on the whole second
	sample. For two independent draws $(\ell_1,x_1)$ and $(\ell_2,x_2)$ from
	the line-point distribution (Definition~\ref{def:line-point-dist}),
	\[
		\E\bigl[\bone{v_{\ell_1}\ne0}\,w(\ell_1,(\ell_2,x_2))\,
			\bone{f(x_1)=g(x_1)}\bigr]
		\le\frac{c}{q}\,
			\E\bigl[\bone{v_{\ell_1}\ne0}\,w(\ell_1,(\ell_2,x_2))\bigr].
	\]
	No condition is imposed on the second direction. The restriction to
	nonzero first directions is part of the assertion, exactly as in
	Lemma~\ref{lem:nondegenerate-weighted-line-collision}.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:nondegenerate-weighted-line-collision}
	Write the average against the product law as the iterated average with
	the second sample on the outside, and apply
	Lemma~\ref{lem:nondegenerate-weighted-line-collision} for each fixed
	second sample to the nonnegative weight
	$\ell\mapsto w(\ell,(\ell_2,x_2))$.
\end{proof}

\begin{lemma}[Formalization support: collision bound for the nondegenerate line-pasting law]
	\label{lem:nondegenerate-line-pasting-collision}
	\lean{MIPStarRE.QPBT.nondegenerateLinePastingDist_collision_bound}
	\leanok
	\uses{def:nondegenerate-line-pasting-law,
		lem:collision-bound-from-fiber-averages,
		lem:prod-nondegenerate-weighted-line-collision, lem:pasting,
		def:ideg-deg-polynomials}
	Fix an admissible tuple $(q,m,d)$ and a degree bound $c$. The
	nondegenerate line-pasting law of
	Definition~\ref{def:nondegenerate-line-pasting-law}, whose second answer
	is evaluated by sending a line-point pair to the optional evaluation at
	that point of a polynomial of degree at most $c$, satisfies the
	conditional collision hypothesis of Lemma~\ref{lem:pasting} with
	$\eta=c/q$. This is the formalization-only Schwartz--Zippel conditioning
	step for the pasting argument inside the proof of
	Lemma~\ref{lem:qld-xz-lines}
	(\texttt{14\_analysis\_of\_the\_pauli\_basis\_test.tex:950--963}); the
	measurement side of that argument is separate.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:collision-bound-from-fiber-averages,
		lem:prod-nondegenerate-weighted-line-collision,
		lem:nondegenerate-line-pasting-normalized}
	Reduce the normalized predicate to unnormalized fiber estimates with
	Lemma~\ref{lem:collision-bound-from-fiber-averages}. Unfolding the
	conditioning multiplies both sides by the retained mass, which is
	positive by Lemma~\ref{lem:nondegenerate-line-pasting-normalized}, and
	turns the claim into the estimate of
	Lemma~\ref{lem:prod-nondegenerate-weighted-line-collision} for the
	indicator weight of the fixed first question.
\end{proof}
...
\begin{lemma}[Restricted line distributions and error inflation]
	...
	\lean{MIPStarRE.QPBT.avg_restricted_le}
	\lean{MIPStarRE.QPBT.avg_restricted_prod_le}
	\lean{MIPStarRE.QPBT.avgOver_prod_restrictedLinePointDist_le}
...
\begin{lemma}[Formalization support: a uniform point under the directly indexed law]
	...
	\lean{MIPStarRE.QPBT.uniformDistribution_map_lineRepMap_add_smul}
