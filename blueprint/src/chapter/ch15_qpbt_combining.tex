...
\begin{proof}
	\leanok
	\uses{lem:qld-comm-cons, lem:ortho, fact:triangle,
...
\begin{lemma}[Formalization support for coefficient polynomial agreement]
	...
	agree at at most $c$ elements of $K$.
...
\begin{definition}[The concrete paired-line POVM]
	...
	define the POVM on the expanded local space by
	\[
		T^{\ell_X,\ell_Z}_{f_X,f_Z}
		=\hat M^{(\mathrm{Line},X),\ell_X}_{f_X}
		 \hat M^{(\mathrm{Line},Z),\ell_Z}_{f_Z}
...
\begin{lemma}[Axis-degree support of the concrete paired-line POVM]
	...
	For either player and $W\in\{X,Z\}$, if $\ell_W$ is axis-parallel
...
\begin{lemma}[Consistency of the constructed paired-line measurement]
	...
	This is a Lean-only construction obligation. Fix a polynomially small
	...
	for every directed opposite placement. The line-error function is chosen
	after $\delta_Q$ and its polynomial bound. This isolates the consistency
	part of the preceding construction while retaining its defining
	measurement; see \cite{gap:qpbt_subline-claims-line-marginal}.
\end{lemma}
...
\begin{definition}[Restricted line distributions]
	...
	For any such $i$, the \emph{$i$-th restricted
	...
	$\ell = (u_0, s, v)$ satisfies $\bar\chi(s) = i$ and coordinates
...
\begin{lemma}[Restricted line distributions and error inflation]
	...
	distributions $D_{\mathrm{ALine},i}$ for $i \in \{0, \dots, m-1\}$, and
	...
	\begin{enumerate}
		...
		\mathrm{DLine}\}$ and any $i, j \in \{0, \dots, m-1\}$,
		\begin{equation}
...
\begin{lemma}[Formalization support: the ordered product on one placement]
	...
	On one placement, the joint point measurement and the ordered
	$Z$-then-$X$ product have squared distance at most $4\delta_Q$.
\end{lemma}
\begin{proof}
	\leanok
	...
	squared-distance triangle inequality to the two $\delta_Q$ bounds.
\end{proof}
...
\begin{lemma}[Formalization support: replacing one factor of a real overlap]
	...
	A complete measurement weights the real part of the overlap gap between operator families by
	at most the square root of their state-dependent squared distance.
\end{lemma}
\begin{proof}
	\leanok
...
\begin{definition}[Directly indexed sub-line laws]
	...
	$D_{\mathrm v,i}$ and independently $\ell_Z$ from the line marginal of
	$D_{\mathrm v,j}$, and retaining $(\ell_X,\ell_Z,x)$.
	The law of $(\ell_X,\ell_Z,\pi_Z(\ell(t)))$ is the mixture obtained
	by retaining $(\ell_X,\ell_Z,z)$ from the corresponding independent
	samples of $\ell_X$ and $(\ell_Z,z)$. These are separate one-point
	marginal identities, with the same mixture weights; they impose no
	joint conditional independence of the two projected points.
...
\begin{proof}
	...
	The representative--parameter map has constant fibers of size $q$; the
	coordinate split and scalar identification are bijections.
\end{proof}
...
\begin{lemma}[Formalization support: real-part replacement under the directly indexed law]
	...
	\uses{lem:overlap-gap-distance, lem:qld-4-10-same-placement,
		lem:qld-sublines-uniform-point, lem:qld-xz-lines}
	...
	Lemma~\ref{lem:claim-17-1}, using $D_{\mathrm{dir}}$ and zero-completed
	optional evaluations. Then
...
\begin{lemma}[Formalization support: the concrete X deficit under the directly indexed law]
	...
	\hat M^{(\mathrm{Point},X),x}_{f_X(x)}$ over both outcomes, with
	zero-completed optional evaluation. There is a universal $C>0$ such that
...
\begin{lemma}[Formalization obligation: complex X-factor removal under the directly indexed law]
	...
	$D_{\mathrm{dir}}$ with zero-completed optional evaluations, the complex
	modulus of the difference in Lemma~\ref{lem:claim-17-2} is at most
	$C m\sqrt{\delta_{\mathrm{Line}}(\eps)}$ for a universal $C>0$.
	The remaining obligation is the complex weighted Cauchy--Schwarz step.
...
\begin{lemma}[Formalization support: the real Z overlap under the directly indexed law]
	...
	\leanok
	\uses{lem:restricted-line-mixture-bounds, lem:qld-4-10-same-placement,
	...
	expectation in Lemma~\ref{lem:claim-17-3} under that law with
	zero-completed optional evaluations, then
	$|\Re B_Z-1|\le
...
\begin{proof}
	...
	in $\F_q^m$; hence
	$\E_{x,z} \sum_{a,b}
	\bra{\hat\psi} \Id \ot (W^{x,z}_{a,b})^\dagger W^{x,z}_{a,b} \ket{\hat\psi}
	\le 4\delta_Q$
	using Lemma~\ref{lem:qld-4-10-same-placement}. The resulting error is
	$2\sqrt{\delta_Q}$, which has the stated asymptotic scale. The adjoint
	is necessary since $W$ need not be Hermitian; the source writes $W^2$.
\end{proof}
...
The following auxiliary estimates are independent of the standing strategy
assumptions of this chapter. They isolate the coefficient-polynomial argument
in the proof of Lemma~\ref{lem:qld-4-7}, with the ordered-correlation error
retained explicitly.

Throughout the section, $F$ is a finite field of cardinality $q$ and $\hilb$
is a finite-dimensional complex Hilbert space. Base points are written
$\mathbf x=(x_0,\ldots,x_{n-1})$, the two scalar variables of the outer
polynomial ring are $\alpha$ and $\beta$, and an exponent pair $e=(e_0,e_1)$
records the monomial $\alpha^{e_0}\beta^{e_1}$. The article
\cite{Ji2020MIPStar} numbers base coordinates and exponents from $1$; both are
numbered from $0$ here, so that the coordinates of a base point and the two
scalar exponents are identified with the index sets used in the formalization.
The two conventions differ by this shift and by nothing else.

\paragraph{Formalization support for the ordered polynomial-image estimates.}
The subordinate results below isolate the algebra of the ordered product, the
contribution of the exceptional scalar coefficient, the choice of a nonlinear
coefficient polynomial, and the resulting collision estimate. None of them is
a named statement of \cite{Ji2020MIPStar}; each records a step of the proof of
Lemma~\ref{lem:aux-ordered-nonlinear-mass} below, which in turn isolates one
part of the proof of Lemma~\ref{lem:qld-4-7}. In all of them
$\{X_b\}_{b\in F}$ and $\{Z_c\}_{c\in F}$ are measurements on $\hilb$, and no
vector is assumed normalized.

\begin{definition}[Formalization support for the ordered linear-answer product]
	\label{def:aux-ordered-indicator}
	\lean{MIPStarRE.QPBT.PolynomialImageBounds.orderedIndicator}
	\leanok
	This formalization-only definition names the ordered product selected by
	the linear field-answer check; it is an operator, and is not asserted to be
	a measurement effect. For $\alpha,\beta,a\in F$, set
	\[
		A(X,Z;\alpha,\beta,a)=\sum_{b,c\in F}
			\mathbf 1_{\alpha b+\beta c=a}\,X_bZ_c.
	\]
\end{definition}

\begin{definition}[Formalization support for the diagonal sandwich sum]
	\label{def:aux-diagonal-indicator}
	\lean{MIPStarRE.QPBT.PolynomialImageBounds.diagonalIndicator}
	\leanok
	This formalization-only definition names the diagonal sandwich sum
	associated with the same check. For $\alpha,\beta,a\in F$, set
	\[
		B(X,Z;\alpha,\beta,a)=\sum_{b,c\in F}
			\mathbf 1_{\alpha b+\beta c=a}\,Z_cX_bZ_c.
	\]
\end{definition}

\begin{lemma}[Formalization support for the ordered Gram identity]
	\label{lem:aux-ordered-gram}
	\lean{MIPStarRE.QPBT.PolynomialImageBounds.orderedIndicator_gram_of_ne_zero, MIPStarRE.QPBT.PolynomialImageBounds.orderedIndicator_norm_sq_of_ne_zero}
	\leanok
	\uses{def:aux-ordered-indicator, def:aux-diagonal-indicator}
	These formalization-only auxiliaries record the local algebra of the
	ordered product. Let $X$ be projective and let $\beta\ne0$. Then
	\[
		A(X,Z;\alpha,\beta,a)^{*}A(X,Z;\alpha,\beta,a)=B(X,Z;\alpha,\beta,a),
	\]
	and consequently
	$\|A(X,Z;\alpha,\beta,a)\psi\|^{2}
	=\langle\psi,B(X,Z;\alpha,\beta,a)\psi\rangle$ for every $\psi\in\hilb$.
	No commutation between the two measurements is assumed.
\end{lemma}
\begin{proof}
	\leanok
	\uses{def:aux-ordered-indicator, def:aux-diagonal-indicator}
	Expand the Gram operator as a double sum over pairs of admissible answers.
	If two pairs satisfy the same linear equation and share their first
	coordinate, then $\beta\ne0$ forces their second coordinates to agree, so
	distinct admissible pairs carry distinct $X$ labels and orthogonality of the
	$X_b$ annihilates every off-diagonal term. On the diagonal, idempotence of
	$X_b$ leaves the sandwich term $Z_cX_bZ_c$. The second identity is the
	quadratic form of the Gram operator.
\end{proof}

\begin{lemma}[Formalization support for the diagonal sandwich bounds]
	\label{lem:aux-diagonal-contraction}
	\lean{MIPStarRE.QPBT.PolynomialImageBounds.diagonalIndicator_bounds}
	\leanok
	\uses{def:aux-diagonal-indicator}
	This formalization-only auxiliary states that the diagonal sandwich sum is
	a positive contraction. If $Z$ is projective, then
	$0\le B(X,Z;\alpha,\beta,a)\le\Id$.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:pasting-measurement, def:aux-diagonal-indicator}
	The operators $Z_cX_bZ_c$ form a measurement indexed by pairs. A subsum of
	the effects of a measurement is nonnegative, and is bounded above by the
	total sum, which is the identity.
\end{proof}

\begin{lemma}[Formalization support for the exceptional coefficient]
	\label{lem:aux-ordered-zero}
	\lean{MIPStarRE.QPBT.PolynomialImageBounds.orderedIndicator_zero, MIPStarRE.QPBT.PolynomialImageBounds.orderedIndicator_gram_le_one}
	\leanok
	\uses{def:aux-ordered-indicator}
	These formalization-only auxiliaries treat the coefficient value excluded by
	the Gram identity. Completeness of $Z$ gives
	\[
		A(X,Z;\alpha,0,a)=\sum_{b:\,\alpha b=a}X_b,
	\]
	the effect at $a$ of the measurement obtained from $X$ through
	$b\mapsto\alpha b$; the value $\alpha=0$ is included. Consequently, if $X$
	and $Z$ are projective, then
	$A(X,Z;\alpha,\beta,a)^{*}A(X,Z;\alpha,\beta,a)\le\Id$ for all
	$\alpha,\beta,a\in F$.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:aux-ordered-gram, lem:aux-diagonal-contraction,
		lem:sandwich-postprocess-projective, def:aux-ordered-indicator}
	Summing over $c$ uses completeness of $Z$. The measurement obtained by
	postprocessing a projective measurement is again projective, so the
	resulting effect is a projection and is bounded by the identity. For
	$\beta\ne0$ the Gram identity turns the same bound into the bound on the
	diagonal sandwich sum.
\end{proof}

\begin{lemma}[Formalization support for the pointwise exceptional error]
	\label{lem:aux-ordered-exceptional-pointwise}
	\lean{MIPStarRE.QPBT.PolynomialImageBounds.abs_orderedIndicator_norm_sq_sub_diagonal_le}
	\leanok
	\uses{def:aux-ordered-indicator, def:aux-diagonal-indicator}
	This formalization-only auxiliary localizes the error of the Gram identity.
	Let $X$ and $Z$ be projective. For all $\alpha,\beta,a\in F$ and every
	$\psi\in\hilb$,
	\[
		\bigl|\,\|A(X,Z;\alpha,\beta,a)\psi\|^{2}
			-\langle\psi,B(X,Z;\alpha,\beta,a)\psi\rangle\,\bigr|
		\le\mathbf 1_{\beta=0}\|\psi\|^{2}.
	\]
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:aux-ordered-gram, lem:aux-ordered-zero,
		lem:aux-diagonal-contraction}
	For $\beta\ne0$ the two quantities agree. At $\beta=0$ both the Gram
	operator and the diagonal sandwich sum are positive contractions, so both
	quadratic forms lie between $0$ and $\|\psi\|^{2}$.
\end{proof}

\begin{lemma}[Formalization support for the averaged exceptional error]
	\label{lem:aux-ordered-exceptional-average}
	\lean{MIPStarRE.QPBT.PolynomialImageBounds.avg_abs_orderedIndicator_norm_sq_sub_diagonal_le, MIPStarRE.QPBT.PolynomialImageBounds.avg_sum_abs_orderedIndicator_norm_sq_sub_diagonal_le}
	\leanok
	\uses{lem:aux-ordered-exceptional-pointwise}
	These formalization-only auxiliaries average the pointwise error over a
	uniform coefficient. Let $X$ and $Z$ be projective, let $\alpha\in F$, and
	let $a\colon F\to F$ be arbitrary. Then, for $\beta$ uniform on $F$,
	\[
		\E_{\beta}\bigl|\,\|A(X,Z;\alpha,\beta,a(\beta))\psi\|^{2}
			-\langle\psi,B(X,Z;\alpha,\beta,a(\beta))\psi\rangle\,\bigr|
		\le\frac{\|\psi\|^{2}}{q}.
	\]
	For a finite index set $\Gamma$, vectors $\psi_g\in\hilb$ and answer
	functions $a_g\colon F\to F$, the same average of the sum over $g\in\Gamma$
	of these quantities is at most $q^{-1}\sum_{g\in\Gamma}\|\psi_g\|^{2}$. The
	tested answer may depend on the coefficient and on the label arbitrarily,
	and no factor counting outcomes occurs.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:aux-ordered-exceptional-pointwise}
	The pointwise bound is supported on the single coefficient value $\beta=0$,
	which has uniform probability $q^{-1}$. Summing the resulting estimate over
	$\Gamma$ commutes with the average.
\end{proof}

\begin{lemma}[Formalization support for projective mass preservation]
	\label{lem:aux-projective-mass}
	\lean{MIPStarRE.QPBT.PolynomialImageBounds.sum_projective_state_norm_sq}
	\leanok
	This formalization-only auxiliary records the normalization used after
	summing over outcomes. For a projective measurement $\{S_g\}_{g\in\Gamma}$
	on $\hilb$ with finite outcome set and every $\psi\in\hilb$,
	\[
		\sum_{g\in\Gamma}\|S_g\psi\|^{2}=\|\psi\|^{2}.
	\]
\end{lemma}
\begin{proof}
	\leanok
	Each summand is the quadratic form of $S_g^{*}S_g=S_g$, and completeness
	sums these effects to the identity.
\end{proof}

\begin{lemma}[Formalization support for the exceptional error after a projective refinement]
	\label{lem:aux-projective-exceptional}
	\lean{MIPStarRE.QPBT.PolynomialImageBounds.avg_sum_abs_projective_orderedIndicator_le, MIPStarRE.QPBT.PolynomialImageBounds.avg_uniform_sum_abs_projective_orderedIndicator_le}
	\leanok
	\uses{lem:aux-ordered-exceptional-average, lem:aux-projective-mass}
	These formalization-only auxiliaries apply the averaged error to the
	outcome vectors of a projective measurement. Let $\{S_g\}_{g\in\Gamma}$ be
	projective with finite outcome set, let $X$ and $Z$ be projective, let
	$\alpha\in F$, and let $a_g\colon F\to F$. Then, for $\beta$ uniform,
	\[
		\E_{\beta}\sum_{g\in\Gamma}
			\bigl|\,\|A(X,Z;\alpha,\beta,a_g(\beta))S_g\psi\|^{2}
			-\langle S_g\psi,B(X,Z;\alpha,\beta,a_g(\beta))S_g\psi\rangle\,\bigr|
		\le\frac{\|\psi\|^{2}}{q}.
	\]
	If the two measurements are replaced by families $X^{t},Z^{t}$ of
	projective measurements indexed by a nonempty finite set $T$ of base
	points, and the answers are allowed to depend on the label, the base point
	and both coefficients, the same bound holds for the uniform average over
	$t\in T$ and $\alpha,\beta\in F$. No commutation among the three
	measurements is assumed.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:aux-ordered-exceptional-average, lem:aux-projective-mass}
	Apply the averaged error to the vectors $S_g\psi$ and replace
	$\sum_g\|S_g\psi\|^{2}$ by $\|\psi\|^{2}$. A uniform law on a product
	factors into iterated uniform averages, and each inner average is bounded
	by the same constant.
\end{proof}

\begin{lemma}[Formalization support for the reversed ordered product]
	\label{lem:aux-ordered-reverse}
	\lean{MIPStarRE.QPBT.PolynomialImageBounds.orderedIndicator_reverse}
	\leanok
	\uses{def:aux-ordered-indicator}
	This formalization-only auxiliary identifies the product taken in the
	opposite order. For all $\alpha,\beta,a\in F$,
	\[
		\sum_{b,c\in F}\mathbf 1_{\alpha b+\beta c=a}\,Z_cX_b
		=A(Z,X;\beta,\alpha,a).
	\]
	The estimates above therefore apply to the reversed product as well, with
	$\alpha$ in the exceptional role.
\end{lemma}
\begin{proof}
	\leanok
	\uses{def:aux-ordered-indicator}
	Exchange the two summation indices and use commutativity of addition in
	$F$.
\end{proof}

\begin{lemma}[Formalization support for the choice of a nonlinear coefficient]
	\label{lem:aux-nonlinear-coeff}
	\lean{MIPStarRE.QPBT.PolynomialImageBounds.exists_nonzero_nonlinear_coeff, MIPStarRE.QPBT.PolynomialImageBounds.specialization_ne_linear_of_coeff}
	\leanok
	These formalization-only auxiliaries select the coefficient that drives the
	collision estimate. Let $R$ be a commutative ring and let $p\in
	R[\alpha,\beta]$ be of no form $r\alpha+s\beta$ with $r,s\in R$. Then $p$
	has a nonzero coefficient $c_e$ at some exponent pair $e$ with $e\ne(1,0)$
	and $e\ne(0,1)$. If moreover $R=F[x_0,\ldots,x_{n-1}]$ and $u\in F^{n}$
	satisfies $c_e(u)\ne0$, then the specialized polynomial
	$p(u;\alpha,\beta)\in F[\alpha,\beta]$ differs from $b\alpha+c\beta$
	simultaneously for all $b,c\in F$.
\end{lemma}
\begin{proof}
	\leanok
	If every coefficient outside the two scalar-linear monomials vanished, then
	comparing coefficients would exhibit $p$ in the excluded form with $r$ and
	$s$ its coefficients at $(1,0)$ and $(0,1)$. Specialization of the
	coefficients commutes with extracting the coefficient at $e$, so the
	specialized polynomial has coefficient $c_e(u)\ne0$ at $e$, whereas
	$b\alpha+c\beta$ has coefficient $0$ there.
\end{proof}

\begin{lemma}[Formalization support for the specialized collision estimate]
	\label{lem:aux-specialization-collision}
	\lean{MIPStarRE.QPBT.PolynomialImageBounds.avg_specialization_eq_linear_le_of_coeff}
	\leanok
	\uses{lem:aux-nonlinear-coeff}
	This formalization-only auxiliary is the collision estimate at a fixed good
	base point. In the situation of Lemma~\ref{lem:aux-nonlinear-coeff} with
	$R=F[x_0,\ldots,x_{n-1}]$ and $c_e(u)\ne0$, for all $b,c\in F$,
	\[
		\Prb_{v\in F^{2}}\bigl[p(u;v_0,v_1)=v_0b+v_1c\bigr]
		\le\frac{\max\{1,\deg_{\alpha,\beta}p\}}{q},
	\]
	the point $v$ being uniform. The bound is derived from the coefficient
	polynomials; no collision law is assumed.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:schwartz-zippel-total-degree, lem:aux-nonlinear-coeff}
	The two polynomials are distinct by Lemma~\ref{lem:aux-nonlinear-coeff}.
	Specialization does not raise the total degree, so both have total degree
	at most $\max\{1,\deg_{\alpha,\beta}p\}$, and Schwartz--Zippel applies.
\end{proof}

\begin{lemma}[Formalization support for the diagonal polynomial-image estimate]
	\label{lem:aux-diagonal-image-bound}
	\lean{MIPStarRE.QPBT.PolynomialImageBounds.avg_diagonalIndicator_le_of_not_linear}
	\leanok
	\uses{def:aux-diagonal-indicator, lem:aux-nonlinear-coeff}
	This formalization-only auxiliary is the polynomial-image estimate for the
	diagonal sandwich sum. Let $p\in F[\mathbf x][\alpha,\beta]$ be of no form
	$r(\mathbf x)\alpha+s(\mathbf x)\beta$, let $X^{u}$ and $Z^{u}$ be
	measurements for $u\in F^{n}$ with every $Z^{u}$ projective, and let
	$\psi\in\hilb$. Write $p=\sum_ec_e(\mathbf x)\alpha^{e_0}\beta^{e_1}$ and
	set
	\[
		D=\max\bigl(\{0\}\cup\{\deg c_e:c_e\ne0\}\bigr),
		\qquad E=\max\{1,\deg_{\alpha,\beta}p\}.
	\]
	Then, for $u\in F^{n}$ and $v\in F^{2}$ uniform,
	\[
		\E_{u}\E_{v}\bigl\langle\psi,
			B(X^{u},Z^{u};v_0,v_1,p(u;v_0,v_1))\psi\bigr\rangle
		\le\frac{D+E}{q}\|\psi\|^{2}.
	\]
	A single nonzero nonlinear coefficient supplies a common exceptional set
	for all answers, so no factor counting outcomes occurs.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:aux-nonlinear-coeff, lem:aux-specialization-collision,
		lem:aux-diagonal-contraction, lem:pasting-measurement,
		lem:schwartz-zippel-total-degree}
	Fix a nonzero coefficient $c_e$ outside the two scalar-linear monomials.
	The sandwich weights $\langle\psi,Z^{u}_cX^{u}_bZ^{u}_c\psi\rangle$ are
	nonnegative and sum to $\|\psi\|^{2}$, so the inner average is a weighted
	average of agreement indicators with these weights. At a base point with
	$c_e(u)\ne0$, each indicator averages to at most $E/q$. The remaining base
	points form the zero set of $c_e$, of probability at most $D/q$ by
	Schwartz--Zippel, and there the quadratic form is at most $\|\psi\|^{2}$
	because the diagonal sandwich sum is a positive contraction.
\end{proof}

\begin{lemma}[Formalization support for the ordered polynomial-image estimate]
	\label{lem:aux-ordered-image-bound}
	\lean{MIPStarRE.QPBT.PolynomialImageBounds.avg_orderedIndicator_norm_sq_le_of_not_linear, MIPStarRE.QPBT.PolynomialImageBounds.avg_sum_orderedIndicator_norm_sq_le_of_not_linear}
	\leanok
	\uses{def:aux-ordered-indicator, lem:aux-diagonal-image-bound}
	These formalization-only auxiliaries transfer the diagonal estimate to the
	ordered product and sum it over labels. In the situation of
	Lemma~\ref{lem:aux-diagonal-image-bound}, with every $X^{u}$ projective as
	well,
	\[
		\E_{u}\E_{v}\|A(X^{u},Z^{u};v_0,v_1,p(u;v_0,v_1))\psi\|^{2}
		\le\frac{D+E+1}{q}\|\psi\|^{2}.
	\]
	For a finite label set $J$, polynomials $p_g$ none of which has the
	excluded form, and vectors $\psi_g\in\hilb$,
	\[
		\E_{u}\E_{v}\sum_{g\in J}
			\|A(X^{u},Z^{u};v_0,v_1,p_g(u;v_0,v_1))\psi_g\|^{2}
		\le\sum_{g\in J}\frac{D_g+E_g+1}{q}\|\psi_g\|^{2},
	\]
	each label weighted by its own squared mass.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:aux-diagonal-image-bound,
		lem:aux-ordered-exceptional-pointwise}
	The squared norm of the ordered product differs from the quadratic form of
	the diagonal sandwich sum only on the exceptional coefficient $v_1=0$, of
	uniform probability $q^{-1}$; adding this contribution to the diagonal
	estimate gives the first inequality. The second follows by summing the
	first over $J$, since the average commutes with the finite sum.
\end{proof}
...
\begin{lemma}[Formalization support for scalar-linearity concentration]
	...
	$\mathbf x=(x_0,\ldots,x_{n-1})$. Assume that no $p_g$ has the form
	$\alpha r(\mathbf x)+\beta s(\mathbf x)$.
	Write $p_g=\sum_e c_{g,e}(\mathbf x)\alpha^{e_0}\beta^{e_1}$ and set
	\[
		D_g=\max\bigl(\{0\}\cup
			\{\deg c_{g,e}:c_{g,e}\ne0\}\bigr),
...
\begin{proof}
	\leanok
	\uses{lem:aux-ordered-image-bound}
	Apply $\|v\|^2\le2\|v-w\|^2+2\|w\|^2$ with $v=\psi_g$ and
	$w=A_g(u,\alpha,\beta)\psi_g$, then average over the base point and both
	coefficients and sum over $g\in J$. The averaged mass of the vectors
	$A_g(u,\alpha,\beta)\psi_g$ is bounded by
	Lemma~\ref{lem:aux-ordered-image-bound}.
\end{proof}
