...
assembled by Fourier averaging. The last two steps require the strategy
state to carry zero-state padding, which the setting of
Lemma~\ref{lem:qld-4-10} does not provide (Remark~\ref{rem:linearity-import});
the proof given for Lemma~\ref{lem:qld-4-10} below therefore forms the
sandwich POVM directly with outcomes in $\F_q^2$, transfers the approximate
commutation to the field-valued point measurements by Parseval's identity,
and makes the sandwich projective by the orthonormalization lemma.
...
\begin{remark}[Obligations of the linearity-theorem import]
	...
	definition.
	The last obligation of the import, the absorption of the ancilla
	register $\hilb'$ and its state $\ket{\mathrm{anc}}$ into the strategy
	state uniformly in $(x,z)$, has two parts. The uniformity holds: the
	ancilla produced by the Fourier--Naimark argument depends only on $t$
	(Lemma~\ref{lem:linearity-common-ancilla}). The absorption does not: it
	is the source's assumption that $\ket{\hat\psi}$ ``has sufficiently
	many ancilla zero qubits'', an assumption on the strategy that is not
	part of the setting of Lemma~\ref{lem:qld-4-10}, and it cannot be
	discharged in general by compression. The Fourier-derived projective
	measurement of the exactly linear family compresses along
	$\ket{\mathrm{anc}}$ to the Fourier-square POVM
	$B^v = (\widehat{\mathcal{O}}^v)^2$, which need not be projective.
	The compressed observables themselves are
	$\sum_v (-1)^{v\cdot u} B^v$. Projectivity of $\{B^v\}$ does not imply
	exact linearity of the original family: a constant observable
	$\mathcal{O}^u=\operatorname{diag}(1,-1)$ has Fourier squares $I$ at zero
	and zero elsewhere, but is not exactly linear. Lemma~\ref{lem:qld-4-10} is
	therefore proved on the original space by a route that does not pass
	through Theorem~\ref{thm:linearity}; see its proof and
...
\begin{lemma}[Formalization support for Theorem~\ref{thm:linearity}: common ancilla]
	...
	\cite{Natarajan2017Linearity}; it records that the ancilla of
	...
	$\hilb'_t$ and a pure state $\ket{\mathrm{anc}_t} \in \hilb'_t$ such that,
	for every finite-dimensional complex Hilbert space $\hilb$, every density
	operator $\rho$ on $\hilb$, every $\delta \ge 0$,
	and every family of binary observables $\{\mathcal{O}^u\}_{u \in \F_2^t}$
	on $\hilb$ satisfying the correlation hypothesis of
	Theorem~\ref{thm:linearity}, there are binary observables
	$\{\mathcal{L}^u\}$ on $\hilb \ot \hilb'_t$ with
	$\mathcal{L}^u \mathcal{L}^{u'} = \mathcal{L}^{u+u'}$ for all $u, u'$ and
	$\E_u \, d_{\rho'}(\mathcal{L}^u, \mathcal{O}^u \ot \Id)^2 \le 2\delta$ for
	$\rho' = \rho \ot \ket{\mathrm{anc}_t}\bra{\mathrm{anc}_t}$.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:naimark-helper, lem:boolean-representation-stability,
		lem:blr-defect-identity}
	In the proof of Theorem~\ref{thm:linearity} the space $\hilb'$ has an
	orthonormal basis indexed by $\F_2^t \sqcup \{\bot\}$ and
	$\ket{\mathrm{anc}}$ is the basis vector of $\bot$, whatever $\hilb$,
	$\rho$, and the family are; the rounded observables of that proof for the
	given family have the two properties.
\end{proof}
...
\begin{lemma}[Formalization support for Definition~\ref{def:symmetric-equivalents}: EPR exchange]
	\label{lem:qld-placement-transfer}
	...
	it proves the crosswise EPR exchange independently, without assuming
	symmetry of the strategy. The permutation of the six registers
	exchanging $\mathsf{A}' \leftrightarrow \mathsf{B}''$ and
	$\mathsf{A}'' \leftrightarrow \mathsf{B}'$ fixes $\ket{\hat\psi}$, since
	each EPR vector is symmetric, and carries the placement
	$\mathsf{A}\mathsf{A}'$ to $\mathsf{A}\mathsf{B}''$ and
	$\mathsf{B}\mathsf{A}''$ to $\mathsf{B}\mathsf{B}'$. Consequently, for
	every operator $P$ on $\hilb \ot (\C^q)^{\ot M}$,
	$\lVert (P)_{\mathsf{A}\mathsf{B}''} \ket{\hat\psi} \rVert
	= \lVert (P)_{\mathsf{A}\mathsf{A}'} \ket{\hat\psi} \rVert$ and
	$\lVert (P)_{\mathsf{B}\mathsf{B}'} \ket{\hat\psi} \rVert
	= \lVert (P)_{\mathsf{B}\mathsf{A}''} \ket{\hat\psi} \rVert$. Each
	placement is a unital $*$-homomorphism, and operators placed on opposite
	register pairs commute.
\end{lemma}
\begin{proof}
	\leanok
	\uses{def:expanded-state, def:EPR}
	The permutation unitary $U$ satisfies $U \ket{\hat\psi} = \ket{\hat\psi}$
	coordinatewise, using $\ket{\mathrm{EPR}_q}$ is symmetric, and
	$U (P)_{\mathsf{A}\mathsf{B}''} U^\dagger = (P)_{\mathsf{A}\mathsf{A}'}$
	entrywise; the norm identities follow by unitary invariance. The
	homomorphism and commutation properties are read off from the expression
	of each placement as a reindexed tensor factor of the bipartitions of
	Definition~\ref{def:symmetric-equivalents}.
\end{proof}

\begin{lemma}[Formalization support for Lemma~\ref{lem:qld-comm-cons}: field-valued commutation]
	\label{lem:qld-point-commutation-field}
	\lean{MIPStarRE.QPBT.sum_norm_sum_smul_sq_of_orthogonal}
	\lean{MIPStarRE.QPBT.sum_fixedCharacter_mul}
	\lean{MIPStarRE.QPBT.ProjectiveSetting.expPointOp_commutator_eq}
	\lean{MIPStarRE.QPBT.ProjectiveSetting.expPoint_comm}
	\leanok
	\uses{lem:qld-comm-cons, def:expanded-point-measurement,
		def:expanded-observables, lem:cancellation, def:povm-distance}
	This formalization-only auxiliary is not a named statement of the paper;
	it transfers item~2 of Lemma~\ref{lem:qld-comm-cons} from the binary
	refinements to the field-valued point measurements. Write
	$\chi(a) = (-1)^{\tr(a)}$. Since
	$\hat{M}^{(\mathrm{Point},X),x}_a = \E_r \chi(ar) \hat{X}^r(x)$ and
	$\hat{M}^{(\mathrm{Point},Z),z}_b = \E_s \chi(bs) \hat{Z}^s(z)$,
	\[
		\big[\hat{M}^{(\mathrm{Point},X),x}_a, \hat{M}^{(\mathrm{Point},Z),z}_b\big]
		= \E_{r,s} \chi(ar) \chi(bs) \big[\hat{X}^r(x), \hat{Z}^s(z)\big] ,
	\]
	and Parseval's identity for the orthogonal characters $a \mapsto \chi(ar)$
	of $\F_q$ gives, for every placement and on average over
	$x, z \in \F_q^m$,
	\[
		\sum_{a,b \in \F_q} \big\lVert \big[\hat{M}^{(\mathrm{Point},X),x}_a,
		\hat{M}^{(\mathrm{Point},Z),z}_b\big] \ket{\hat\psi} \big\rVert^2
		= \E_{r,s} \big\lVert \big[\hat{X}^r(x), \hat{Z}^s(z)\big]
		\ket{\hat\psi} \big\rVert^2 \le C\, \delta_{\mathrm{Anticom}}(\eps) ,
	\]
	with a universal constant $C$ and no loss in $q$.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:qld-comm-cons, lem:cancellation, def:expanded-point-measurement}
	Expanding a squared norm of a weighted sum in inner products and using
	$\sum_a \chi(ar)\chi(ar') = q\,[r = r']$ (Lemma~\ref{lem:cancellation})
	gives Parseval's identity; the commutator of the expanded observables
	factorizes as in the proof of Lemma~\ref{lem:qld-comm-cons}, whose twisted
	commutation estimate bounds the right-hand side.
\end{proof}

\begin{lemma}[Formalization support for Lemma~\ref{lem:qld-4-10}: the sandwich POVM]
	\label{lem:qld-sandwich}
	...
	\leanok
	\uses{def:expanded-point-measurement, lem:qld-comm-cons,
		lem:qld-point-commutation-field, fact:add-a-proj, def:povm-distance,
		def:symmetric-equivalents}
	This formalization-only auxiliary is not a named statement of the paper;
	it is the first step of the proof of Lemma~\ref{lem:qld-4-10} with
	field-valued outcomes. For $x, z \in \F_q^m$ and $a, b \in \F_q$ let
	\[
		R^{x,z}_{a,b} = \hat{M}^{(\mathrm{Point},Z),z}_b \,
		\hat{M}^{(\mathrm{Point},X),x}_a \, \hat{M}^{(\mathrm{Point},Z),z}_b .
	\]
	Then $\{R^{x,z}_{a,b}\}_{a,b \in \F_q}$ is a POVM; on every placement,
	on average over $x, z$,
	$R^{x,z}_{a,b} \approx_{\delta_{\mathrm{Anticom}}}
	\hat{M}^{(\mathrm{Point},Z),z}_b \hat{M}^{(\mathrm{Point},X),x}_a$; and
	for every pair of opposite placements the ordered product on the first
	is $\eps$-close to the reversed ordered product on the second.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:qld-point-commutation-field, lem:qld-comm-cons, fact:add-a-proj,
		fact:triangle}
	$R^{x,z}_{a,b} = (\hat{M}^X_a \hat{M}^Z_b)^\dagger (\hat{M}^X_a \hat{M}^Z_b)$
	is positive and $\sum_{a,b} R^{x,z}_{a,b} = \sum_b \hat{M}^Z_b = \Id$.
	Since $R^{x,z}_{a,b} - \hat{M}^Z_b \hat{M}^X_a
	= \hat{M}^Z_b [\hat{M}^X_a, \hat{M}^Z_b]$ and $\hat{M}^Z_b$ is a
	contraction, Lemma~\ref{lem:qld-point-commutation-field} gives the
	closeness. For the cross-placement consistency, the two factors are
	transported one at a time: $\hat{M}^Z_b$ on the first placement times the
	difference of the two placements of $\hat{M}^X_a$, then $\hat{M}^X_a$ on
	the second placement times the difference of the two placements of
	$\hat{M}^Z_b$; each is controlled by item~1 of
	Lemma~\ref{lem:qld-comm-cons} through Lemma~\ref{fact:add-a-proj}, and
	the two estimates combine by the triangle inequality.
\end{proof}

\begin{lemma}[Formalization support for Lemma~\ref{lem:qld-4-10}: consistency of the sandwich]
	\label{lem:qld-sandwich-consistency}
	...
	\leanok
	\uses{lem:qld-sandwich, lem:qld-placement-transfer, def:consistency,
		fact:agreement, lem:qld-comm-cons, lem:qld-point-commutation-field}
	This formalization-only auxiliary is not a named statement of the paper;
	it replaces the Cauchy--Schwarz calculation in the first step of the proof
	of Lemma~4.10 of \cite{Ji2020MIPStar}
	(\texttt{references/qpbt-paper/14\_analysis\_of\_the\_pauli\_basis\_test.tex},
	lines~747--778) by an exact identity.
	Let $P_1, P_2$ denote the two placements of an
	operator $P$ on a pair of opposite register pairs, let
	$W_b = (\hat{M}^Z_b)_1 (\hat{M}^Z_b)_2$, and let
	$D^W_c = (\hat{M}^W_c)_1 - (\hat{M}^W_c)_2$. Then, for fixed $(x,z)$,
	\[
		\sum_{(a,b) \ne (a',b')} \bra{\hat\psi} (R^{x,z}_{a,b})_1
		(R^{x,z}_{a',b'})_2 \ket{\hat\psi}
		= \tfrac12 \sum_b \lVert D^Z_b \ket{\hat\psi} \rVert^2
		+ \tfrac12 \sum_{a,b} \lVert D^X_a W_b \ket{\hat\psi} \rVert^2 ,
	\]
	and the right-hand side is at most
	$c_{x,z} = \tfrac12 \sum_b \lVert D^Z_b \hat\psi \rVert^2
	+ \tfrac32 \big( \sum_a \lVert D^X_a \hat\psi \rVert^2 + K_1 + K_2 \big)$,
	where $K_i = \sum_{a,b} \lVert ([\hat{M}^X_a, \hat{M}^Z_b])_i
	\hat\psi \rVert^2$. On average over $x, z$,
	$c_{x,z} \le C (\eps + \sqrt{\eps})$ for a universal constant $C$.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:qld-sandwich, fact:agreement, lem:qld-comm-cons,
		lem:qld-point-commutation-field}
	The placements commute and the effects are Hermitian projectors, so
	$\bra{\hat\psi} (R_{a,b})_1 (R_{a,b})_2 \ket{\hat\psi}
	= \bra{W_b \hat\psi} (\hat{M}^X_a)_1 (\hat{M}^X_a)_2 \ket{W_b \hat\psi}$,
	and the identity
	$\sum_c \bra{w} P_c Q_c \ket{w} = \lVert w \rVert^2
	- \tfrac12 \sum_c \lVert (P_c - Q_c) w \rVert^2$ for projective
	measurements, applied once with $w = W_b \hat\psi$ and once with
	$w = \hat\psi$, gives the displayed identity. Writing
	$D^X_a W_b = W_b D^X_a + (\hat{M}^Z_b)_2 ([\hat{M}^X_a, \hat{M}^Z_b])_1
	- (\hat{M}^Z_b)_1 ([\hat{M}^X_a, \hat{M}^Z_b])_2$ and using that the
	$\hat{M}^Z_b$ are contractions whose squares sum to the identity gives the
	bound; its average is controlled by item~1 of
	Lemma~\ref{lem:qld-comm-cons} and Lemma~\ref{lem:qld-point-commutation-field}.
\end{proof}

\begin{lemma}[Formalization support for Lemma~\ref{lem:qld-4-10}: orthonormalization]
	...
	\leanok
	\uses{lem:qld-sandwich-consistency, lem:ortho, lem:ortho-explicit-constant,
		def:povm-distance, def:symmetric-equivalents}
	This formalization-only auxiliary is not a named statement of the paper;
	it applies Lemma~\ref{lem:ortho} for each point pair. For every $(x,z)$
	there is a projective measurement $\{\hat{Q}^{x,z}_{a,b}\}_{a,b \in \F_q}$
	on $\hilb_{\mathsf{A}} \ot (\C^q)^{\ot M}$ with
	$\sum_{a,b} \lVert (\hat{Q}^{x,z}_{a,b} - R^{x,z}_{a,b})_{\mathsf{A}\mathsf{A}'}
	\ket{\hat\psi} \rVert^2 \le 220\, c_{x,z}^{1/4}$, and on average over
	$x, z$ the left-hand side is at most $220 (\E_{x,z} c_{x,z})^{1/4}$; the
	same holds for Bob with the bipartition
	$\mathsf{B}\mathsf{A}'' \mid \mathsf{A}\mathsf{A}'\mathsf{B}'\mathsf{B}''$.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:qld-sandwich-consistency, lem:ortho-explicit-constant}
	Read in the bipartition
	$\mathsf{A}\mathsf{A}' \mid \mathsf{B}\mathsf{A}''\mathsf{B}'\mathsf{B}''$,
	the consistency defect of $R^{x,z}$ against $R^{x,z}$ on the opposite
	placement is at most $c_{x,z}$ by
	Lemma~\ref{lem:qld-sandwich-consistency}, so
	Lemma~\ref{lem:ortho-explicit-constant} supplies the projective
	measurement for each $(x,z)$; choosing one for every pair, the average of
	$220\, c_{x,z}^{1/4}$ is at most $220 (\E c_{x,z})^{1/4}$ by two
	applications of Jensen's inequality for the square root.
\end{proof}
...
\begin{proof}
	\leanok
	\uses{lem:qld-comm-cons, def:expanded-point-measurement,
		lem:qld-point-commutation-field, lem:qld-sandwich,
		lem:qld-sandwich-consistency, lem:qld-sandwich-orthonormalization,
		lem:qld-placement-transfer, fact:add-a-proj, fact:triangle, lem:ortho,
		def:povm-distance, def:consistency}
	The source proceeds through the binary refinements
	$\hat{M}^{(\mathrm{Point},W),u,r}_b$, an approximate linearity in the index
	pair $(r,s)$, Theorem~\ref{thm:linearity}, and Fourier averaging; the last
	two steps require the strategy state to carry zero-state padding
	(Remark~\ref{rem:linearity-import}), which is not part of the present
	setting. The measurements are instead constructed on
	$\hilb \ot (\C^q)^{\ot M}$ itself
	\cite{gap:qpbt_linearity-theorem-quotation}. Throughout, every
	intermediate error is a $\poly(\eps)$ function.

	\emph{The sandwich POVM.} For $x, z \in \F_q^m$ and $a, b \in \F_q$ let
	$R^{x,z}_{a,b} = \hat{M}^{(\mathrm{Point},Z),z}_b
	\hat{M}^{(\mathrm{Point},X),x}_a \hat{M}^{(\mathrm{Point},Z),z}_b$
	(Lemma~\ref{lem:qld-sandwich}). It is a POVM with outcomes in $\F_q^2$;
	by the field-valued commutation estimate of
	Lemma~\ref{lem:qld-point-commutation-field}, which transfers item~2 of
	Lemma~\ref{lem:qld-comm-cons} from the binary refinements to the
	measurements $\{\hat{M}^{(\mathrm{Point},W),u}_a\}$ by Parseval's identity
	without loss, $R^{x,z}_{a,b} \approx_{\delta_{\mathrm{Anticom}}}
	\hat{M}^Z_b \hat{M}^X_a$ on every placement, on average over $x, z$; and
	by the self-consistency of each factor (item~1 of
	Lemma~\ref{lem:qld-comm-cons}) and Lemma~\ref{fact:add-a-proj}, the
	ordered product on one placement is $\eps$-close to the reversed product
	on the opposite placement.

	\emph{Self-consistency of the sandwich.} For fixed $(x,z)$ the
	off-diagonal mass of $(R^{x,z})_{\mathsf{A}\mathsf{A}'}$ against
	$(R^{x,z})_{\mathsf{B}\mathsf{A}''}$ is computed exactly in
	Lemma~\ref{lem:qld-sandwich-consistency}: with
	$W_b = (\hat{M}^Z_b)_{\mathsf{A}\mathsf{A}'} (\hat{M}^Z_b)_{\mathsf{B}\mathsf{A}''}$
	and $D^W_c$ the difference of the two placements of $\hat{M}^W_c$, it
	equals
	$\tfrac12 \sum_b \lVert D^Z_b \hat\psi \rVert^2
	+ \tfrac12 \sum_{a,b} \lVert D^X_a W_b \hat\psi \rVert^2$, and the second
	sum is at most three times the sum of
	$\sum_a \lVert D^X_a \hat\psi \rVert^2$ and the two placed commutator
	sums. This gives a nonnegative bound $c_{x,z}$ whose average over $x, z$
	is $O(\eps + \delta_{\mathrm{Anticom}})$.

	\emph{Orthonormalization.} For each $(x,z)$, Lemma~\ref{lem:ortho} applied
	to $R^{x,z}$ on $\mathsf{A}\mathsf{A}'$ against $R^{x,z}$ on
	$\mathsf{B}\mathsf{A}''$ gives a projective measurement
	$\{\hat{Q}^{x,z}_{a,b}\}$ on $\hilb_{\mathsf{A}} \ot (\C^q)^{\ot M}$ with
	$\sum_{a,b} \lVert (\hat{Q}^{x,z}_{a,b} - R^{x,z}_{a,b})_{\mathsf{A}\mathsf{A}'}
	\ket{\hat\psi} \rVert^2 \le 220\, c_{x,z}^{1/4}$; since
	$t \mapsto t^{1/4}$ is concave, the average over $x, z$ is at most
	$220 (\E_{x,z} c_{x,z})^{1/4}$
	(Lemma~\ref{lem:qld-sandwich-orthonormalization}). The bipartition
	$\mathsf{B}\mathsf{A}'' \mid \mathsf{A}\mathsf{A}'\mathsf{B}'\mathsf{B}''$
	gives Bob's measurements in the same way. The closeness of $\hat{Q}$ to
	$R$ on $\mathsf{A}\mathsf{B}''$ and $\mathsf{B}\mathsf{B}'$ follows from
	that on $\mathsf{A}\mathsf{A}'$ and $\mathsf{B}\mathsf{A}''$ by
	Lemma~\ref{lem:qld-placement-transfer}.

	\emph{Conclusions.} On every pair of opposite placements, the triangle
	inequality (Lemma~\ref{fact:triangle}) along
	$\hat{Q} \approx R \approx \hat{M}^Z \hat{M}^X \approx \hat{M}^X \hat{M}^Z$,
	together with the cross-placement consistency of the ordered products and
	the commutation estimate on the second placement, gives
	\eqref{eq:qld-q-cons-m-hat-xz} and \eqref{eq:qld-q-cons-m-hat-zx}, and,
	continuing back to $\hat{Q}$ on the second placement,
	\eqref{eq:qld-q-self-cons}, with all symmetric equivalents. When
	$\eps \le 1$ the error terms $\eps$, $\sqrt{\eps}$, and
	$(\eps + \sqrt{\eps})^{1/4}$ are bounded by multiples of $\eps^{1/8}$;
	when $\eps > 1$, any two operator families satisfying
	$\sum_a A_a^\dagger A_a \le \Id$ and
	$\sum_a B_a^\dagger B_a \le \Id$ for every question have averaged squared
	state-dependent distance at most $4$ on the unit state $\ket{\hat\psi}$. Hence
	$\delta_Q(\eps) = K \eps^{1/8}$ for a universal constant $K$.
\end{proof}
...
\begin{theorem}[Formalization support for Lemma~\ref{lem:qld-4-12}: averaged affine coarse-graining]
	\label{thm:qld-affine-coarse-graining}
	\lean{MIPStarRE.QPBT.opFamilyDistSq_uniform_affine_postprocess_le}
	\leanok
	\uses{def:povm-distance}
	This formalization-only auxiliary is not a named statement of the paper;
	it supplies the squared-distance estimate used in the coarse-graining
	step of Lemma~4.12 of \cite{Ji2020MIPStar}.
	Let $\F_q$ be a finite field, let $T$ be a finite nonempty set, and let
	$\ket\psi$ be a vector in a finite-dimensional complex Hilbert space
	$\hilb$. For each $t \in T$,
	let $\{A^t_{a,b}\}_{a,b \in \F_q}$ and $\{B^t_{a,b}\}_{a,b \in \F_q}$
	be operator families on $\hilb$ with equal total operators:
	$\sum_{a,b} A^t_{a,b} = \sum_{a,b} B^t_{a,b}$.
	For $\alpha,\beta,c \in \F_q$, set
	\[
		A^{t,\alpha,\beta}_c =
		\sum_{a,b\,:\,\alpha a+\beta b=c} A^t_{a,b},
		\qquad
		B^{t,\alpha,\beta}_c =
		\sum_{a,b\,:\,\alpha a+\beta b=c} B^t_{a,b}.
	\]
	Then, with all averages uniform and independent,
	\[
		\E_{t,\alpha,\beta} \sum_c
		\big\lVert (A^{t,\alpha,\beta}_c-B^{t,\alpha,\beta}_c)
		\ket\psi \big\rVert^2
		\le \E_t \sum_{a,b}
		\big\lVert (A^t_{a,b}-B^t_{a,b})\ket\psi \big\rVert^2.
	\]
\end{theorem}
\begin{proof}
	\leanok
	\uses{def:povm-distance}
	Fix $t$ and write $v_{a,b}=(A^t_{a,b}-B^t_{a,b})\ket\psi$.
	The equality of total operators gives $\sum_{a,b} v_{a,b}=0$.
	Two distinct pairs $(a,b)$ and $(a',b')$ satisfy
	$\alpha a+\beta b=\alpha a'+\beta b'$ with probability $1/q$ over
	uniform independent $\alpha,\beta$: the corresponding nonzero linear
	equation in two variables has $q$ solutions. Expanding squared norms
	in inner products therefore gives
	\[
		\E_{\alpha,\beta} \sum_c
		\Big\lVert \sum_{a,b\,:\,\alpha a+\beta b=c} v_{a,b} \Big\rVert^2
		= (1-q^{-1})\sum_{a,b}\lVert v_{a,b}\rVert^2
		+ q^{-1}\Big\lVert\sum_{a,b}v_{a,b}\Big\rVert^2
		\le \sum_{a,b}\lVert v_{a,b}\rVert^2.
	\]
	Averaging over $t$ proves the claim.
\end{proof}
...
\begin{proof}
	...
	\leanok
	\uses{lem:qld-4-10, thm:qld-affine-coarse-graining}
	Define
	$\hat{Q}^{x,z,\alpha,\beta}_{c}
	= \sum_{a,b \,:\, \alpha a + \beta b = c} \hat{Q}^{x,z}_{a,b}$,
	a coarse-graining of the projective measurement of
	Lemma~\ref{lem:qld-4-10}, hence itself projective. Completeness gives
	$\sum_{a,b}\hat{Q}^{x,z}_{a,b}=\Id$ and
	\[
		\sum_{a,b}\hat{M}^{(\mathrm{Point},X),x}_a
		\hat{M}^{(\mathrm{Point},Z),z}_b
		= \sum_{a,b}\hat{M}^{(\mathrm{Point},Z),z}_b
		\hat{M}^{(\mathrm{Point},X),x}_a = \Id.
	\]
	Register placement preserves these sums and the identity. Thus each
	pair of operator families compared in Lemma~\ref{lem:qld-4-10} has
	equal total operators, including both ordered products. Apply
	Theorem~\ref{thm:qld-affine-coarse-graining} to each pair, averaging
	uniformly and independently over $(x,z)$ and both coefficients
	$\alpha,\beta$. This is the uniform distribution on $\F_q^{2m+2}$,
	and the three squared-distance bounds retain the error $\delta_Q$.
	The same argument applies to every opposite pair of register placements.
\end{proof}
...
\begin{definition}[Restricted line distributions]
	...
	Here the paper's one-based indexing is retained. Under zero-based
	indexing, write $i_0=i-1$ and $\chi_0(s)=\chi(s)-1$. Then
	$\chi(s)=i$ means $\chi_0(s)=i_0$, and the vanishing coordinates
	are those indexed by $0\le j<i_0$.
\end{definition}
