...
\begin{lemma}[Uniform specialization in a block of variables]
	...
	% The specialization step of the source proof of lem:qld-4-7, between
	% eq:qld-g-2 and eq:qld-g-prime-xpt-bound in references/qpbt-paper/
	% 14_analysis_of_the_pauli_basis_test.tex.  The source applies it with
	% D = (2m+2)d and with the weights given by the expectations of a
	% complete measurement.
	Let $p \in \F_q[z_1,\dots,z_m,x_1,\dots,x_k]$ have total degree at most
	$D$ and assume that $p$ is not a polynomial in $z$ alone.  Write
	$p = \sum_\mu c_\mu(z)\,x^\mu$.  Then there is an exponent $\mu \neq 0$
	with $c_\mu \neq 0$ such that $c_\mu$ has total degree at most $D$,
	\[
...
\begin{proof}
	\uses{lem:qld-4-13-established, lem:qld-4-12, thm:naimark,
		...
		def:combine-map, lem:qpbt-block-specialization}
