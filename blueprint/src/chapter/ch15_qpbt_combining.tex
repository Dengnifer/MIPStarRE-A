...
\begin{lemma}[Formalization support: a uniform point from the directly indexed law]
	...
	\uses{lem:qld-sublines, def:line-point-dist, def:line-representative,
		def:combine-map}
	Let $D_{\rm dir}$ be the auxiliary distribution recorded by
	\texttt{SubLineWitness}, whose extended-line marginal is that of
	\texttt{directLinePointDist}. A uniform affine parameter on that
...
consistency defect at most $\delta_P$. These are the hypotheses of
\texttt{CombinedPointsWitness} and \texttt{CombinedLinesWitness}.
Let $D_{\rm dir}$ be any \texttt{SubLineWitness}: it has mass one, the
\texttt{directLinePointDist} extended-line marginal, incidence and affine
...
polynomial representatives of degree at most $md$, evaluations have
\texttt{Option} values, and completed point effects are zero at undefined
