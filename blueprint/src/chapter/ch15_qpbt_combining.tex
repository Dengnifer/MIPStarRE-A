...
% to O^u on average over uniformly random u only. Its binary-measurement
% distance bound delta becomes the operator-distance bound 2 delta under
% the convention used below; it is stated for
...
% cited theorem, and none is reproduced here. See rem:linearity-import and
% the normalization analysis in gap:qpbt_linearity-distance-normalization.
...
\begin{proof}
	...
	Lemma~\ref{lem:qld-4-10}. The factor two in the distance conclusion is
	the normalization correction documented in
	\cite{gap:qpbt_linearity-distance-normalization}.
\end{proof}
...
\begin{remark}[Obligations of the linearity-theorem import]
	...
	proof of Lemma~\ref{lem:qld-4-10}: the error suffers only the explicit
	constant-factor loss ($\delta$ in the correlation hypothesis becomes
	$2\delta$ in squared operator distance); the linearity of the
	$\{\mathcal{L}^u\}$ is exact and
	...
	the identity. These quotation discrepancies and the fiberwise
	common-extension obligation are documented in
	...
	pair. In particular, the binary-measurement distance of the cited theorem
	is one half of the squared operator distance used here, which accounts
	for the factor two and cannot be omitted; see
	\cite{gap:qpbt_linearity-distance-normalization}. One obligation remains
	open and is flagged where it
...
\begin{lemma}[Combined line measurement on the extended coordinates]
	...
	% Correction of the source statement (references/qpbt-paper/
	% 14_analysis_of_the_pauli_basis_test.tex, lem:qld-4-13): neither of its
	% two proof routes establishes poly(m^2 eps, md/q). The route retained
	% below establishes m * poly(eps, md/q), which is the form the sequel
	% uses. See
...
\begin{proof}[Proof of Lemma~\ref{lem:qld-4-13}]
	...
	function of the stated form $m \cdot \poly(\eps, md/q)$. The source's
	printed form $\poly(m^2\eps, md/q)$ does not follow from this bound, and
	its second route rests on a joint-law decomposition not provided by
	Property~2 of Lemma~\ref{lem:qld-sublines}. The corrected bound is the
	one used in the sequel (Remark~\ref{rem:qld-4-13-source-defects}; the two
	...
	\eqref{eq:qld-4-13}, with the stated error
	$\delta_{\mathrm{combine}} = m \cdot \poly(\eps, md/q)$. The second
	relation follows from the
...
\begin{remark}[The error term in the source proof of
	...
	The source states the error as
	$\delta_{\mathrm{combine}} = \poly(m^2\eps, md/q)$, which its proof
	does not establish. Lemma~\ref{lem:qld-4-13} instead records the
	corrected form $m \cdot \poly(\eps, md/q)$. The source proof in
	\cite{Ji2020MIPStar} asserts
	...
	therefore follows the first route only and establishes the corrected
	bound $m \cdot \poly(\eps, md/q)$, which is the form consumed by
