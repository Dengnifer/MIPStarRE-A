...
\begin{proof}
	...
	\uses{lem:qld-comm-cons, def:expanded-point-measurement,
		lem:ortho, fact:triangle, def:povm-distance,
		def:symmetric-equivalents}
	For each pair $x,z$, form the field-valued sandwich POVM
	\[
		R^{x,z}_{a,b}
		...
		\qquad a,b\in\F_q.
	\]
	Completeness and projectivity of the point measurements make this a
	POVM. Their field-valued approximate commutation bounds its distance
	from the ordered product, and their opposite-placement consistency
	bounds the off-diagonal agreement defect of the two sandwich POVMs.
	Apply the orthonormalization lemma (Lemma~\ref{lem:ortho}) separately
	on Alice's and Bob's expanded local spaces for each point pair.
	Averaging the resulting distances gives projective measurements close
	to the sandwiches. The squared-distance triangle inequality then gives
	self-consistency and both ordered-product comparisons. Tensor-register
	exchange gives the other two directed opposite-placement comparisons.
	The error terms are bounded by $K\eps^{1/8}$ for a universal $K$:
	for $0\leq\eps\leq1$ use monotonicity of the fractional powers, and
	for $\eps>1$ use the uniform bound $4$ for these squared distances.
	This construction stays on the original expanded local spaces. It
	does not use the source's binary refinements followed by quantum
	linearity, whose additional state-padding requirement is discussed in
...
\begin{definition}[The constructed paired-line POVM]
	...
	For either player, define
	$T^{\ell_X,\ell_Z}_{f_X,f_Z}
	=\hat M^{(\mathrm{Line},X),\ell_X}_{f_X}
	 \hat M^{(\mathrm{Line},Z),\ell_Z}_{f_Z}
	 \hat M^{(\mathrm{Line},X),\ell_X}_{f_X}$.
	This is a POVM. If $\ell_W$ is axis-parallel, effects whose
	$W$-polynomial answer has degree greater than $d$ vanish.
	This construction, from the proof of Lemma~\ref{lem:qld-xz-lines},
	asserts no consistency estimate.
\end{definition}

\begin{lemma}[The exact $X$ marginal of the paired-line POVM]
	\label{lem:combined-line-measurement-x-marginal}
	\lean{MIPStarRE.QPBT.ProjectiveSetting.combinedLineMeasurement_sum_Z}
	\leanok
	\uses{def:combined-line-measurement, def:expanded-line-measurement}
	For every line pair and every $f_X$,
	\[
...
\begin{lemma}[Consistency of the constructed paired-line measurement]
	...
	measurement. Its proof remains unfinished; see \cite{gap:qpbt_subline-claims-line-marginal}.
\end{lemma}
...
\begin{lemma}[Auxiliary directly indexed subline distribution]
	...
	Define the directly indexed extended line-point law as follows.
	Choose the line kind with equal probabilities, an index
	$j\in\{1,\ldots,2m+2\}$ uniformly, and a point $u\in\F_q^{2m+2}$
	uniformly. For an axis line take direction $e_j$; for a diagonal line
	choose a uniform vector and zero its coordinates before $j$.
	Replace $u$ by its canonical representative to describe the line,
	retaining $u$ as its sampled point.
	There exists a probability distribution $D_{\mathrm{dir}}$ on
	$(\ell,\ell_X,\ell_Z)$ whose extended-line marginal is the line
	marginal of this law, satisfying the incidence, separate projected
	marginal mixture, and axis-closure properties stated in
	Lemma~\ref{lem:qld-sublines}. Its line parametrizations also supply the
	affine compatibility in Definition~\ref{def:combine-map}.
	This auxiliary samples $j$ directly, rather than as $\chi_{2m+2}(s)$
	for a uniform scalar seed $s$. It does not identify $D_{\mathrm{dir}}$
	with the source distribution $D$; that source correspondence remains
	unresolved as explained in \cite{gap:qpbt_ld-dimension-divisibility}.
\end{lemma}
\begin{proof}
	\leanok
	Sample the auxiliary source lines by the blockwise procedure described
	above, starting from the directly sampled extended index. The separate
	$X$ and $Z$ marginals are mixtures of the restricted product laws.
	Incidence, affine compatibility, and axis closure follow from the
	coordinate-block formulas.
\end{proof}
...
\begin{lemma}[Formalization support: replacing one factor of an overlap]
	...
	A complete measurement weights the real part of the overlap gap between
	operator families by at most the square root of their state-dependent
	squared distance. This auxiliary does not bound the complex modulus.
\end{lemma}
\begin{proof}
	\leanok
...
\begin{lemma}[Auxiliary uniform point under the directly indexed law]
	...
	\leanok
	\uses{lem:qld-sublines-direct, def:line-representative, def:combine-map}
	For any directly indexed witness $D_{\mathrm{dir}}$ of
	Lemma~\ref{lem:qld-sublines-direct}, a uniform affine parameter on the
	sampled extended line gives a uniform
...
\begin{remark}[Scalar conventions and source targets]
	\label{rem:subline-real-part-scope}
	In the next three source claims, scalar approximation bounds the
	complex modulus of the difference. The following auxiliary notation
	separates the results proved for real parts under $D_{\mathrm{dir}}$.
	For any directly indexed witness, and any point and line families
	satisfying the conclusions of Lemmas~\ref{lem:qld-4-10}
	and~\ref{lem:qld-xz-lines} with errors $\delta_Q,\delta_P$, define
	\[
		A_F(D_{\mathrm{dir}})
		=\E_{D_{\mathrm{dir}}}\E_{t\in\F_q}\sum_{f_X,f_Z}
		\bra{\hat\psi}T^{\ell_X,\ell_Z}_{f_X,f_Z}
		\ot F^{x,z}_{f_X(x),f_Z(z)}\ket{\hat\psi},
		\quad (x,z,\alpha,\beta)=\ell(t).
	\]
	Write $A_Q,A_{ZX},A_Z$ for $F=Q$, $F=M^ZM^X$, and $F=M^Z$.
	The averages include all line-polynomial answers with the optional
	evaluation convention: point effects are zero at an undefined
	evaluation, and $Q$ is completed by zero on either undefined coordinate.
	The next three auxiliary entries do not certify the source claims.
	In particular, a bound on $\lvert\operatorname{Re}(A_Q-A_{ZX})\rvert$
	does not bound $\lvert A_Q-A_{ZX}\rvert$; no reality assertion for
	the ordered product has been established. Both this scalar difference
	and the distribution difference are recorded in
	\cite{gap:qpbt_subline-claims-line-marginal}.
\end{remark}

\begin{lemma}[Auxiliary real-part ordered-product estimate]
	\label{lem:claim-17-1-re-direct}
	\lean{MIPStarRE.QPBT.subline_replace_by_ordered_product_re_direct}
	\leanok
	\uses{lem:qld-sublines-direct, lem:qld-sublines-uniform-point,
		lem:qld-4-10-same-placement, lem:overlap-gap-distance,
		lem:qld-4-10, lem:qld-xz-lines}
	Under the hypotheses and conventions of
	Remark~\ref{rem:subline-real-part-scope}, there is a universal $C>0$
	such that
	$\lvert\operatorname{Re}(A_Q(D_{\mathrm{dir}})
	-A_{ZX}(D_{\mathrm{dir}}))\rvert\leq C\sqrt{\delta_Q}$.
\end{lemma}
\begin{proof}
	\leanok
	Regroup by optional point evaluations and apply
	Lemma~\ref{lem:overlap-gap-distance}. Completing the point outcomes
	adds zero terms, and Lemma~\ref{lem:qld-sublines-uniform-point}
	transports the squared-distance average to independent uniform $x,z$.
	Lemma~\ref{lem:qld-4-10-same-placement} bounds it by $4\delta_Q$,
	so $C=2$ suffices.
\end{proof}

\begin{lemma}[Internal real-part $X$-factor removal obligation]
	\label{lem:claim-17-2-re-direct}
	\lean{MIPStarRE.QPBT.subline_remove_X_factor_re_direct}
	\uses{lem:qld-sublines-direct, def:combined-line-measurement,
		lem:combined-line-measurement-x-marginal, lem:qld-comm-line-cons}
	For every admissible projective setting and every directly indexed
	subline witness, let $T$ be the concrete POVM of
	Definition~\ref{def:combined-line-measurement}. With the scalar and
	optional-evaluation conventions of
	Remark~\ref{rem:subline-real-part-scope}, there is a universal $C>0$
	such that
	\[
		\lvert\operatorname{Re}(A_{ZX}(D_{\mathrm{dir}})
		-A_Z(D_{\mathrm{dir}}))\rvert
		\leq Cm\sqrt{\delta_{\mathrm{Line}}(\eps)}.
	\]
	No joint-point family or evaluated paired-line consistency estimate
	is assumed here. This is an unfinished internal obligation, not the
	complex-modulus source claim. Its intended proof uses the exact $X$
	marginal, expanded line-point consistency, and the separate restricted
	$X$ marginal of $D_{\mathrm{dir}}$.
\end{lemma}

\begin{lemma}[Auxiliary real-part $Z$-correlation estimate]
	\label{lem:claim-17-3-re-direct}
	\lean{MIPStarRE.QPBT.subline_Z_term_near_one_re_direct}
	\leanok
	\uses{lem:qld-sublines-direct, lem:restricted-line-mixture-bounds,
		lem:qld-4-10, lem:qld-xz-lines}
	Under the hypotheses and conventions of
	Remark~\ref{rem:subline-real-part-scope}, there is a universal $C>0$
	such that
	\[
		\lvert\operatorname{Re}A_Z(D_{\mathrm{dir}})-1\rvert
		\leq C\sqrt m
		(\delta_P^{1/4}+\delta_Q^{1/4}+\eps^{1/4}).
	\]
\end{lemma}
\begin{proof}
	\leanok
	The separate restricted $Z$ marginal and point comparisons bound
	$1-\operatorname{Re}A_Z$ by
	$2\sqrt{4m^2\delta_P}+2\sqrt{4\delta_Q}$.
	Since $0\leq\operatorname{Re}A_Z\leq1$, its deficit is at most the
	square root of this bound. Splitting the square root and using $m\geq1$
	gives the displayed estimate with $C=2$.
\end{proof}
...
\begin{proof}
	...
	- \hat{M}^{(\mathrm{Point},Z),z}_{b}
	\hat{M}^{(\mathrm{Point},X),x}_{a}$
	and apply the complex Cauchy--Schwarz inequality to the difference.
	Completeness of $T$ bounds the first factor by $1$. For the squared
	second factor, use
	$\sum_{f_X,f_Z \,:\, f_X(x)=a,\ f_Z(z)=b}
	T^{\ell_X,\ell_Z}_{f_X,f_Z}\leq\Id$ to obtain
	\[
		\E_{x,z}\sum_{a,b}
		\bra{\hat\psi}\Id\ot
		(W^{x,z}_{a,b})^\dagger W^{x,z}_{a,b}\ket{\hat\psi}
		\leq 4\delta_Q.
	\]
	Here uniformity of the extended point makes $x,z$ independent and
	uniform, and the same-placement bound is
	Lemma~\ref{lem:qld-4-10-same-placement}. Thus the error is at most
	$2\sqrt{\delta_Q}$, with the stated asymptotic order.
	The adjoint is necessary because $W$ need not be self-adjoint; the
	source's $W^2$ and $\delta_Q$ in this intermediate calculation are
	replaced by $W^\dagger W$ and $4\delta_Q$.
	This is a mathematical argument, not a completed Lean proof of the
	source claim: the available scalar estimate controls only real parts
	under $D_{\mathrm{dir}}$. See
	\cite{gap:qpbt_subline-claims-line-marginal}.
\end{proof}
...
\begin{proof}
	\uses{lem:qld-comm-line-cons, lem:qld-sublines, lem:qld-xz-lines,
		lem:combined-line-measurement-x-marginal, lem:restricted-line-mixture-bounds, def:ith-restricted-line,
