...
\begin{definition}[Formalization support: retained nondegenerate mass]
	...
	Definition~\ref{def:ld-game}, and write $D_{\mathsf{Line}}^L$ for the
	line-point distribution of Definition~\ref{def:line-point-dist}. For two
	independent samples $(\ell_X,x),(\ell_Z,z)\sim D_{\mathsf{Line}}^L$, define
	the retained mass
	\[
		\mu_{\mathrm{nd}}(L)
		:= \sum_{\substack{(\ell_X,x),(\ell_Z,z)\\
			\operatorname{dir}(\ell_X)\ne 0}}
		D_{\mathsf{Line}}^L(\ell_X,x)D_{\mathsf{Line}}^L(\ell_Z,z).
	\]
...
\begin{proof}
	\leanok
	\uses{def:line-point-dist, lem:axis-line-zero-direction-mass}
...
\begin{definition}[Formalization support: conditioned pasting distribution]
	...
	\]
	Denote the resulting distribution by $D_{\mathrm{nd}}^L$. Thus the paired
	lines are ordered $X,Z$, followed by the full $Z$ sample and then the full
	...
	independent, unconditioned samples from $D_{\mathsf{Line}}^L$; the transfer
	back to that source law remains part of the uncertified proof of that lemma.
\end{definition}
