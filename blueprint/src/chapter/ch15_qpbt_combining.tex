...
\begin{theorem}[Resampling a line parameter]
	...
	\lean{MIPStarRE.QPBT.avgOver_lineRepMap_resample_parameter}
	\leanok
...
Displayed coordinate labels are one-based. In a formal declaration at ambient
dimension $r$, the label $i\in\{1,\dots,r\}$ is represented by the zero-based
index $i-1\in\{0,\dots,r-1\}$.
...
\begin{definition}[Restricted line distributions]
	...
	For any $i \in \{1, \dots, m\}$, the \emph{$i$-th restricted refined
	...
	$(\ell,u)$ whose retained seed $s$ satisfies $\chi(s)=i$, so the geometric
	...
	where $\ell=(u_0,s,v)$ satisfies $\chi(s)=i$ and coordinates
...
\begin{lemma}[Restriction of a given line consistency bound]
	\label{lem:restricted-line-consistency-bound}
	\lean{MIPStarRE.QPBT.restricted_lines_consistency_bound}
	\leanok
	\uses{lem:qld-xz-lines, lem:qld-4-10, def:ith-restricted-line,
		def:consistency, def:expanded-state}
	This auxiliary estimate isolates the application following
	Definition~\ref{def:ith-restricted-line} in \cite{Ji2020MIPStar}.
	There is a universal constant $C>0$ such that the following holds.
	Fix admissible parameters and a projective setting, and suppose that
	point measurements $\hat Q$ and line measurements $T$ satisfying the
	conclusions of Lemmas~\ref{lem:qld-4-10} and~\ref{lem:qld-xz-lines}
	are given in the seed-bearing presentation, with consistency error
	$\delta_P$ for the latter over
	$\widetilde D_{\mathrm{Line}}\times\widetilde D_{\mathrm{Line}}$.
	For any types $\mathrm{v}_1,\mathrm{v}_2\in
	\{\mathrm{ALine},\mathrm{DLine}\}$ and displayed one-based indices
	$i,j\in\{1,\dots,m\}$, represented formally by $i-1$ and $j-1$,
	\begin{equation}
		\label{eq:qld-xz-lines-restricted}
		\E_{(\ell_X,x)\sim \widetilde D_{\mathrm{v}_1,i}}\
		\E_{(\ell_Z,z)\sim \widetilde D_{\mathrm{v}_2,j}}
		\sum_{f_X,f_Z}
		\bra{\hat\psi}
		...
		\le C m^2\delta_P\,.
	\end{equation}
	The same bound holds for every directed pair of opposite placements.
	The constant $C$ is independent of the parameters and measurements. If the
	line measurements factor through the source line descriptions, forgetting
	the retained seeds gives the source restriction estimate of
	\cite{Ji2020MIPStar}.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:restricted-line-mixture-bounds, def:consistency}
	Take $C=4$. Each summand is nonnegative, since the two positive operators act on
	opposite registers. Apply item 2 of
	Lemma~\ref{lem:restricted-line-mixture-bounds} to this defect and the
	given bound $\delta_P$ over
	$\widetilde D_{\mathrm{Line}}\times\widetilde D_{\mathrm{Line}}$.
\end{proof}

\begin{theorem}[Formalization support for the sub-line distribution]
	\label{thm:direct-sub-line-distribution}
	\lean{MIPStarRE.QPBT.SubLineWitness, MIPStarRE.QPBT.exists_subLineWitness}
	\leanok
	\uses{def:admissible, def:line, def:line-representative,
		def:ith-restricted-line, def:combine-map}
	This auxiliary result is not the source statement of
	Lemma~\ref{lem:qld-sublines}. Fix admissible $(q,m,d)$ and put
	$n=2m+2$. Let $D^{\mathrm{dir}}_{\mathrm{Line}}$ be the directly
	indexed line-point law of \cite{gap:qpbt_ld-dimension-divisibility}:
	choose a kind and a displayed one-based index $j\in\{1,\dots,n\}$
	independently and uniformly, a uniform point $u\in\F_q^n$, and direction
	$e_j$ in the axis case or a uniform direction with coordinates before $j$ zero in
	the diagonal case. The formal index is the corresponding zero-based value
	$j-1\in\{0,\dots,n-1\}$. The line records $j$, its direction $v$, and the
	canonical base $L_v^{\mathrm{Ln}}(u)$. No hypothesis $n\mid q$ is imposed.
	There exists a probability distribution $D$ on triples
	$(\ell,\ell_X,\ell_Z)$, with $\ell$ directly indexed and
	$\ell_X,\ell_Z$ seed-indexed lines in $\F_q^m$, such that:
	\begin{enumerate}
		\item The marginal of $\ell$ is the line marginal of
		$D^{\mathrm{dir}}_{\mathrm{Line}}$.
		\item For every triple in the support of $D$, write
		$\ell=b+\F_q v$, $\ell_X=b_X+\F_q v_X$, and
		$\ell_Z=b_Z+\F_q v_Z$ with their recorded bases and directions.
		There are $a_X,c_X,a_Z,c_Z\in\F_q$ such that, for all $t\in\F_q$,
		\[
			\operatorname{proj}_X(b+tv)=b_X+(a_X+c_Xt)v_X,
			\qquad
			\operatorname{proj}_Z(b+tv)=b_Z+(a_Z+c_Zt)v_Z.
		\]
		Thus every point of $\ell$ projects onto both recorded sub-lines,
		with the affine compatibility of Definition~\ref{def:combine-map}.
		\item Sample a triple from $D$ and set $u=b_\ell+tv_\ell$ for
		independent uniform $t\in\F_q$, writing
		$u=(u_X,u_Z,\alpha,\beta)$. There is a probability vector
		$(\lambda_{\mathrm v,i,j})$ on
		$\{\mathrm{ALine},\mathrm{DLine}\}\times\{1,\dots,m\}^2$
		(the formal component uses the zero-based pair $(i-1,j-1)$),
		such that, if $\widetilde L_{\mathrm v,i}$ denotes the line marginal
		of $\widetilde D_{\mathrm v,i}$, the two separate marginal laws are
		\begin{align*}
			\mathcal L\big((\ell_X,u_X),\ell_Z\big)
			&=\sum_{\mathrm v,i,j}\lambda_{\mathrm v,i,j}
			\widetilde D_{\mathrm v,i}\otimes
			\widetilde L_{\mathrm v,j},\\
			\mathcal L\big(\ell_X,(\ell_Z,u_Z)\big)
			&=\sum_{\mathrm v,i,j}\lambda_{\mathrm v,i,j}
			\widetilde L_{\mathrm v,i}\otimes
			\widetilde D_{\mathrm v,j}.
		\end{align*}
		The tildes are essential because both projected lines lie on the
		seed-bearing carriers; in particular, an axis line does not lie on the
		source carrier until its seed is forgotten. Applying the forgetful maps
		to the two projected-line coordinates gives the corresponding identities
		for the untilded source restrictions.
		These identities make no assertion of conditional independence of
		$u_X$ and $u_Z$.
		\item If $\ell$ is axis-parallel, both $\ell_X$ and $\ell_Z$ are
		axis-parallel.
	\end{enumerate}
	Transport to the seed-indexed extended line-point law in
	Lemma~\ref{lem:qld-sublines} remains unresolved, as recorded in
	\cite{gap:qpbt_ld-dimension-divisibility}.
\end{theorem}
\begin{proof}
	\leanok
	\uses{def:line-representative, def:ith-restricted-line, def:line-point-dist}
	Sample the extended kind and coordinate index directly. For each source
	block, use the coordinate's index within that block when it lies there,
	and the first coordinate otherwise; choose a seed uniformly in the corresponding
	fiber of $\chi$. In the axis case use the chosen coordinate direction.
	In the diagonal case truncate a uniform direction block at that index,
	reusing the extended direction block whenever its projection is retained.
	Take canonical representatives of the projected sampled point for the
	two source bases. All sampling laws are normalized, and forgetting the
	sub-lines recovers the directly indexed extended line marginal. In each branch,
	the projected direction is a scalar multiple of the chosen sub-line
	direction, giving the affine identities and axis closure. Canonical
	representatives are unchanged along each extended line. Resampling its
	point therefore preserves the two sub-lines; regrouping the independent
	point, seed, and direction coordinates gives the two restricted product
	marginals. Averaging over the branch indices gives their common weights
	$\lambda_{\mathrm v,i,j}$.
\end{proof}
