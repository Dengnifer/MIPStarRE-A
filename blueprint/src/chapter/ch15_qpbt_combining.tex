...
\begin{lemma}[Formalization support: joint point self-consistency on line-point samples]
	\label{lem:qld-point-self-consistency-transport}
	...
	These auxiliary estimates are not named statements of the paper. Fix a
	supplied family of projective joint point measurements whose squared
	distance across every directed opposite placement is at most $\delta_Q$
	when $(x,z)$ is uniform. Its consistency defect on the same uniform point
	pairs is at most $\delta_Q$. The defect has the same bound when two
	line-point samples are drawn independently and their point coordinates
	are used as $(x,z)$. The source uses these estimates in the second
	pasting hypothesis after Equation~\ref{eq:qld-q-self-cons}.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:qld-line-point-marginal-identities, fact:agreement}
	Projectivity bounds the defect by squared distance. The product of the
	two uniform point marginals of the line-point law gives the second bound.
\end{proof}
