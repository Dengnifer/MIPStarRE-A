...
\begin{definition}[Formalization support for Definition~\ref{def:combine-map}: the separated locus]
	...
	$(f, g)$. Call $p \in \mathrm{ideg}_{d,2m+2}(\F_q)$ \emph{separated} when,
	viewed as a polynomial in $\alpha$ and $\beta$ over the point variables,
	every coefficient of $p$ vanishes except those of $\alpha$ and of $\beta$,
	the coefficient of $\alpha$ involves only the $x$ variables, and the
	coefficient of $\beta$ involves only the $z$ variables. The two components
	are recovered by specialization: setting $(\alpha, \beta) = (1, 0)$ and
	$z = 0$ returns $f$, and setting $(\alpha, \beta) = (0, 1)$ and $x = 0$
	returns $g$. The condition is stated by coefficient vanishing and variable
...
\begin{lemma}[Formalization support for Definition~\ref{def:combine-map}: the separated image]
	...
	Lemma~\ref{lem:qld-4-7} with pairs of bounded-degree polynomials. The map
	$(f, g) \mapsto \mathrm{combine}_{f,g}$ is injective. Its image is exactly
	the separated locus of
	Definition~\ref{def:combine-map-separated-locus}, and the two specialization
	maps invert it there. For $d \ge 1$, a combined polynomial lies in
	$\mathrm{ideg}_{d,2m+2}(\F_q)$ if and only if both components lie in
	$\mathrm{ideg}_{d,m}(\F_q)$, and a separated
	$p \in \mathrm{ideg}_{d,2m+2}(\F_q)$ has exactly one pair of components in
	$\mathrm{ideg}_{d,m}(\F_q)$; the two statements give a bijection between
	pairs of bounded-degree polynomials and the bounded separated locus. The
...
\begin{proof}
	...
	respectively; the two block inclusions are injective. Specialization recovers
	each component from the coefficient of the corresponding combining variable,
	which gives both round-trip identities and injectivity. If $p$ is separated,
	its two coefficients are supported in the respective blocks, so each is the
	renaming of a polynomial in $m$ variables and recombining the two returns
	$p$. Individual degrees are unchanged by renaming along an injection, so each
	component inherits the bound of $p$ from the bound on the coefficients of
	$p$; conversely the tuple degree bound supplies the bound for
