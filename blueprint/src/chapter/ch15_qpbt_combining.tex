...
\begin{theorem}[Resampling a line parameter]
	...
	$v \in K^m$, and write $R_v = L^{\lnf}_v$ for the canonical line
...
\begin{corollary}[Resampling a point from the line-point distribution]
	...
	$(q,m,d,k)$ as in Definition~\ref{def:ld-game}, let
	$D$ be any of $D_{\mathrm{ALine}}$, $D_{\mathrm{DLine}}$, or their
	equal mixture $D_{\mathrm{Line}}$. Write $b_\ell$ and $v_\ell$ for
	the canonical base and direction of a line. For every real-valued
	function $f$ on line-point pairs,
	\[
		\E_{(\ell,u)\sim D} f(\ell,u)
		= \E_{(\ell,u)\sim D}\E_{t\in\F_q}
...
\begin{proof}
	...
	identities. Taking their equal mixture proves the mixed identity.
\end{proof}
...
\begin{definition}[Restricted line distributions]
	...
	Both restricted distributions have total mass one.
\end{definition}
...
\begin{lemma}[Restricted line distributions and error inflation]
	...
	$D_{\mathrm{DLine},i}$. For nonnegative real-valued functions on
	line-point samples, the following bounds hold:
	\begin{enumerate}
		\item any average over the line-point distribution
		$D_{\mathrm{Line}}$ bounded by $\delta$ is bounded over each
		restricted distribution $D_{\mathrm{ALine},i}$ and
		$D_{\mathrm{DLine},i}$ by $2m\delta$;
		\item any average over $D_{\mathrm{Line}} \times D_{\mathrm{Line}}$
		bounded by $\delta$ is bounded over any product of restricted
...
\begin{lemma}[Restriction of a given line consistency bound]
	...
	are given, with consistency error $\delta_P$ for the latter.
	For any types $\mathrm{v}_1,\mathrm{v}_2\in
	\{\mathrm{ALine},\mathrm{DLine}\}$ and indices
	$i,j\in\{1,\dots,m\}$,
	\begin{equation}
	...
	The constant $C$ is independent of the parameters and measurements.
\end{lemma}
\begin{proof}
	\leanok
	...
	given bound $\delta_P$ over $D_{\mathrm{Line}}\times D_{\mathrm{Line}}$.
\end{proof}
...
\begin{theorem}[Formalization support for the sub-line distribution]
	...
	choose a kind and an index $j\in\{1,\dots,n\}$ independently and
	uniformly, a uniform point $u\in\F_q^n$, and direction $e_j$ in the
	axis case or a uniform direction with coordinates before $j$ zero in
	the diagonal case. The line records $j$, its direction $v$, and the
	canonical base $L_v^{\lnf}(u)$. No hypothesis $n\mid q$ is imposed.
	...
	\begin{enumerate}
		...
		such that, if $L_{\mathrm v,i}$ denotes the line marginal of
		$D_{\mathrm v,i}$, the two separate marginal laws are
		\begin{align*}
			\mathcal L\big((\ell_X,u_X),\ell_Z\big)
			&=\sum_{\mathrm v,i,j}\lambda_{\mathrm v,i,j}
			D_{\mathrm v,i}\otimes L_{\mathrm v,j},\\
			\mathcal L\big(\ell_X,(\ell_Z,u_Z)\big)
			&=\sum_{\mathrm v,i,j}\lambda_{\mathrm v,i,j}
			L_{\mathrm v,i}\otimes D_{\mathrm v,j}.
		\end{align*}
