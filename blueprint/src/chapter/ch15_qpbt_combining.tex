...
\begin{lemma}[Formalization support: the restored defect of the paired-line
	...
	\leanok
	\uses{lem:qld-comm-line-cons, lem:pasting,
	...
	strategy pass with error $\eps$, let a joint point family have consistency
	error $\delta_Q$, and let $\mu$ be the probability that the sampled $X$
	direction is nonzero. For every directed opposite placement, the consistency
	defect between the joint point family and the evaluated POVM
...
\begin{proof}
	...
	the retained mass $\mu$; this gives the first summand. The discarded
	event has probability at most $1/q$ and contributes a defect at most one,
	so restoring the original law costs at most $1/(2q)$.
\end{proof}
...
\begin{proof}
	...
	Lemma~\ref{lem:combined-line-restored-defect} bounds the defect of the
...
\begin{example}[The paired-line construction given a joint point family]
	...
	\uses{lem:combined-line-measurement-consistency,
		def:concrete-paired-line-measurement,
		lem:concrete-paired-line-degree-support}
	This formalization-only auxiliary statement displays its point family as an
	explicit hypothesis and is therefore not a formalization of
	Lemma~\ref{lem:qld-xz-lines}, whose statement quantifies the point family
	existentially. Fix a polynomially small function $\delta_Q(\eps)$. There is
	a function $\delta_P(\eps,md/q)=\poly(\eps,md/q)$ such that for every
	projective strategy passing with error $\eps$ and every joint point family
	with consistency error $\delta_Q(\eps)$, the POVM $T^{\ell_X,\ell_Z}$ of
	Definition~\ref{def:concrete-paired-line-measurement} has the two axis-degree
	properties and consistency defect at most $\delta_P(\eps,md/q)$ with that
	point family.
	The linked declaration is quarantined by its conditional name: it is recorded
	here, in a separate formalization-support node that displays the point-family
	assumption, and Lemma~\ref{lem:qld-xz-lines} links neither it nor any
	statement-level formalization mark.
\end{example}
\begin{proof}
	\leanok
	\uses{lem:combined-line-measurement-consistency,
	...
	consistency bound, assembled on the explicitly given point family.
\end{proof}
...
\begin{lemma}[Formalization support: the paired subline overlap]
	...
	\leanok
	\uses{lem:qld-sublines, lem:claim-17-1, lem:claim-17-2, lem:claim-17-3,
		def:combine-map}
	...
	distribution, which remain separate and uncertified. See
	\cite{gap:qpbt_combined-lines-error-term}.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:qld-sublines, def:combine-map}
...
\begin{lemma}[Formalization support: the extended-line measurement of a subline
	...
	\leanok
	\uses{lem:qld-sublines, def:combine-map, def:consistency,
		def:ideg-deg-polynomials}
...
\begin{proof}
	\leanok
	\uses{lem:qld-4-10, lem:qld-sublines, lem:combined-lines-given-points,
		lem:subline-joint-overlap, lem:extended-line-measurement-support,
		thm:qld-combining-polynomial-bound}
	The formal construction conditions the directly indexed subline law on
	...
	paired line family, so that construction enters here as a formalization
	auxiliary with its point family already fixed, not as a certification of
	Lemma~\ref{lem:qld-xz-lines}. The two properties of
	...
	Applying it to the source statement remains conditional
