...
\begin{remark}[Normalization of the distance in the linearity conclusion]
	...
	The conclusion of Theorem~\ref{thm:linearity} bounds the mean squared
	...
	there is read as the state-dependent operator distance, and the one
	carried by the expansion displayed above and by the corresponding Lean
	statement. The quotation in \cite{Ji2020MIPStar} does not assert it: there
	...
	claiming an exact one. The calculation behind the
	bound in \cite[Section~3.1]{Natarajan2017Linearity} computes the squared
	distance between the binary projective measurements associated with the
	...
	distance, and the bound $\delta$ for the mean squared operator distance
	stated above is refuted by a four-point counterexample
	...
	Thus the displayed bound in Theorem~\ref{thm:linearity} is not the
	conclusion established by the Fourier argument; that conclusion is stated
	as Lemma~\ref{lem:boolean-representation-stability}.
\end{remark}
...
\begin{proof}
	\leanok
	\uses{prop:fourier-fact-vector, lem:boolean-characters}
...
\begin{lemma}[Formalization support for the binary-measurement distance]
	...
	$P_S^\pm = (\Id \pm S)/2$ for the effects of the associated binary
	projective measurements, and set
...
\begin{lemma}[Formalization support for Boolean representation stability]
	...
	\leanok
	\uses{def:povm-conventions, lem:fourier-square-measurement,
		lem:naimark-helper}
	This is not a named statement of \cite{Natarajan2017Linearity}; it records
	...
	therefore the bound $2\delta$, not the bound $\delta$ displayed in
	Theorem~\ref{thm:linearity}; the constant $2$ separating the two is the
	normalization recorded in
	Remark~\ref{rem:linearity-distance-normalization}. The present statement is
	consequently not a proof of the displayed form of
	Theorem~\ref{thm:linearity}, which remains unmarked, and it is separate
	from the source-facing statement repair.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:binary-measurement-distance, lem:blr-defect-identity,
		lem:boolean-fourier-transform, lem:fourier-square-measurement,
		lem:naimark-helper}
