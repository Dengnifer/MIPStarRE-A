...
\begin{lemma}[Formalization support: conditioning and restoring line defects]
	...
	Let $\mu$ be the probability that the first line in a pair of independent
	line-point samples has nonzero direction. For arbitrary complete
	measurements on directed opposite placements, conditioning on this event
...
\begin{proof}
	...
	bounds. Identify its sandwich with the concrete paired-line measurement,
...
\begin{definition}[Conditional extended-line construction]
	...
	$(f_X,f_Z)\mapsto h$. The positive fiber mass makes $D_{\mid\ell}$ a
	probability distribution, and finite disintegration gives
	\[
		\mathbb E_{\ell}\mathbb E_{D_{\mid\ell}}[F]=\mathbb E_D[F]
	\]
	for every real-valued function $F$ on supported subline triples.
\end{definition}

\begin{lemma}[Formalization support: conditional extended-line identities]
	...
	each of its subline fibers has positive mass. The selected affine data satisfy
	the projection identities in
...
\begin{proof}
	...
	\uses{def:qld-conditional-extended-line-measurement,
		def:direct-subline-law, def:combine-map}
	Positivity follows from the explicit direct-line sampler. The evaluation
	identities follow by substituting the compatible affine data in the
	polynomial combination formula.
\end{proof}
...
\uses{lem:combined-line-measurement-consistency, def:direct-subline-law,
	  ...
	  lem:qld-extended-line-overlap-comparison, lem:qld-4-10,
	  lem:qld-sublines, thm:qld-direct-line-real-overlap,
...
\begin{theorem}[Quarter powers and square-root deficits]
	...
	\leanok
	For every $x\in\R$, the real quarter power $x^{1/4}$ is nonnegative.
	If $m\geq1$ and $\delta_P,\delta_Q\geq0$, then
	\[
	 \sqrt{2\sqrt{4m^2\delta_P}+2\sqrt{4\delta_Q}}
	 \leq2\sqrt m\bigl(\delta_P^{1/4}+\delta_Q^{1/4}\bigr).
	\]
...
\begin{proof}
	\leanok
	Separate the two nonnegative summands under the outer square root, simplify
	the square roots of $4m^2\delta_P$ and $4\delta_Q$, and compare the remaining
	factor $2$ with $2\sqrt m$.
\end{proof}
