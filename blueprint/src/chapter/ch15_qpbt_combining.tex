...
Polynomial absorption and the independent unit defect bound give the result.
\end{proof}
...
\begin{proof}
\leanok
\uses{lem:combined-line-measurement-consistency, def:direct-subline-law,
  thm:qld-direct-line-real-overlap}
Condition the directly indexed subline law on its extended line and average
the combined polynomial outcomes of the constructed $X$-$Z$-$X$ POVM.
The real-part overlap estimates on both opposite placements give the bound
$m\cdot\poly(\eps,md/q)$, with the unit consistency bound handling large
errors. Axis degree support follows from the affine combination formula.
This proves the stated directly indexed auxiliary. Its comparison with the
printed source formulation remains subject to the separate question-law and
completed-answer issues in \cite{gap:qpbt_ld-dimension-divisibility} and
\cite{gap:qpbt_combined-lines-error-term}.
\end{proof}
...
\begin{lemma}[Restriction and completion of combined polynomial outcomes]
	...
	This is formalization support for the restriction and completion in the
	proof of Lemma~\ref{lem:qld-4-7}, not its quantitative conclusion.
	For admissible $(q,m,d)$, the map
	$(g_X,g_Z)\mapsto\mathrm{combine}_{g_X,g_Z}$ is injective on bounded
	individual-degree polynomials, and its evaluation is
	$\alpha g_X(x)+\beta g_Z(z)$. Given a projective measurement
	$\{T_g\}$ on polynomials in $2m+2$ variables, define
	$R_{g_X,g_Z}=T_{\mathrm{combine}_{g_X,g_Z}}$, and let $\mathcal G$
	be the image of the combining map. Then $R$ is a projective
	sub-measurement, with the exact operator identity
	\[
		\Id-\sum_{g_X,g_Z}R_{g_X,g_Z}=\sum_{g\notin\mathcal G}T_g.
	\]
	For any specified pair $p_0$, adding this missing operator to $R_{p_0}$
	and leaving all other effects unchanged gives a projective measurement.
	No bound on the state mass of the missing operator, or on consistency
	with point measurements, is asserted here.
\end{lemma}
\begin{proof}
	\leanok
	\uses{def:combine-map, lem:sandwich-postprocess-projective}
...
$3(\sqrt{p(x)+Cn g(x,y)}+y)$ is at most $n h(x,y)$ for a
