...
not the source law $D_{\mathrm v,i}$ of
Definition~\ref{def:ith-restricted-line}, which carries the same zero-based
index; that source restriction is the one written $D_{\mathrm v,i+1}$ in the
one-based notation of \cite{Ji2020MIPStar}. Identifying its push-forward with
...
\begin{definition}[Directly indexed sub-line laws]
	...
	\leanok
	\uses{def:ith-restricted-line-refined, def:line-representative,
		def:combine-map}
	...
	is the mixture obtained by sampling $(\ell_X,x)$ from the seed-bearing
	restriction $\widetilde D_{\mathrm v,i}$ of
	Definition~\ref{def:ith-restricted-line-refined} and independently
	$\ell_Z$ from the line marginal of $\widetilde D_{\mathrm v,j}$, and
	retaining $(\ell_X,\ell_Z,x)$.
	...
	marginal identities over the seed-bearing restrictions, with the same
	mixture weights for both projected marginals; they impose no
	joint conditional independence of the two projected points.
	Identification with the source restrictions $D_{\mathrm v,i}$, and with
	the source law $D$, is a separate obligation;
...
\begin{lemma}[Formalization support: the concrete X deficit under the directly indexed law]
	...
	\leanok
	\uses{lem:concrete-line-X-marginal,
...
\begin{proof}
	...
	is a mixture of seed-bearing restricted laws. Error inflation and
	$m\ge1$ give the bound.
\end{proof}
...
mixtures over the seed-bearing restrictions $\widetilde D_{\mathrm v,i}$ of
Definition~\ref{def:ith-restricted-line-refined}, with the same mixture
weights for both projected marginals, as in Property~2 of
Definition~\ref{def:direct-subline-law}, and axis-line closure. No equality
with the source restrictions $D_{\mathrm v,i}$, or with the seed-indexed law
$D$, is assumed or established.
