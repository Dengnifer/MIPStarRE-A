...
\begin{proof}
	\leanok
	\uses{lem:qld-comm-cons, lem:cancellation, lem:ortho-explicit-constant,
		fact:triangle, lem:symmetric-equivalents-transfer,
		def:expanded-point-measurement, def:povm-distance}
	We give the field-valued construction used in the formal proof.
	For each player and each pair $(x,z)$, form the sandwich POVM
	\[
		R^{x,z}_{a,b}
		= \hat{M}^{(\mathrm{Point},Z),z}_{b}
		  \hat{M}^{(\mathrm{Point},X),x}_{a}
		...
		\qquad (a,b)\in\F_q^2.
	\]
	Positivity and completeness follow from projectivity of the point
	measurements. Fourier inversion and character orthogonality transfer
	the commutation estimate of Lemma~\ref{lem:qld-comm-cons} to these
	field-valued effects: Parseval's identity expresses the sum of squared
	commutator norms over $(a,b)$ as the average over trace characters,
	without a factor depending on $q$. Thus $R$ is within squared distance
	$O(\sqrt\eps)$ of the ordered $Z$-then-$X$ product. Moving the point
	factors between opposite placements using their self-consistency bounds
	gives an average consistency defect $O(\eps+\sqrt\eps)$ for the two
	sandwich POVMs.

	Apply Lemma~\ref{lem:ortho-explicit-constant} separately for each $(x,z)$
	on each player's original expanded local space. Jensen's inequality for
	$t\mapsto t^{1/4}$ bounds the average squared distance from the resulting
	projective family $Q$ to $R$ by
	$O((\eps+\sqrt\eps)^{1/4})$. The crosswise exchange of the two EPR pairs
	fixes the expanded state and transfers this bound to the other two
	placements by Lemma~\ref{lem:symmetric-equivalents-transfer}.
	Insert $R$ and the ordered products in the squared-distance triangle
	inequality. Their commutation and cross-placement consistency estimates
	give all three asserted comparisons. For $0\le\eps\le1$ the total error
	is at most $K\eps^{1/8}$ for a universal $K$; for $\eps\ge1$, the uniform
	squared-distance bound $4$ is absorbed by increasing $K$.
	This constructs $Q$ on the stated spaces. The source's alternative route
	through binary refinements and quantum linearity requires the additional
	ancillary-space absorption discussed in Remark~\ref{rem:linearity-import};
	that route is not used here.
\end{proof}
...
\begin{lemma}[Formalization support: replacing one factor of an overlap]
	...
	For a probability distribution and a normalized state, a complete measurement
	weights the difference of the real parts of two operator overlaps by at most
	the square root of their state-dependent squared distance.
\end{lemma}
\begin{proof}
	\leanok
...
\begin{lemma}[Formalization support: a uniform point of the extended space]
	...
	\uses{lem:qld-sublines, def:line-point-dist, def:line-representative,
		def:combine-map}
	For a subline law with the directly indexed extended-line marginal, a
	uniform affine parameter on the sampled extended line gives a uniform
...
\begin{proof}
	...
	sides. Completeness bounds the first factor by $1$, and
	$\sum_{f_X,f_Z \,:\, f_X(x)=a,\ f_Z(z)=b}
	T^{\ell_X,\ell_Z}_{f_X,f_Z} \le \Id$
	for every $x, z, a, b$ bounds the outcome weights in the second factor.
	When $\ell$ is
	...
	in $\F_q^m$; hence
	$\E_{x,z} \sum_{a,b}
	\bra{\hat\psi} \Id \ot (W^{x,z}_{a,b})^*W^{x,z}_{a,b} \ket{\hat\psi}
	\le 4\delta_Q$
	by Lemma~\ref{lem:qld-4-10-same-placement}. The resulting error
	$2\sqrt{\delta_Q}$ has the asserted order. The adjoint square is needed
	because the ordered product need not be Hermitian.
	This source-law argument is not certified by the formal real-part
	estimate below; the scalar and distributional differences are recorded
	in \cite{gap:qpbt_combined-lines-error-term}.
\end{proof}

\begin{lemma}[Formalization support: real-part replacement under the direct law]
	\label{lem:claim-17-1-direct-real}
	\lean{MIPStarRE.QPBT.subline_replace_by_ordered_product}
	\leanok
	\uses{lem:qld-4-10, lem:qld-xz-lines, lem:qld-sublines-uniform-point}
	Let $Q,T$ satisfy the joint-point and paired-line conclusions at errors
	$\delta_Q,\delta_P$. Let $D_{\mathrm{dir}}$ be a probability law satisfying
	the incidence and separate marginal-mixture properties of
	Lemma~\ref{lem:qld-sublines}, together with affine compatibility for
	Definition~\ref{def:combine-map} and closure of axis lines under projection.
	Replace its extended-line marginal by the directly indexed law.
	Write $\mathbb E_{\mathrm{dir}}$
	for sampling this law and then a uniform affine parameter on the extended
	line. Put
	\begin{align*}
		A_{\mathrm{dir}}&=\mathbb E_{\mathrm{dir}}\sum_{f_X,f_Z}
			\langle\hat\psi,(T_{f_X,f_Z}\otimes Q_{f_X(x),f_Z(z)})
			\hat\psi\rangle,\\
		B_{\mathrm{dir}}&=\mathbb E_{\mathrm{dir}}\sum_{f_X,f_Z}
			\langle\hat\psi,(T_{f_X,f_Z}\otimes M^Z_{f_Z(z)}M^X_{f_X(x)})
			\hat\psi\rangle.
	\end{align*}
	Here the placements are $\mathsf{A}\mathsf{A}'$ and
	$\mathsf{B}\mathsf{A}''$, and $M^W$ denotes the expanded point effect.
	All point effects at an undefined line evaluation are zero; equivalently,
	the point measurements are extended by a zero effect at the extra outcome
	$\bot$. There is a universal $C>0$ such that
	\[
		|\Re A_{\mathrm{dir}}-\Re B_{\mathrm{dir}}|\le C\sqrt{\delta_Q}.
	\]
	This is a Lean-only statement. Neither a complex-modulus estimate nor
	transport to the source law is asserted.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:qld-4-10-same-placement, lem:overlap-gap-distance,
		lem:qld-sublines-uniform-point}
	Postprocess $T$ by the two completed evaluations. Apply the real-part
	overlap estimate, then the uniform $(x,z)$ marginal identity. The
	same-placement squared distance is at most $4\delta_Q$, so $C=2$
	suffices. The squared distance uses $W^*W$, not $W^2$.
\end{proof}
...
\begin{lemma}[Formalization support: the $Z$ correlation under the direct law]
	\label{lem:claim-17-3-direct-real}
	\lean{MIPStarRE.QPBT.subline_Z_term_near_one}
	\leanok
	\uses{lem:claim-17-1-direct-real}
	Use the hypotheses, placements, and completed evaluations of
	Lemma~\ref{lem:claim-17-1-direct-real}, and put
	\[
	...
	\]
	The opposite-placed positive effects commute, so this correlation is real.
	The statement still uses the directly indexed law; its transport to the
	source law of Lemma~\ref{lem:claim-17-3} remains open, as discussed in
	\cite{gap:qpbt_combined-lines-error-term}.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:restricted-line-mixture-bounds, lem:qld-4-10-same-placement,
		lem:overlap-gap-distance}
	Use the separate $Z$ marginal mixture to reduce to restricted product
	distributions. The paired-line consistency bound and the same-placement
	ordered-product bound give
	\[
		0\le 1-\Re Z_{\mathrm{dir}}
		\le 2\sqrt{4m^2\delta_P}+2\sqrt{4\delta_Q}.
	\]
	Positivity and completeness also give $0\le\Re Z_{\mathrm{dir}}\le1$.
	Hence the square of its deficit is at most its deficit. Taking square
	roots and using $m\ge1$ proves the assertion with $C=2$; the additional
	$\eps^{1/4}$ term is nonnegative.
\end{proof}
