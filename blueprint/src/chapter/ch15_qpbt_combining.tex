...
source construction of the combined point measurement proceeds in three steps: the
...
assembled by Fourier averaging. The proof of Lemma~\ref{lem:qld-4-10}
below instead forms the sandwich directly with field-valued outcomes and
orthonormalizes it on the original expanded local spaces. The source's
linearity route is retained in Remark~\ref{rem:combined-points-linearity-route}.
...
\begin{proof}
	...
	Remark~\ref{rem:linearity-import} and discussed in
	Remark~\ref{rem:combined-points-linearity-route}.
\end{proof}
...
\begin{remark}[Obligations of the linearity-theorem import]
	...
	load-bearing for the source's linearity proof of Lemma~\ref{lem:qld-4-10}:
	the error is
	...
	the source's proof of Lemma~\ref{lem:qld-4-10} only through averages over $u$.
	...
	conversion obligations, explained in
	Remark~\ref{rem:combined-points-linearity-route}: the theorem is
	...
	assumes present without construction. This is an obligation of the source's
	linearity route, not of the completed field-valued sandwich proof of
	Lemma~\ref{lem:qld-4-10}, which introduces no additional ancillary space.
\end{remark}
...
\begin{proof}
	\leanok
	\uses{lem:qld-comm-cons, lem:expanded-point-measurement-properties,
		prop:fourier-fact-scalar, lem:ortho-explicit-constant, fact:triangle,
		def:expanded-state, def:EPR}
	We give a field-valued variant of the sandwich construction in
	\cite{Ji2020MIPStar} (source label \texttt{lem:qld-4-10}); see
	\path{docs/paper-gaps/qpbt_combined-points-direct.tex} for details.
	Write $X_a=\hat{M}^{(\mathrm{Point},X),x}_a$ and
	$Z_b=\hat{M}^{(\mathrm{Point},Z),z}_b$. Fourier inversion expresses
	$[X_a,Z_b]$ as the Fourier transform of the commutators of the expanded
	observables. Orthogonality of the additive characters and Parseval's
	identity transfer Lemma~\ref{lem:qld-comm-cons} to the field-valued
	effects without a factor depending on $q$:
	\[
		\E_{x,z}\sum_{a,b}\|[X_a,Z_b]\ket{\hat\psi}\|^2
		\leq C_1\sqrt{\eps}
	\]
	on every placement, for a universal $C_1$. Form the POVM
	$R^{x,z}_{a,b}=Z_bX_aZ_b$ with $a,b\in\F_q$. The identity
	$R_{a,b}-Z_bX_a=Z_b[X_a,Z_b]$ and contraction by $Z_b$ give the
	same bound on its squared distance from the ordered product.

	For opposite placements, denoted by subscripts $1,2$, put
	$D^X_a=(X_a)_1-(X_a)_2$, $D^Z_b=(Z_b)_1-(Z_b)_2$, and
	$W_b=(Z_b)_1(Z_b)_2$. Projectivity and completeness give the exact
	consistency-defect identity
	\[
		1-\sum_{a,b}\bra{\hat\psi}(R_{a,b})_1(R_{a,b})_2\ket{\hat\psi}
		=\frac12\sum_b\|D^Z_b\ket{\hat\psi}\|^2
		+\frac12\sum_{a,b}\|D^X_aW_b\ket{\hat\psi}\|^2.
	\]
	Commuting $D^X_a$ past $W_b$ leaves the two placed commutators.
	Their bounds and point self-consistency bound the average defect by
	$C_0(\eps+\sqrt{\eps})$. Apply
	Lemma~\ref{lem:ortho-explicit-constant} for each $(x,z)$ on each player
	space. Averaging its fourth-root error by concavity produces projective
	$Q^{x,z}$ on the original expanded local spaces, with squared distance
	$O((\eps+\sqrt{\eps})^{1/4})$ from $R^{x,z}$.

	The ancillary involution exchanging $\mathsf{A}'$ with $\mathsf{B}''$
	and $\mathsf{A}''$ with $\mathsf{B}'$ fixes $\ket{\hat\psi}$: it
	exchanges the identical EPR factors and reverses each pair. Thus it
	transfers these same-side distances to the other bipartition without
	assuming symmetry of the strategy. It is not the permutation
	$U_\sigma U_\theta$ of Lemma~\ref{lem:symmetric-equivalents-transfer}.
	Transporting the factors of an ordered product one at a time by point
	self-consistency, then applying the squared-distance triangle inequality,
	gives both ordered-product comparisons and self-consistency of $Q$ on
	all opposite placements. For $0\leq\eps\leq1$ every term is bounded by
	$K\eps^{1/8}$ for a universal $K\geq4$; for $\eps>1$ the bound $4$
	for square-summable families suffices. Take $\delta_Q(\eps)=K\eps^{1/8}$.
\end{proof}

\begin{remark}[The source's linearity route for combined points]
	\label{rem:combined-points-linearity-route}
	The following is the source argument, whose common-ancilla step remains
	open as described in Remark~\ref{rem:linearity-import}; it is not used by
	the completed proof above. Throughout this argument every intermediate
	error is a $\poly(\eps)$ function
	...
	To finish this route, the ancilla register and its state would have to be
	absorbed into zero-state padding of the strategy state, uniformly in
	$(x,z)$, as recorded in Remark~\ref{rem:linearity-import}. After that
	unproved step, one would define
	$\hat{Q}^{x,z}_{a,b}
...
\begin{lemma}[Completed point-to-line marginal distances]
	\label{lem:combined-point-line-marginal-distance}
	\lean{MIPStarRE.QPBT.exists_combinedPoints_line_marginal_distance_le}
	\leanok
	\uses{lem:qld-4-10, def:expanded-line-measurement, lem:qld-comm-line-cons,
		def:line-point-dist, def:povm-distance, def:symmetric-equivalents}
	There is a universal $C\geq1$ with the following property. Let
	$(q,m,d)$ be admissible, let $\eps,\delta_Q\in\mathbb R$, and let
	$\mathcal S$ be a projective strategy of value at least $1-\eps$.
	Suppose joint point measurements $\{\hat Q^{x,z}_{a,b}\}$ are supplied
	on both expanded local spaces, are projective, and satisfy the three
	comparisons of Lemma~\ref{lem:qld-4-10} on all opposite placements,
	with each averaged squared distance at most $\delta_Q$.
	Sample $(\ell_X,x)$ and $(\ell_Z,z)$ independently from the line-point
	distribution, and denote their product distribution by $\mu$.
	Use the completed alphabet $\Omega=\F_q\cup\{\bot\}$ and write
	\[
		Q^{X,x,z}_a=\sum_b\hat Q^{x,z}_{a,b},\qquad
		Q^{Z,x,z}_b=\sum_a\hat Q^{x,z}_{a,b},\qquad
		Q^{X,x,z}_{\bot}=Q^{Z,x,z}_{\bot}=0.
	\]
	Let $E^{W,\ell,u}_c$ be the evaluated expanded-line effect of
	Lemma~\ref{lem:qld-comm-line-cons}: for $c\in\F_q$ it sums the effects
	with $f(u)=c$, and for $c=\bot$ it sums those without an evaluation at
	$u$. For every directed opposite placement pair $(p_1,p_2)$ of
	Definition~\ref{def:symmetric-equivalents} and each $W\in\{X,Z\}$,
	putting $u_X=x$ and $u_Z=z$, one has
	\[
		\E_{\mu}\sum_{c\in\Omega}
		\big\|\big((Q^{W,x,z}_c)_{p_1}
		-(E^{W,\ell_W,u_W}_c)_{p_2}\big)\ket{\hat\psi}\big\|^2
		\leq 8\delta_Q+C\bigl(\eps+\delta_{\mathrm{Line}}(\eps)\bigr),
	\]
	where $\delta_{\mathrm{Line}}(\eps)=\sqrt{\eps}$. This is a quantified
	intermediate estimate for \cite{Ji2020MIPStar}
	(source label \texttt{eq:pasting-q1}); it does not assert existence of
	the combined line measurement.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:qld-4-10, fact:add-a-proj, lem:cool-closeness-fact,
		lem:qld-comm-cons, lem:qld-comm-line-cons, fact:triangle,
		def:line-point-dist}
	The marginal contraction calculation following
	\eqref{eq:qld-q-cons-m-hat-zx} bounds the squared distance from the
	$X$ marginal to the opposite $X$ point measurement by $4\delta_Q$;
	the $Z$ marginal uses \eqref{eq:qld-q-cons-m-hat-xz} with the same
	bound. Adding a zero effect at $\bot$ preserves these distances.
	The point marginal of the line-point distribution is uniform, so
	independent line-point sampling preserves both bounds. On placement
	$p_2$, compare the evaluated line measurement first with the point
	measurement on $p_1$ using Lemma~\ref{lem:qld-comm-line-cons}, and then
	with the point measurement on $p_2$ using Lemma~\ref{lem:qld-comm-cons}.
	The squared-distance triangle inequality gives
	$C_0(\eps+\delta_{\mathrm{Line}}(\eps))$. A second application yields
	$2(4\delta_Q)+2C_0(\eps+\delta_{\mathrm{Line}}(\eps))$; take $C=2C_0$.
\end{proof}

\begin{lemma}[Completed point-to-line marginal consistency defects]
	\label{lem:combined-point-line-marginal-defect}
	\lean{MIPStarRE.QPBT.exists_combinedPoints_line_marginal_defect_le}
	\leanok
	\uses{lem:combined-point-line-marginal-distance, def:consistency}
	There is a universal $C\geq1$ such that, for every projective strategy
	and supplied projective joint point family satisfying the hypotheses of
	Lemma~\ref{lem:combined-point-line-marginal-distance}, the following
	holds. With its independent sampling distribution $\mu$, completed
	alphabet $\Omega$, and completed families $Q^W,E^W$, for all directed
	opposite placements $(p_1,p_2)$ and both $W\in\{X,Z\}$,
	\[
		\E_{\mu}\sum_{\substack{c,c'\in\Omega\\c\ne c'}}
		\Re\bra{\hat\psi}(Q^{W,x,z}_c)_{p_1}
		(E^{W,\ell_W,u_W}_{c'})_{p_2}\ket{\hat\psi}
		\leq8\delta_Q+C\bigl(\eps+\delta_{\mathrm{Line}}(\eps)\bigr).
	\]
	Equivalently, first extend the joint outcome pair $(a,b)$ to
	$\Omega^2$ with zero effects on pairs containing $\bot$, and then take
	its coordinate marginals. These are the same completed $Q^X,Q^Z$.
	This is the consistency-defect form of the two comparisons in the
	source display \texttt{eq:pasting-q1}.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:combined-point-line-marginal-distance,
		lem:sandwich-postprocess-compose, lem:sandwich-postprocess-projective,
		def:expanded-line-measurement, fact:agreement}
	Composition of postprocessings identifies the two descriptions of each
	completed marginal. Projectivity follows from projectivity of the supplied
	joint point measurements and of the constructed expanded-line measurements;
	it is preserved by evaluation, completion, marginalization, and placement.
	For projective families the consistency defect is bounded by their squared
	state-dependent distance. Apply
	Lemma~\ref{lem:combined-point-line-marginal-distance} to each coordinate.
\end{proof}

\begin{lemma}[Point self-consistency under line-point sampling]
	\label{lem:combined-point-self-consistency-line-point}
	...
	This formalization-only auxiliary records the second pasting hypothesis
	used in \cite{Ji2020MIPStar}, following the source display
	\texttt{eq:pasting-q1}. Let $(q,m,d)$ be admissible, let
	$\eps,\delta_Q\in\mathbb R$, and let $\mathcal S$ be a projective
	strategy of value at least $1-\eps$. Suppose joint point measurements
	$\{\hat Q^{x,z}_{a,b}\}_{a,b\in\F_q}$ are supplied on both expanded
	local spaces, are projective, and satisfy the three comparisons of
	Lemma~\ref{lem:qld-4-10} on all opposite placements, each with averaged
	squared distance at most $\delta_Q$. Sample $(\ell_X,x)$ and
	$(\ell_Z,z)$ independently from the line-point distribution, and write
	$\mu=D_{\mathrm{Line}}\times D_{\mathrm{Line}}$ for their law.
	For every directed opposite placement pair $(p_1,p_2)$ of
	Definition~\ref{def:symmetric-equivalents},
	\[
		\E_{\mu}\sum_{\substack{(a,b),(c,d)\in\F_q^2\\(a,b)\ne(c,d)}}
		\Re\bra{\hat\psi}(\hat Q^{x,z}_{a,b})_{p_1}
		(\hat Q^{x,z}_{c,d})_{p_2}\ket{\hat\psi}
		\leq\delta_Q.
	\]
\end{lemma}
\begin{proof}
	\leanok
	\uses{fact:agreement, lem:alnf, lem:dlnf, def:line-point-dist}
	Placement preserves projectivity, so Fact~\ref{fact:agreement} bounds
	the consistency defect by the supplied squared-distance bound for
	uniform point pairs. Lemmas~\ref{lem:alnf} and~\ref{lem:dlnf} give
	uniform point marginals for both components of the line-point
	distribution, hence also for their equal mixture. Independence of the
	two samples makes $(x,z)$ a uniform point pair. Since the integrand
	depends only on $(x,z)$, this change of sampling preserves the defect.
\end{proof}
...
\begin{proof}
	\uses{def:expanded-line-measurement, lem:qld-4-10, fact:add-a-proj,
		...
		def:symmetric-equivalents, lem:combined-point-line-marginal-defect,
	...
	together with the analogous relation for $Z$. The completed-alphabet forms
	of these intermediate comparisons, with explicit error
	$8\delta_Q+C(\eps+\delta_{\mathrm{Line}}(\eps))$, are proved in
	Lemmas~\ref{lem:combined-point-line-marginal-distance}
	and~\ref{lem:combined-point-line-marginal-defect}. For the joint line
	measurement, the POVM construction and degree support are formalized;
	the pasting argument establishing its consistency remains unformalized.
	The source's conclusion then follows
	...
	and by \eqref{eq:qld-q-self-cons}, whose consistency-defect form under
	the required sampling is proved in
	Lemma~\ref{lem:combined-point-self-consistency-line-point}.
