...
\begin{lemma}[Formalization support: swapping paired answer coordinates]
	\label{lem:paired-postprocessing-swap}
	\lean{MIPStarRE.QPBT.postprocess_pair_swap_effect}
	\leanok
	\uses{def:povm-conventions}
	Let $M$ be a finite measurement and let $e$ map its answers to pairs. If
	$e^{\mathrm{op}}$ is obtained by exchanging the two coordinates, then the
	effect of the postprocessed measurement $M[e^{\mathrm{op}}]$ at $(b,a)$ is
	the effect of $M[e]$ at $(a,b)$.
\end{lemma}
\begin{proof}
	\leanok
	\uses{def:povm-conventions}
	Both effects are the sum of the effects of $M$ over the same fiber of $e$.
\end{proof}

\begin{lemma}[Formalization support: invariance under outcome equivalence]
	\label{lem:consistency-defect-outcome-equivalence}
	\lean{MIPStarRE.QPBT.consistencyDefect_outcome_equiv}
	\leanok
	\uses{def:consistency}
	Simultaneously relabeling the common finite answer set of two operator
	families by a bijection leaves their consistency defect unchanged.
\end{lemma}
\begin{proof}
	\leanok
	\uses{def:consistency}
	Change variables by the bijection in both finite answer sums.
\end{proof}

\begin{lemma}[Formalization support: the conditioned paired-line defect]
	\label{lem:combined-line-conditioned-defect}
	\lean{MIPStarRE.QPBT.exists_combinedLine_conditioned_defect_le}
	\leanok
	\uses{lem:qld-4-10,
		def:concrete-paired-line-measurement, def:consistency}
	There are a constant $c\ge1$ and a function $f(x,y)=\poly(x,y)$ with the
	following property. Let a projective strategy pass with error $\eps$, and
	let complete projective joint point measurements satisfy all the conclusions
	of Lemma~\ref{lem:qld-4-10} with error $\delta_Q$. Write $\mu$ for the
	probability that the sampled $X$ direction is nonzero. For every directed
	opposite placement, under the line-point law conditioned on that event, the
	consistency defect between the completed joint point measurement and the
	evaluated concrete paired-line POVM, with both answer pairs ordered $Z$ then
	$X$, is at most
	\[
		f\!\left(\frac{md}{q},
...
\begin{lemma}[Formalization support: restoration of the paired-line law]
	\label{lem:combined-line-mass-restoration}
	\lean{MIPStarRE.QPBT.ProjectiveSetting.combinedLineMeasurement_consistency_le_conditioned}
	\leanok
	\uses{lem:qld-4-10,
		def:concrete-paired-line-measurement, def:line-point-dist, def:consistency}
	Let $\mu$ be the probability that the sampled $X$ direction is nonzero.
	Let complete projective joint point measurements satisfy all the conclusions
	of Lemma~\ref{lem:qld-4-10}. For every directed opposite placement, the
	consistency defect under the unconditioned product line-point law between
	the completed joint point measurement on the first placement and the
	evaluated concrete paired-line POVM on the second, with answers ordered $X$
	then $Z$, is at most $\mu$ times the corresponding defect under the
	conditioned law, plus $1/(2q)$.
\end{lemma}

\begin{lemma}[Formalization support: symmetry across opposite placements]
	\label{lem:consistency-defect-opposite-symmetry}
	\lean{MIPStarRE.QPBT.consistencyDefect_opposite_symm}
	\leanok
	\uses{def:expanded-state, def:consistency}
	For two complete measurement families placed on opposite registers of the
	expanded state, exchanging the two placements leaves their consistency
	defect unchanged.
\end{lemma}
\begin{proof}
	\leanok
	\uses{def:expanded-state, def:consistency}
	Effects on opposite registers commute. Exchange the two finite answer sums
	and commute the two effects in every off-diagonal term.
\end{proof}

\begin{lemma}[Formalization support: the restored defect of the paired-line
	measurement]
	\label{lem:combined-line-restored-defect}
	\lean{MIPStarRE.QPBT.exists_combinedLine_restored_defect_le}
	\leanok
	\uses{lem:qld-4-10,
		def:concrete-paired-line-measurement, def:consistency}
	This formalization-only auxiliary statement isolates the pasting estimate
	behind Lemma~\ref{lem:combined-line-measurement-consistency} before the
	polynomial error functions are composed. There are a constant $c\ge1$ and a
	function $f(x,y)=\poly(x,y)$ with the following property. Let a projective
	strategy pass with error $\eps$, and let complete projective joint point
	measurements satisfy, for every directed opposite placement, self-consistency
	and consistency with both ordered products $\hat M^X_a(x)\hat M^Z_b(z)$ and
	$\hat M^Z_b(z)\hat M^X_a(x)$, each with squared-distance error at most
	$\delta_Q$; these are all the conclusions of Lemma~\ref{lem:qld-4-10}. Let
	$\mu$ be the probability that the sampled $X$ direction is nonzero. For every
	directed opposite placement, the consistency defect between the joint point
	measurements and the evaluated POVM
	$T^{\ell_X,\ell_Z}$, averaged over two independent line-point samples of the
	unconditioned law, is at most
	\[
		\mu\, f\!\left(\frac{md}{q},\,
			\frac{8\delta_Q+c\,(\eps+\sqrt{\eps})}{\mu}\right)
		+\frac{1}{2q}.
	\]
	Both sides of the comparison use the completed answer alphabet described in
	Lemma~\ref{lem:combined-line-measurement-consistency}.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:combined-line-conditioned-defect,
		lem:paired-postprocessing-swap,
		lem:consistency-defect-outcome-equivalence,
		lem:combined-line-mass-restoration,
		def:concrete-paired-line-measurement}
	Condition the line-point law on the sampled $X$ direction being nonzero.
	On that event the point-to-line comparisons of
	Lemma~\ref{lem:qld-comm-line-cons} supply the collision and consistency
	inputs of Lemma~\ref{lem:pasting} for the sandwich
	$T^{\ell_X,\ell_Z}$, with the conditional error obtained by dividing by
	the retained mass $\mu$; this gives the first summand. The axis component
	has no zero directions, while the diagonal component has zero-direction
	mass at most $1/q$. Since the two components have equal weight, the
	discarded event has probability at most $1/(2q)$. Its defect is at most
	one, so restoring the original law costs at most $1/(2q)$.
\end{proof}
...
\begin{lemma}[Consistency of the constructed paired-line measurement]
	...
	\leanok
	\uses{lem:qld-4-10, def:concrete-paired-line-measurement, def:consistency}
	This is a formalization-only auxiliary theorem. Fix a polynomially small
...
\begin{proof}
	\leanok
	\uses{lem:combined-line-restored-defect,
		lem:consistency-defect-opposite-symmetry,
		lem:consistency-defect-integrand-le-one,
		thm:qld-conditioned-polynomial-bound}
	First exchange the two opposite placements using
	Lemma~\ref{lem:consistency-defect-opposite-symmetry}.
	Lemma~\ref{lem:combined-line-restored-defect} then bounds the defect of the
	two measurements under the unconditioned line-point law by
	$\mu f(md/q,(8\delta_Q(\eps)+c(\eps+\sqrt{\eps}))/\mu)+md/q$, since the
	retained mass satisfies $\mu\in[1/2,1]$ and $1/(2q)\le md/q$.
	Theorem~\ref{thm:qld-conditioned-polynomial-bound} converts that
	expression into a single function $\delta_P(\eps,md/q)=\poly(\eps,md/q)$
	of the two arguments, chosen after $\delta_Q$ and its polynomial bound.
	The unit bound on the consistency defect of normalized POVMs covers the
	parameter ranges in which the composed expression exceeds one.
\end{proof}
...
\begin{lemma}[Formalization support: the ordered product on one placement]
	...
	The same bound holds for the $X$-then-$Z$ product.
\end{lemma}
\begin{proof}
	\leanok
...
\begin{lemma}[Existence of a directly indexed sub-line witness]
	...
	compatibility, separate projected-marginal, and axis-closure properties of
	Definition~\ref{def:direct-subline-law}.
\end{lemma}
\begin{proof}
	\leanok
	\uses{def:direct-subline-law}
	Use the directly indexed sampling law. Its extended-line marginal is the
	direct line marginal; the two projected point laws are the stated mixtures,
	and the sampled coordinates give the incidence, affine compatibility, and
	axis-closure properties on every point of the support.
\end{proof}
...
\begin{lemma}[Auxiliary estimates for the paired subline overlap]
	...
	\uses{def:paired-subline-overlap, def:direct-subline-law,
		lem:qld-4-10-same-placement, lem:overlap-gap-distance,
		lem:concrete-X-deficit-direct, lem:qld-sublines-uniform-point}
...
\begin{proof}
	...
	product. The second uses the separate $X$ marginal of the directly indexed law
	and the concrete $X$-overlap deficit. The third uses the separate $Z$ marginal,
	positivity, and point self-consistency.
\end{proof}
...
\begin{lemma}[Formalization support: the extended-line measurement of a subline
	witness]
	\label{lem:extended-line-measurement-support}
	...
	\leanok
	\uses{def:extended-line-measurement, def:paired-subline-overlap,
		def:combine-map, def:consistency, def:ideg-deg-polynomials}
	Let a subline witness and a paired line family be given, and let the extended
	line measurement be the conditional average of the paired-line POVMs along
	the sublines of an extended line, pushed forward by the combining map. Two
	properties hold. First, on the completed answer alphabet its consistency
	defect with the zero-extended joint point family, taken over the directly
	indexed extended line-point law, is at most $1$ minus the average paired
	subline overlap, for both directed opposite placements. Second, on an
	extended axis line it has no outcome of degree exceeding $d$ in the axis
	variable.
\end{lemma}
\begin{proof}
	\leanok
	\uses{def:extended-line-measurement, def:paired-subline-overlap,
		def:combine-map}
	The first property is the identification of a consistency defect with one
	minus the corresponding overlap for a probability law and normalized POVMs,
	applied to the extended measurement and the zero-extended point family. The
	second follows because the axis closure of the subline witness keeps each
	conditioned sample on an axis line, where the degree support of the paired
	line POVM and the combining map give the bound.
\end{proof}
...
\begin{lemma}[Formalization support for Lemma~\ref{lem:qld-4-13}]
	...
	controlled function
	$\delta_{\combine}:\mathbb R^2\to\mathbb R$ such that, for every admissible
	...
	\[
		C m\,\delta_{\combine}\!\left(\eps,\frac{md}{q}\right).
	\]
...
\begin{proof}
	\leanok
	\uses{lem:qld-4-10, lem:combined-lines-given-points,
		lem:direct-subline-witness-exists, lem:subline-joint-overlap,
		lem:extended-line-measurement-support,
		lem:consistency-defect-integrand-le-one,
		thm:qld-combining-polynomial-bound}
	The construction conditions the directly indexed subline law on
	the extended line and averages the combined polynomial outcomes of the
	constructed $X$-$Z$-$X$ line measurement. The point family is the one
	supplied by Lemma~\ref{lem:qld-4-10}; applied to it, the conditional
	construction of~\ref{lem:combined-lines-given-points} produces the
	paired line family. Because its point family is already fixed, this use does
	not prove Lemma~\ref{lem:qld-xz-lines}.
	Lemma~\ref{lem:direct-subline-witness-exists} provides the directly indexed
	sub-line law. The two properties of
	Lemma~\ref{lem:extended-line-measurement-support} give the degree support
	and reduce both consistency displays to the paired subline overlap, which
	Lemma~\ref{lem:subline-joint-overlap} estimates for both opposite
	placements; Theorem~\ref{thm:qld-combining-polynomial-bound} collects the
	resulting quarter powers into the form $m\cdot\poly(\eps,md/q)$, with the
	unit bound on consistency handling large errors. This proves the stated
	directly indexed auxiliary.
	It does not yield the source statement until the question space and
	distribution are transported and the completed answer comparison, including
	zero-direction singleton lines, is converted to the source sum over
	$a \in \F_q$; these obligations are recorded in
