...
\begin{remark}[Normalization of the distance in the linearity conclusion]
	...
	mathematical, not a claim of a formalized asymptotic theorem. For the
	four-point family of Lemma~\ref{lem:linearity-four-point-obstruction},
	the optimal operator error is $1$ at correlation $1/4$ and $\delta=3/4$;
	the identity representation attains it. In contrast, the exact
	distance/defect identity of Lemma~\ref{lem:boolean-representation-stability}
	gives error $3/2$ for the Fourier--Naimark construction. Thus the sharp
	universal coefficient does not assert optimality of that construction for
	each input. The corrected estimates suffice for the fiberwise application
	in Remark~\ref{rem:linearity-import}: a mean squared defect $\eta$ gives
	correlation $1-\eta/2$ and output error at most $\eta$. The combined-point
	proof below uses the independent field-valued construction instead.
	This closes the normalization discrepancy as a documented
	constant/convention deviation, not a proof of the printed operator bound;
	the two printed propositions remain unasserted and refuted. The separate
	state-preserving padding obligation remains open.
\end{remark}
