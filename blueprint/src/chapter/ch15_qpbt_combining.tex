...
\begin{remark}[Obligations of the linearity-theorem import]
	...
	cited statement in four respects: it inserts the restriction
	...
	\cite{Natarajan2017Linearity}; it appends the identity
	$\mathcal{L}^u = \Id$ for $u = 0$; and it reports the bound $\delta$
	of \cite[equation~(3)]{Natarajan2017Linearity}, which bounds the mean
	squared distance between the associated binary projective
	measurements, as a bound on the mean squared operator distance,
	thereby losing the factor of two recorded in
	Remark~\ref{rem:linearity-distance-normalization}. None of the four is
	reproduced here: the restriction is dropped, the pointwise closeness
	is not used, the appended identity is instead derived from the
	conclusion, since exact linearity at $u = u' = 0$ gives
	$(\mathcal{L}^0)^2 = \mathcal{L}^0$, and an idempotent unitary equals
	the identity, and Theorem~\ref{thm:linearity} carries the corrected
	operator bound $2\delta$. These four discrepancies, together with the
	common-extension obligation recorded below, are analyzed in
	\cite{gap:qpbt_linearity-theorem-quotation}. The import carries two
...
\begin{lemma}[Formalization support for Boolean representation stability]
	...
	\leanok
	\uses{def:povm-conventions, lem:binary-measurement-distance,
		lem:blr-defect-identity, lem:fourier-square-measurement,
		lem:naimark-helper, lem:naimark-rounding}
	This is not a named statement of \cite{Natarajan2017Linearity}; it records
