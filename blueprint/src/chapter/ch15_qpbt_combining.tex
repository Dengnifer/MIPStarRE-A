...
% probability bookkeeping of the proof-only conditioning step above.  It is a
...
\begin{lemma}[Formalization support: conditioning and restoring a finite average]
	...
	\leanok
	Let $\mu$ be a finitely supported probability distribution on a sample
	type, let $G$ be a decidable event of positive mass $p = \mu(G) > 0$, and
	write $\mu \mid G$ for the distribution $\mu$ conditioned on $G$. Then,
	...
	\end{equation}
	The mass $\mu(G^{c})$ discarded by the conditioning is retained in full in
	\eqref{eq:restricted-average-restoration}; no term is dropped.
\end{lemma}
\begin{proof}
	\leanok
...
% and Lemma~\ref{lem:direct-subline-law-exists}; transport to this statement
...
\begin{lemma}[Formalization support: existence of a directly indexed sub-line law]
	\label{lem:direct-subline-law-exists}
	\lean{MIPStarRE.QPBT.exists_subLineWitness}
	\leanok
	\uses{def:direct-subline-law, def:ith-restricted-line,
		def:line-representative, def:combine-map}
	For every admissible $(q,m,d)$ there exists a directly indexed sub-line law
	$D_{\mathrm{dir}}$ as in Definition~\ref{def:direct-subline-law}.

	This is not Lemma~\ref{lem:qld-sublines}, and it does not certify it. The
	extended line-point law is the directly indexed one, so the statement
	carries no divisibility hypothesis, and its mixture property asserts only
	the two separate projected point marginals, not a joint conditional law.
	Transport to the source carrier and law remains open; see
	\cite{gap:qpbt_ld-dimension-divisibility}.
\end{lemma}
\begin{proof}
	\leanok
	\uses{def:direct-subline-law, def:ith-restricted-line,
		def:line-representative, def:combine-map}
	Take $D_{\mathrm{dir}}$ to be the law of the sampling procedure of
	Definition~\ref{def:direct-subline-law}: sample the extended line-point
	pair, read off its kind, coordinate index and direction, and emit the two
	source lines obtained by projecting onto the $x$ and $z$ blocks. The
	extended-line marginal is the sampled law by construction. Incidence, the
	affine compatibility maps, and axis closure are checked pointwise on the
	support after recovering the auxiliary sample, and the two projected point
	marginals are the corresponding mixtures.
\end{proof}
...
\begin{proof}
	...
	\uses{def:consistency, def:ld-game, def:admissible-size,
		def:ideg-deg-polynomials}
	Comparing the coefficient outcomes with their evaluations at a uniform
	parameter costs $\mathrm{deg}/q$, the probability that two distinct
	polynomials of degree at most $\mathrm{deg}$ agree at a uniform point.
	...
	parameter average of the same integrand.
\end{proof}
...
\begin{proof}
	...
	the coefficient-collision loss $d/q + (2m+2)d/q \le 5md/q$. The value of
	the strategy is one minus one ninth of the branch total; capping the
	rejection probability by one and applying the capped scalar estimate for
	$\mathrm{env}$ gives the stated bound. Naimark dilation preserves the
