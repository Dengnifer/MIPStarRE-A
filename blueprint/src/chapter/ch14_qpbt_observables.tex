...
\begin{proof}
	...
	two answers leaves it unchanged.
\end{proof}
...
\end{lemma}
\begin{proof}
	\leanok \uses{lem:qld-win-implications, lem:povm-to-obs, fact:data-processing, fact:agreement, lem:commutation-analysis, thm:ms-rigidity, fact:add-a-proj, def:strategy-observables}
...
\begin{proof}
	...
	The last two items admit a second route, which is the one carried out in
	the formalization and which establishes them in the opposite order.
	Item~\ref{eq:qld-comm-line-pt-cons2} is proved first, directly from the
	exact overlap identity
	$\bra{\hat{\psi}} \hat{M}^{(\mathrm{Line},W),\ell}_{[\mathrm{eval}_u(\cdot)=a]}
	\ot \hat{M}^{(\mathrm{Point},W),u}_{a} \ket{\hat{\psi}}
	= \bra{\psi} M^{(\mathrm{Line},W),\ell}_{[\mathrm{eval}_u(\cdot)=a]}
	\ot M^{(\mathrm{Point},W),u}_{a} \ket{\psi}$
	between the expanded and the strategy overlaps, which holds because the
	ancillary projectors $\tau^{W,\ell}$ and $\tau^{W,u}$ agree exactly on
	the EPR pairs, together with item~1 of Lemma~\ref{fact:agreement}; this
	gives item~\ref{eq:qld-comm-line-pt-cons2} with error linear in $\eps$.
	Item~\ref{eq:qld-comm-line-pt-cons} then follows from it by the
	projective refinement of Lemma~\ref{fact:add-a-proj2}, again with error
	linear in $\eps$, so that the elementary sub-measurement inequality
	displayed above is not needed. The three linear estimates are weakened to
	the common error $\delta_{\mathrm{Line}}(\eps) = \sqrt{\eps}$, using
	the universal bound $4$ on the distance between two complete measurements
	on a normalized state, because the statement of the lemma requires a
	single error function; for $\eps \leq 1$ the linear estimates are the
	stronger ones.

	The symmetric equivalents are obtained without the transfer of
	Lemma~\ref{lem:symmetric-equivalents-transfer}: all three items are
	proved separately on each of the four register pairs
	$\mathsf{A}\mathsf{A}'$, $\mathsf{B}\mathsf{A}''$,
	$\mathsf{B}\mathsf{B}'$ and $\mathsf{A}\mathsf{B}''$ of
	Definition~\ref{def:symmetric-equivalents}, so no symmetry hypothesis on
