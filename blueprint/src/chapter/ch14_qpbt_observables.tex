...
\begin{remark}[Winning implications and evaluation conventions]
	\label{rem:qld-win-implications-typos}
	The algebraic corrections and the evaluation convention have different
	status. In
	...
	remaining parameters by making the final error in
	Theorem~\ref{thm:pauli} trivial. That argument does not establish the
	unrestricted statement of the intermediate lemma. The proof above uses
	the exact probability
	$\Pr[\gamma\ne0]=\frac12(1-q^{-1})^{m+1}$ from
	Lemma~\ref{fact:omega-anticomm-prob}. Since $m\mid q$ gives
	$1\leq m\leq q$, the commuting and anticommuting events have probabilities
	at least $1/2$ and $1/16$, respectively. The degree-zero counterexample
	to a printed probability bound in
	Remark~\ref{rem:omega-anticomm-prob-correction} is outside the source
	domain $m,d\geq1$; it does not refute the winning lemma there.
	The substantive difference is that the source states item~\ref{enu:qld-ld} of
	...
	for POVMs. The adopted win predicate requires $f(t)=a$ for every parameter
	$t$ representing the sampled point. On a singleton line this accepts exactly
	answers inducing the constant function $a$ on $\F_q$; it does not require
	a constant coefficient representative. For example, the allowed answer
	$f(T)=T$ at a zero-direction diagonal question with
	$(q,m,d)=(2,1,1)$ has no evaluation at its point. This illustrates a local
	evaluation obstruction, not a high-success counterexample to the lemma.
	Item~\ref{enu:qld-ld} completes the line family by the effect of all
	non-evaluating answers and the point family by the zero $\bot$ effect.
	The added line effect contributes to both the consistency defect and
	the squared-distance sum. Its control uses the universal acceptance
	clause; it cannot be justified by a typographical correction alone.
	The complete typed measurements first send malformed answers of the
	common answer alphabet to fixed valid answers, as in
	Definition~\ref{def:strategy-observables}; their mass is controlled by
	rejection. This is separate from adjoining the evaluation outcome $\bot$.

	The established estimates suffice for the subsequent construction with
	these conventions. The observable proof uses the value-only
	anticommutator estimate of
	Lemma~\ref{lem:qld-ms-anticommutator-original-state}, not the unrestricted
	printed Magic Square extraction assertion. The signed commutation estimate
	yields the commuting expanded observables of Lemma~\ref{lem:qld-comm-cons},
	and the completed low-degree comparison supplies
	Lemma~\ref{lem:qld-comm-line-cons}. The field-valued combined points and
	completed line measurements feed the directly indexed extended-line
	construction of Lemma~\ref{lem:qld-4-13-established}, followed by the
	global polynomial-pair measurement and Pauli extraction. The source's
	extended seed-indexed law and its printed extended-line error are separate
	questions; this implication argument does not establish them.
	The verdict is a documented deviation: the existing marks on the winning
	lemmas concern their stated completed families and the specified game.
	They do not establish identification with the printed source's
	field-only evaluation and acceptance convention on zero directions. The
	detailed comparison and the limits of the known counterexamples are in
