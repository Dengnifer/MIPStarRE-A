...
\begin{proof}
	...
	$0$ with probability exactly $\frac12$: the first vector $b_0$ of the
	fixed self-dual normal basis satisfies $\tr(b_0) = 1$ by
	Lemma~\ref{lem:one}, the trace
	(Definition~\ref{def:binary-representation}) is additive, and $\F_q$
	has characteristic $2$, so the translation $c \mapsto c + b_0$ is an
