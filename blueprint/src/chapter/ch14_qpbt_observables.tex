...
\begin{definition}[The strategy--Pauli product measurement on a line]
	...
	\leanok
	\uses{def:expanded-line-measurement, def:ideg-deg-polynomials, fact:data-processing}
	...
	Its image under the addition map $(f',f'') \mapsto f'+f''$ is the
	measurement of Definition~\ref{def:expanded-line-measurement}:
	\[
		\hat{M}^{(\mathrm{Line},W),\ell}_f
		\,=\, \sum_{f'+f''=f} \hat{N}^{(\mathrm{Line},W),\ell}_{(f',f'')}\,.
	\]
	The two measurements have different outcome sets and are related by this
	coarse-graining, which is the data-processing presentation
	(Fact~\ref{fact:data-processing}) used in the first item of
