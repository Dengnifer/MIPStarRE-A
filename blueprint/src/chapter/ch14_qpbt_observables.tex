...
\begin{lemma}[Probability of anticommuting tuples]
	...
	The probability that a tuple $\omega = (u_X,u_Z,r_X,r_Z)$ sampled
	uniformly at random from $(\F_q^m)^2 \times (\F_q)^2$ is anticommuting is
	at least
	\[
		(1 - q^{-m}) \cdot (1 - q^{-1}) \cdot \Big(1 - \frac{md}{q}\Big) \cdot \frac{1}{2}
		\,\geq\, \Big(1 - \frac{3md}{q}\Big) \cdot \frac{1}{2}\,.
	\]
	The probability that $\omega$ is commuting is also at least
	$\big(1 - \frac{3md}{q}\big) \cdot \frac{1}{2}$.
\end{lemma}
\begin{proof}
	\uses{lem:schwartz-zippel, def:admissible-size, def:ideg-deg-polynomials, def:low-degree-encoding}
	The vector $\mathrm{ind}_m(u_X)$ vanishes if and only if $u_X = 0$, and
	for all $r_X, r_Z, u_Z$,
	\[
		\tr\big( (r_X \cdot \mathrm{ind}_m(u_X)) \cdot (r_Z \cdot \mathrm{ind}_m(u_Z)) \big)
		\,=\, \tr\big( (r_X r_Z) \cdot g_{\mathrm{ind}_m(u_X)}(u_Z) \big)\,,
	\]
	where, for $u_X \neq 0$, the polynomial
	$g_{\mathrm{ind}_m(u_X)} \in \mathrm{ideg}_{d,m}(\F_q)$ is nonzero of
	total degree at most $md$. Condition on the event that $u_X \neq 0$,
	$r_X \neq 0$ and $g_{\mathrm{ind}_m(u_X)}(u_Z) \neq 0$; by the
	Schwartz--Zippel lemma (Lemma~\ref{lem:schwartz-zippel}) and the
	independence of $u_X, r_X, u_Z$, this event has probability at least
	$(1-q^{-m})(1-q^{-1})(1 - md/q)$. Conditioned on this event, the product
	$(r_X r_Z) \cdot g_{\mathrm{ind}_m(u_X)}(u_Z)$ is a uniformly random
	element of $\F_q$ over the choice of $r_Z$. Since $q$ is admissible, the
	trace of an element of $\F_q$ equals the coordinate sum of its binary
	representation (see Chapter~\ref{chap:qpbt-algebra}), so a uniform
	element of $\F_q$ has trace $0$ with probability exactly $1/2$. Both
	bounds follow.
\end{proof}
...
\begin{lemma}
	...
	$A = \E_{x} \alpha_x A^x$ and $B = \E_{x} \alpha_x B^x$; that is,
	$\big\| (A - B) \ket{\psi} \big\|^2 \leq \delta$.
\end{lemma}
\begin{proof}
	Expand
	\[
		...
		\,\leq\, \delta\,,
	\]
	using the triangle inequality followed by Jensen's inequality.
\end{proof}
...
\end{lemma}
\begin{proof}
	\uses{fact:omega-anticomm-prob, fact:agreement, def:pauli-question-distribution, def:pauli-win-predicate}
	...
	inside the Pauli basis test. For
	items~\ref{enu:qld-comm},~\ref{enu:qld-comm-cons}
	and~\ref{enu:qld-ms-cons}, Lemma~\ref{fact:omega-anticomm-prob} shows
	that a uniformly sampled tuple $\omega$ is commuting (respectively,
	anticommuting) with probability at least
	$(1 - 3md/q) \cdot \frac12 \geq \frac14$ provided $6md \leq q$, so the
	conditioning increases the failure probabilities by at most a constant
	factor. The assumption $6md \leq q$ holds without loss of generality:
	the error function $\delta_{\mathrm{qld}}$ of Theorem~\ref{thm:pauli}
	is monotonically increasing in the universal constant $a$, so $a \geq 6$
	may be assumed, and if $6md > q$ then
	$\delta_{\mathrm{qld}} \geq a\,md/q \geq 1$ and the conclusion of
	Theorem~\ref{thm:pauli} holds trivially. Item~\ref{enu:qld-ms} follows
	because the Magic Square check is executed with constant probability
	over the choice of a pair of questions. Finally, that all approximations
