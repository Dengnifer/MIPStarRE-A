...
\begin{lemma}[Pasting two consistent measurements; Fact 4.35 in \cite{Natarajan2018NEEXP}]\label{lem:pasting}
	...
	\end{equation}
	the first symmetric equivalent of that comparison in the sense of Definition~\ref{def:symmetric-equivalents}. The source states the lemma for symmetric strategies, for which \eqref{eq:pasting-1-sym} is the second comparison in \eqref{eq:pasting-1} read from the other player; it is the only consequence of that convention used by the imported proof, and the statement without it is unattested (\cite{gap:qpbt_pasting-product-error}). Define a family of measurements $\{J^x_{g_1,\,g_2}\}$ by
...
\begin{lemma}[Formalization support for Lemma~\ref{lem:pasting}]
	...
	\leanok
	\uses{def:pasting-tensor-placement, lem:pasting-pinched-reduction}
...
\begin{lemma}[Formalization support for Lemma~\ref{lem:pasting}]
	...
	Then the defect of the pasted family satisfies
	\[
		2\delta + \sqrt{K}, \qquad
