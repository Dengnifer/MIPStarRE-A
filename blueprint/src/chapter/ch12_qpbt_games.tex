...
\begin{lemma}[Formalization support for extensionality of finite distributions]\label{lem:distribution-ext-support-weight}
	\lean{MIPStarRE.QPBT.Distribution.ext_of_support_of_weight}
	\leanok
	Two finite distributions with the same support and the same weight at every point are equal.
\end{lemma}
\begin{proof}
	\leanok
	A finite distribution is determined by its finite support and weight function, so equality of these data gives equality of the distributions.
\end{proof}

\begin{lemma}[Formalization support for composed push-forwards]\label{lem:distribution-map-map}
	\lean{MIPStarRE.QPBT.Distribution.map_map}
	\leanok
	\uses{lem:distribution-ext-support-weight}
	Let $\mu$ be a finite distribution on $A$, and let $e\colon A \to B$ and $f\colon B \to C$. Then $f_*(e_*\mu) = (f \circ e)_*\mu$.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:distribution-ext-support-weight}
	The two distributions have the same image support. At each $c \in C$, both weights are the sum of $\mu(a)$ over the fiber $\{a : f(e(a))=c\}$.
\end{proof}

\begin{lemma}[Formalization support for uniform weights]\label{lem:uniform-distribution-weight}
	\lean{MIPStarRE.QPBT.uniformDistribution_weight_apply}
	\leanok
	If $A$ is a finite nonempty set, then every $a \in A$ has weight $1/|A|$ under the uniform distribution on $A$.
\end{lemma}
\begin{proof}
	\leanok
	This is the defining weight of the uniform distribution on a finite nonempty set.
\end{proof}

\begin{lemma}[Formalization support for uniform push-forwards with equicardinal fibers]\label{lem:uniform-distribution-map-equi-cardinal-fibers}
	\lean{MIPStarRE.QPBT.uniformDistribution_map_of_card_fiber}
	\leanok
	\uses{lem:distribution-ext-support-weight, lem:uniform-distribution-weight}
	Let $A$ and $B$ be finite nonempty sets and let $e\colon A \to B$. If every fiber $e^{-1}(b)$ has the same cardinality $c$, then the push-forward of the uniform distribution on $A$ along $e$ is the uniform distribution on $B$.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:distribution-ext-support-weight, lem:uniform-distribution-weight}
	The common fiber cardinality is positive and $|A|=c|B|$. Hence every $b \in B$ lies in the image and receives weight $c/|A|=1/|B|$.
\end{proof}

\begin{lemma}[Formalization support for uniform push-forwards along equivalences]\label{lem:uniform-distribution-map-equivalence}
	\lean{MIPStarRE.QPBT.uniformDistribution_map_equiv}
	\leanok
	\uses{lem:uniform-distribution-map-equi-cardinal-fibers}
	Every equivalence $e\colon A \simeq B$ between finite nonempty sets sends the uniform distribution on $A$ to the uniform distribution on $B$.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:uniform-distribution-map-equi-cardinal-fibers}
	Every fiber of an equivalence has exactly one element, so the claim follows from the equicardinal-fiber criterion.
\end{proof}

\begin{lemma}[Formalization support for the first marginal of a uniform product]\label{lem:uniform-product-first-marginal}
	\lean{MIPStarRE.QPBT.uniformDistribution_map_fst}
	\leanok
	\uses{lem:uniform-distribution-map-equi-cardinal-fibers}
	For finite nonempty sets $A$ and $B$, the first marginal of the uniform distribution on $A \times B$ is the uniform distribution on $A$.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:uniform-distribution-map-equi-cardinal-fibers}
	Every fiber of the first projection has cardinality $|B|$, so the equicardinal-fiber criterion applies.
\end{proof}

\begin{lemma}[Formalization support for the second marginal of a uniform product]\label{lem:uniform-product-second-marginal}
	\lean{MIPStarRE.QPBT.uniformDistribution_map_snd}
	\leanok
	\uses{lem:uniform-distribution-map-equi-cardinal-fibers}
	For finite nonempty sets $A$ and $B$, the second marginal of the uniform distribution on $A \times B$ is the uniform distribution on $B$.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:uniform-distribution-map-equi-cardinal-fibers}
	Every fiber of the second projection has cardinality $|A|$, so the equicardinal-fiber criterion applies.
\end{proof}

\begin{lemma}[Formalization support for binding uniform push-forwards]\label{lem:uniform-bind-map}
	\lean{MIPStarRE.QPBT.bind_uniformDistribution_map}
	\leanok
	\uses{lem:distribution-ext-support-weight, lem:uniform-distribution-weight}
	Let $A$ and $B$ be finite nonempty sets and let $g\colon A \to B \to C$. Sampling $a$ uniformly from $A$, then $b$ uniformly from $B$, and returning $g(a,b)$ gives the push-forward of the uniform distribution on $A \times B$ under $(a,b) \mapsto g(a,b)$.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:distribution-ext-support-weight, lem:uniform-distribution-weight}
	Both distributions are supported on the image of $g$. The weight of $c \in C$ is the number of pairs $(a,b)$ with $g(a,b)=c$, divided by $|A||B|$, on both sides.
\end{proof}
