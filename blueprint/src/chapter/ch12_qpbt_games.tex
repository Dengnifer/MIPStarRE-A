...
\begin{lemma}[Approximate symmetrization for symmetric games]
	\label{lem:symmetric-strat-approx}
	...
	This formalization-only node is not a named statement of the paper; it is the approximate form that the proof of Lemma~\ref{lem:symmetric-strat} establishes, isolated in Remark~\ref{rem:symmetric-strat-limit}, with the source's nonempty answer alphabet displayed as an explicit hypothesis. Let $\mathfrak G = (\mathcal X, \mathcal A, \mu, D)$ be a symmetric game whose answer alphabet $\mathcal A$ is nonempty, and suppose $\operatorname{val}^*(\mathfrak G) = 1-\eps$ for some $\eps \ge 0$. Then for every $\eps' > \eps$ there exists a symmetric and projective strategy $\mathcal S = (\ket{\psi}, M)$ with
	\[
...
\begin{proof}
	...
	Fix $\eps' > \eps$. Since $\mathcal A$ is nonempty, the one-dimensional strategy answering a fixed answer deterministically is a strategy for $\mathfrak G$, so the set of strategy values is nonempty; it is bounded above by $1$, so $\operatorname{val}^*(\mathfrak G)$ is the least upper bound of that set. The supremum property therefore supplies a strategy $\mathcal S'$ with $\operatorname{val}^*(\mathfrak G, \mathcal S') > 1-\eps'$, which is the step the source proof states as ``by definition''. Applying Lemma~\ref{lem:symmetric-strat-given-strategy} to $\mathcal S'$ produces a symmetric projective strategy of exactly the value of $\mathcal S'$, hence at least $1-\eps'$. The nonemptiness hypothesis is load-bearing in the formalization: with an empty answer alphabet no strategy exists at all, while the value is still defined as the totalized supremum of the empty set, so the conclusion fails there. No attainment step is used and none is claimed at $\eps' = \eps$; that step is the open source gap of Lemma~\ref{lem:symmetric-strat}, documented in \cite{gap:qpbt_symmetrization-attainment}.
\end{proof}
