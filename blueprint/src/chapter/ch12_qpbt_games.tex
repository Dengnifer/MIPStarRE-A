...
\begin{lemma}[Cross move with independent opposite-register effects]
	...
	\begin{align*}
	 &\left|\sum_{a,g}\langle\psi|\Id\ot G_gP_aG_gP_a|\psi\rangle
	   -\sum_{g,a}\langle\psi|H_g\ot P_aG_gP_a|\psi\rangle\right|\\
	 &\qquad\leq
...
\begin{proof}
	...
	\uses{lem:pasting-measurement, lem:pasting-coarse-commutator,
		lem:pasting-explicit-error}
	Apply the explicit defect bound and dominate it by the universal additive
	polynomial error. The full conditional argument is recorded in
...
\begin{lemma}[Formalization support for Lemma~\ref{lem:pasting}]
	...
	\[
		\Bigl| \sum_{a_1, g_2} \bra{\psi} \Id \ot G_{g_2} P_{a_1} G_{g_2} P_{a_1}
			\ket{\psi} - \sum_{g_2} \bra{\psi} G_{g_2} \ot \sum_{a_1} P_{a_1}
...
\begin{lemma}[Formalization support for Lemma~\ref{lem:pasting}]
	...
	\[
		...
		= \sum_{a_1, g_2} \bigl\| (\Id \ot P_{a_1} G_{g_2}) \ket{\psi} \bigr\|^2
			+ \sum_{a_1} \bra{\psi} \Id \ot P_{a_1}^2 \ket{\psi}
			- 2 \sum_{a_1, g_2} \bra{\psi} \Id \ot G_{g_2} P_{a_1} G_{g_2} P_{a_1}
				\ket{\psi}\,.
	\]
\end{lemma}
\begin{proof}
...
\begin{lemma}[Formalization support for Lemma~\ref{lem:pasting}]
	...
	\leanok
	\uses{def:pasting-tensor-placement, def:povm-distance}
	Let $G_{g_2}$ be a projective measurement on the second tensor factor
	depending on a question, let $Q_{a_2}$ be its coarse-graining along the
	evaluation map at the sampled second question, let $P_{a_1}$ be a
	measurement, and let $\ket{\psi}$ be a unit vector. Assume the conditional
	collision bound with parameter $\eta$, and assume that the averaged coarse
