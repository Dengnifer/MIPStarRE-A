...
\begin{definition}[Bracket notation for post-processed measurements]\label{def:bracket}
	...
	Let $\{M^x_a\}_{a \in \mathcal A}$ be a family of POVMs indexed by $x \in \mathcal X$, and let $f \colon \mathcal A \to \mathcal B$ be an arbitrary function. We write $\bigl\{ M^x_{[f(\cdot)=b]} \bigr\}$ for the POVM derived from $\{M^x_a\}$ by applying the function $f$ to the outcome before returning it:
	\[
...
\begin{proof}
	\uses{thm:naimark}
	By definition, for any $\eps' > \eps$ there exists a strategy $\mathcal S' = (\ket{\psi'}, A, B)$ with $\operatorname{val}^*(\mathfrak G, \mathcal S') \ge 1-\eps'$. Using Naimark dilation (Theorem~\ref{thm:naimark}, in the completed form recorded in Remark~\ref{rem:naimark-for-games}) we may assume without loss of generality that $\ket{\psi'} \in \C^d_{A'} \ot \C^d_{B'}$ for some integer $d$ and that every $A^x$ and $B^x$ is a projective measurement. Let
	\[
	...
	\]
	and let each player measure $M$ on their pair of registers. Because the decision predicate $D$ of the symmetric game treats the two players symmetrically, the resulting symmetric projective strategy $\mathcal S = (\ket{\psi}, M)$ satisfies $\operatorname{val}^*(\mathfrak G, \mathcal S) = \operatorname{val}^*(\mathfrak G, \mathcal S')$. The passage from a value of at least $1-\eps'$ for every $\eps' > \eps$ to a value of at least $1-\eps$ is discussed in Remark~\ref{rem:symmetric-strat-limit}.
\end{proof}

\begin{remark}[Limit step in the symmetrization lemma]\label{rem:symmetric-strat-limit}
	The construction in the proof of Lemma~\ref{lem:symmetric-strat} produces, for every $\eps' > \eps$, a symmetric projective strategy of value at least $1-\eps'$; the source asserts the conclusion for $\eps$ itself without giving the limiting argument. Since the dilated dimension $d$ may grow as $\eps' \downarrow \eps$, this final step is not immediate; it requires an additional compactness or limit argument, or else the conclusion should be read as: for every $\eps' > \eps$ there exists a symmetric projective strategy of value at least $1-\eps'$.
\end{remark}
...
\begin{definition}[Distance between POVMs]\label{def:povm-distance}
	\uses{def:povm-conventions, def:state-distance}
	Let $\mathcal X$ be a finite set and $\mu$ a distribution on $\mathcal X$. Let $\ket{\psi} \in \mathcal H$ be a quantum state, and for all $x \in \mathcal X$ let $\{M^x_a\}$ and $\{N^x_a\}$ be two POVMs on $\mathcal H$. We say that \emph{$\{M^x_a\}$ and $\{N^x_a\}$ are $\delta$-close on state $\ket{\psi}$ and under distribution $\mu$} if
	\[
		\E_{x \sim \mu} \sum_a \lVert (M^x_a - N^x_a)\, \ket{\psi} \rVert^2 \le O(\delta)\,,
	\]
	and we write $M^x_a \approx_\delta N^x_a$ to denote this when the state $\ket{\psi}$ and the distribution $\mu$ are clear from context. This distance is referred to as the \emph{state-dependent} distance.
\end{definition}
...
\end{lemma}
\begin{proof}
	Stated in the source without proof; these are Facts 4.13 and 4.14 of \cite{Natarajan2018NEEXP}. For the first item, expand each term $\lVert (A^x_a \ot \Id - \Id \ot B^x_a)\ket{\psi} \rVert^2$ and use $ (A^x_a)^2 \le A^x_a$, $(B^x_a)^2 \le B^x_a$ together with completeness of the two POVMs: summing over $a$ and averaging over $x$ leaves exactly twice the off-diagonal mass, which is $O(\delta)$ by hypothesis. For the converse items, write the off-diagonal mass $\sum_{a \ne b} \bra{\psi} A^x_a \ot B^x_b \ket{\psi}$ as an inner product involving $(A^x_a \ot \Id - \Id \ot B^x_a)\ket{\psi}$ and use projectivity of both families, or of one family combined with the Cauchy--Schwarz inequality; the latter route costs a square root in the error.
\end{proof}
...
\begin{proof}
	...
	\]
	Since $\sum_{g\,:\,g(x)=a} (S^x_g)^\dagger S^x_g \le \sum_g (S^x_g)^\dagger S^x_g \le \Id$, this is at most $\E_x \sum_a \lVert (A^x_a - B^x_a)\ket{\psi} \rVert^2 \le \delta$.
\end{proof}
...
\begin{proof}
	...
	\end{align*}
	where the last step uses $A^x_a \le \Id$. All expectations are taken according to $\mu$, and the last quantity is at most $\delta$ by assumption.
\end{proof}
...
\begin{lemma}[Sandwiched simultaneous measurement; modified from Fact 4.34 in \cite{Natarajan2018NEEXP}]\label{lem:ld-sandwich}
	...
	\[
		C^x_{g_1,\,g_2,\,\dots,\,g_k} = G^{k,\,x}_{g_1} \cdots G^{2,\,x}_{g_{k-1}}\; G^{1,\,x}_{g_k}\; G^{2,\,x}_{g_{k-1}} \cdots G^{k,\,x}_{g_1}\,.
	\]
...
\begin{remark}[Index pairing in the sandwich]\label{rem:ld-sandwich-indexing}
	As printed in the source, the sandwich in Lemma~\ref{lem:ld-sandwich} pairs the $i$-th measurement with the outcome $g_{k+1-i}$: the outermost factor $G^{k,\,x}$ carries the outcome $g_1$ and the innermost factor $G^{1,\,x}$ carries $g_k$. In Fact 4.34 of \cite{Natarajan2018NEEXP}, from which the lemma is imported, the natural pairing $G^{k,\,x}_{g_k} \cdots G^{1,\,x}_{g_1} \cdots G^{k,\,x}_{g_k}$ is used. The transposed indices appear to be a typographical slip in the source; the statement above records the source text as written, and any use of the lemma should confirm the intended pairing against \cite{Natarajan2018NEEXP}.
\end{remark}

\begin{lemma}[Pasting two consistent measurements; Fact 4.35 in \cite{Natarajan2018NEEXP}]\label{lem:pasting}
	\uses{def:consistency, def:bracket, def:povm-conventions}
	Let $\mathcal D$ be a distribution on $(x, y_1, y_2) \in \mathcal X \times \mathcal Y_1 \times \mathcal Y_2$. For $i \in \{1,2\}$ let $\mathcal G_i$ be a collection of functions $g_i \colon \mathcal Y_i \to \mathcal R_i$ and let $\{(G_i)^x_g\}_{g \in \mathcal G_i}$ be families of measurements such that $\{(G_2)^x_g\}_g$ is projective for every $x$. Suppose further that for every $(x, y_1)$ it holds that for $g_2 \ne g_2' \in \mathcal G_2$ the probability, on average over $y_2$ chosen from $\mathcal D$ conditioned on $(x, y_1)$, that $g_2(y_2) = g_2'(y_2)$ is at most $\eta$. Let $\{A^{x,y_1,y_2}_{a_1,\,a_2}\}$ be a family of projective measurements with outcomes $(a_1, a_2) \in \mathcal R_1 \times \mathcal R_2$ such that for $i \in \{1,2\}$,
...
\begin{definition}[Graph distribution]\label{def:graph-distribution}
	Let $G = (U, E)$ be an undirected graph with vertex set $U$ and edge set $E$. Edges in $E$ are written as multisets $\{u, v\}$ of two vertices; the case $u = v$ represents a self-loop. Suppose there are $m$ edges, $k$ of which are self-loops. Then the \emph{graph distribution $\mu_G$ of $G$} is the distribution over $U \times U$ such that for every $(u,v) \in U \times U$,
	\[
		\mu_G(u, v) =
		\begin{cases}
