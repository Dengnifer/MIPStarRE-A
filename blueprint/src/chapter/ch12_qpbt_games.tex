...
% The source statement at 06_nonlocal_games_and_mipstar.tex:94-99 assumes only the
% value of the game and therefore requires an unsupported attainment step.  The
% statement below is the value-preserving given-strategy form actually proved by
% the source construction; see gap:qpbt_symmetrization-attainment.
\begin{lemma}[Symmetrization of strategies for symmetric games]\label{lem:symmetric-strat}
		...
		Let $\mathfrak G = (\mathcal X, \mathcal A, \mu, D)$ be a symmetric game,
		let $\eps \in \R$, and let $\mathcal S' = (\ket{\psi'}, A, B)$ be a
		strategy such that
		$\operatorname{val}^*(\mathfrak G, \mathcal S') \ge 1-\eps$.
		Then there exists a symmetric and projective strategy
		$\mathcal S = (\ket{\psi}, M)$ such that
		\[
			\operatorname{val}^*(\mathfrak G, \mathcal S)
			= \operatorname{val}^*(\mathfrak G, \mathcal S') \ge 1-\eps.
		\]
		This is the corrected given-strategy form of the source lemma; the
		attainment discrepancy is documented in
		\cite{gap:qpbt_symmetrization-attainment}.
\end{lemma}
\begin{proof}
		\uses{thm:naimark}
		Using Naimark dilation (Theorem~\ref{thm:naimark}), each dilated
		sub-measurement being completed to a projective measurement by assigning
		to one fixed outcome the residual projector of its family, we may replace
		$\mathcal S'$ by a projective strategy of exactly the same value.  Write
		its state as $\ket{\psi'} \in \C^d_{A'} \ot \C^d_{B'}$.  Let
		\[
	...
	\]
		and let each player measure $M$ on their pair of registers.  Because the
		decision predicate treats both players symmetrically, the resulting
		symmetric projective strategy has exactly the value of $\mathcal S'$.
		This proves the statement without an attainment argument.
\end{proof}

\begin{remark}[Attainment in the symmetrization lemma]\label{rem:symmetric-strat-limit}
		The source states the stronger implication from
		$\operatorname{val}^*(\mathfrak G)=1-\eps$ to a strategy attaining that
		value.  Its proof first chooses strategies of value at least $1-\eps'$ for
		every $\eps'>\eps$, and the dimensions may grow as $\eps'\downarrow\eps$;
		it supplies no finite-dimensional limiting strategy.  The corrected
		Lemma~\ref{lem:symmetric-strat} instead starts from the strategy that the
		construction actually symmetrizes.  A strict lower bound on the game value
		still supplies such a strategy, so every source use remains valid.  The
		attainment gap and this correction are documented in
		\cite{gap:qpbt_symmetrization-attainment}.
\end{remark}
...
\begin{definition}[Typed conditionally linear distributions]\label{def:typed-cl-distributions}
	\lean{MIPStarRE.QPBT.typedCLDistribution, MIPStarRE.QPBT.ldQuestionDistribution_eq_typedCL, MIPStarRE.QPBT.pauliQuestionDistribution_eq_typedCL}
	\leanok
