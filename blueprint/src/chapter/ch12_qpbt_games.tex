...
\begin{lemma}[Formalization support for Lemma~\ref{lem:ld-sandwich}]
	...
	collision estimate in \cite[Fact~4.34]{Natarajan2018NEEXP}. The off-diagonal overlap of two
	codeword measurements is at most the averaged off-diagonal overlap of their
	evaluations plus $\eps$.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:sandwich-evaluated-diagonal-overlap,
		lem:distance-point-defect}
...
\begin{lemma}[Formalization support for Lemma~\ref{lem:ld-sandwich}]
	...
	\lean{MIPStarRE.QPBT.SandwichProduct.measurement_sum_adjoint_mul_le_one}
	\leanok
	\uses{def:povm-conventions}
...
\begin{lemma}[Formalization support for Lemma~\ref{lem:ld-sandwich}]
	...
	\lean{MIPStarRE.QPBT.SandwichProduct.heteroKron_isProj}
	\leanok
...
\begin{lemma}[Formalization support for Lemma~\ref{lem:ld-sandwich}]
	...
	\lean{MIPStarRE.QPBT.SandwichProduct.measurement_pair_fiber_sum_adjoint_mul_le_one}
	\leanok
	\uses{def:povm-conventions}
...
\begin{lemma}[Formalization support for Lemma~\ref{lem:ld-sandwich}]
	...
	\lean{MIPStarRE.QPBT.SandwichProduct.postprocess_effect_mul_effect}
	\leanok
	\uses{def:povm-conventions, def:bracket}
...
\begin{lemma}[Formalization support for Lemma~\ref{lem:ld-sandwich}]
	...
	palindromic insertion in \cite[Fact~4.34]{Natarajan2018NEEXP}. If the multipliers $S_z^x$
	satisfy
	\[
		\sum_{z:\,\operatorname{eval}_x(z)=a}(S_z^x)^\dagger S_z^x\leq\Id
	\]
	for every $x,a$, then left multiplication by $S_z^x$ does not increase the
	operator-family distance after reindexing by $\operatorname{eval}_x$.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:distance-sum-contraction}
...
\begin{definition}[Typed conditionally linear functions]\label{def:typed-cl-functions}
	...
	\leanok
	\uses{def:cl-func}
	Let $\mathcal T$ be a set (the \emph{type set}) and let $V$ be $\F^n$ for some integer $n \ge 0$. A \emph{$\mathcal T$-typed family of $\ell$-level conditionally linear functions} (implicitly, \emph{on $V$}) is a collection $\{L_t\}_{t \in \mathcal T}$ such that, for each $t \in \mathcal T$, $L_t$ is an $\ell$-level conditionally linear function on $V$. Finiteness of $\mathcal T$ plays no part in this definition, which is therefore stated, as in the linked declaration, for an arbitrary type set; it is required from Definition~\ref{def:graph-distribution} onwards, where $\mathcal T$ indexes a distribution. The source states the definition for a finite type set, and every type set occurring below is finite.
\end{definition}
