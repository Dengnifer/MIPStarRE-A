...
\begin{lemma}[Extraction of the EPR state and the Pauli observables]\label{lem:qld-unitary}
	...
	The existence declaration linked above assumes the global polynomial-pair measurement of Lemma~\ref{lem:qld-4-7} as data, instead of obtaining it from that lemma as the proof below does; it is therefore a conditional form of the statement above and does not certify it. The discrepancy and the declaration that must replace it are recorded in \cite{gap:qpbt_extraction-transfer}.
\end{lemma}
\begin{proof}
