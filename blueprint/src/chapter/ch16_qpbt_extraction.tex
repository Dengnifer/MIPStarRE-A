...
\]
For a $g$ that is not an encoding the two sides of this identity are, as functions of $u$, distinct polynomials; see Remark~\ref{rem:qld-decoding-identity}.
...
\begin{definition}[Pulled-apart Pauli observables]\label{def:tilde-w-observables}
	\uses{def:tilde-m-measurement, def:dual-self-dual-normal-basis, def:subfield-trace}
	Let $\{e_j\}_{j \in \{1,\ldots,t\}}$ be the self-dual basis for $\F_q$ over $\F_2$ of Definition~\ref{def:dual-self-dual-normal-basis}, and let $\tr = \tr_{q \to 2} \colon \F_q \to \F_2$ be the subfield trace. For all $W \in \{X,Z\}$, $\tilde{u} \in \F_q^M$, and $j \in \{1,\ldots,t\}$ define the operator
...
\end{lemma}
\begin{proof}
	\uses{def:tilde-m-measurement, def:tau-dot-product-projector, def:s-w-marginals, lem:pauli-observable-expansion, lem:twisted-commutation, lem:qld-4-7}
...
\end{lemma}
\begin{proof}
	\uses{lem:qld-4-7, lem:tildew-product-form, lem:twisted-commutation, def:tau-dot-product-projector, def:indicator-vector, def:decoding-map, def:generalized-pauli}
