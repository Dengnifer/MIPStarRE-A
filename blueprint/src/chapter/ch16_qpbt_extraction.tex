...
\begin{lemma}[Consistency and self-consistency of the pulled-apart measurements]\label{lem:qld-construct-the-paulis}
	...
	\begin{enumerate}
		...
		\]
	\end{enumerate}
	The Lean declaration is an open source-facing construction obligation. The
	separate estimates for a supplied global measurement do not establish it.
	In particular, the current global-witness constructor is unfinished and has
	only a positive-error domain; the zero-error case of this statement must also
	be obtained. This distinction is recorded in \cite{gap:qpbt_decoding-identity}.
\end{lemma}
\begin{proof}
...
\begin{example}[Conditional estimates given a global polynomial-pair witness]
	...
	Here the measurements and their consistency estimates are explicit
	hypotheses. They are not constructed from the strategy. The estimate for
	Alice's pulled-apart measurement and Bob's point measurement is proved under
	these hypotheses; the other point orientation and observable self-consistency
	remain conditional proof obligations. Thus this auxiliary entry certifies
	neither the paired source statement nor the witness constructor; see
	\cite{gap:qpbt_decoding-identity}.
\end{example}
