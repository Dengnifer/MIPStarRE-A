...
The following finite-dimensional estimates justify the insertion of range
projections in the final soundness argument. The omitted step in the source
is discussed in \cite{gap:qpbt_extraction-transfer}.
...
\begin{theorem}[Bob's bipartite isometry transfer]
  ...
  \leanok
  Let the two isometries, finite answer set, comparison operators, and vectors
  be as in Theorem~\ref{thm:pauli-isometry-transfer-alice-support}.
  Let $M_a$ instead act on $\mathcal H_{\mathrm B}$ and let $\{N_a\}$ be a
  complete POVM on $\mathcal K_{\mathrm B}$ satisfying
  $N_a\phi_{\mathrm B}=\phi_{\mathrm B}M_a$. Then
  \[
...
\begin{theorem}[Square root of the soundness error]
  \label{thm:pauli-error-square-root-support}
  \lean{MIPStarRE.QPBT.sqrt_deltaQld_le}
  \leanok
  For real $a,b$, write
  $\delta_{\mathrm{qld}}^{a,b}(\eps,m,d,q)
  =a(md)^a(\eps^b+q^{-b}+2^{-bmd})$.
  For every admissible $(q,m,d)$, every $a\ge1$, every real $b$, and every
  $\eps\ge0$,
  \[
    \sqrt{\delta_{\mathrm{qld}}^{a,b}(\eps,m,d,q)}
    \le \delta_{\mathrm{qld}}^{a,b/2}(\eps,m,d,q).
  \]
\end{theorem}
\begin{proof}
  \leanok
  Admissibility gives $md\ge1$, so $A=a(md)^a\ge1$ and $\sqrt A\le A$.
  For the remaining sum, subadditivity of the square root gives
  $\sqrt{x+y+z}\le\sqrt x+\sqrt y+\sqrt z$ for nonnegative $x,y,z$.
  Taking the square root of each real power halves its exponent.
\end{proof}
