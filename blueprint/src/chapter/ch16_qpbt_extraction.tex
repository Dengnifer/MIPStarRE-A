...
\begin{proof}
	...
	$m \geq 2$. Two further steps of that same chain are likewise unproved. The
	classical soundness statement invoked there, Theorem~\ref{lem:ld-soundness},
	...
	\cite{gap:qpbt_ld-dimension-divisibility}. The combined-lines consistency
	of Lemma~\ref{lem:qld-4-13}, which Lemma~\ref{lem:qld-4-7} consumes, now
	carries the corrected error $m \cdot \poly(\eps, md/q)$ established by its
	proof; the unsupported printed form $\poly(m^2 \eps, md/q)$ and the
	correction are documented in
	\cite{gap:qpbt_combined-lines-error-term}. The present proof is therefore
	complete only modulo the three low-degree-test obligations. Since the
