...
% Unmarked, tracked in issue #667; no permanent exemption. `canonicalComplement` fixes the
% pivot set by the intrinsic prefix-rank characterization instead of running the
% reduced-row-echelon algorithm this node prescribes. The two agree by the
% basis-independence paragraph below, but that identification is not itself formalized,
% so a statement-level mark here would claim the algorithmic presentation as matched.
...
% Unmarked, tracked in issue #667, for the reason recorded above
% Definition~def:canonical-complement. `isCompl_registerSubmodule_canonicalComplement`
% proves exactly the displayed conclusion for the prefix-rank complement, and its own
% docstring places the equivalence with a reduced-row-echelon algorithm outside the
% statement; the proof below is the row-echelon route and is not the route Lean takes.
\begin{lemma}\label{lem:canonical-complement}
	\lean{MIPStarRE.QPBT.isCompl_registerSubmodule_canonicalComplement}
