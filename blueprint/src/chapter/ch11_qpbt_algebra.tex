...
\begin{definition}[Canonical complement]\label{def:canonical-complement}
	...
	\[
		F^\perp = \{\hat{e}_j : j \notin J\}\;.
	\]
	The canonical complement is a \emph{set} of vectors, not a subspace, and it
	depends on the set $F$ and not only on $\mathrm{span}(F)$. If $S$ is a
	register subspace spanned by $\hat{E}_0 \subseteq \hat{E}$, then the
	canonical complement of $\hat{E}_0$ is $\hat{E} \setminus \hat{E}_0$, and its
	span coincides with $S^\perp$.
\end{definition}
...
\begin{definition}[Canonical linear map with prescribed kernel]\label{def:cl-canonical}
	...
	Lemma~\ref{lem:canonical-complement}. When the basis $F$ for $S$ is clear
	from context, we call this map the \emph{canonical linear map with kernel
	$S$}. Note that $\ker(L) = S$.
\end{definition}
...
\begin{definition}[The map $L^\perp$]\label{def:Lperp}
	...
	kernel basis $F$. (The map $L^\perp$ depends on the chosen basis $F$, which
	the notation leaves implicit.)
\end{definition}
...
\begin{definition}[Coordinate map and multiplication tables]\label{def:subfields-kappa}
	Let $q$ be a prime power and $k$ an integer, so that $\F_q$ is a subfield of
	$\F_{q^k}$ and $\F_{q^k}$ is a linear space of dimension $k$ over $\F_q$.
...
\begin{lemma}[Coordinate maps of self-dual bases]\label{lem:downsize_field}
	...
	Let $q$ be a prime power, $k$ an integer, and $\{e_i\}$ a self-dual basis of
	$\F_{q^k}$ over $\F_q$. The map $\kappa_q$ corresponding to $\{e_i\}$
	satisfies:
	\begin{enumerate}
...
\begin{definition}[Individual and total degree]\label{def:polynomials-degree}
	...
	\]
	with coefficients $c_\alpha \in \F_q$. It has \emph{individual degree (at
	most) $d$} if $c_\alpha \neq 0$ only when $\alpha_i \le d$ for all
	$1 \le i \le m$, and \emph{total degree (at most) $d$} if $c_\alpha \neq 0$
	only when $\sum_i \alpha_i \le d$. (Affine) \emph{multilinear} polynomials
	are those of individual degree $1$. The set of $m$-variate polynomials of
	individual degree at most $d$, viewed as functions $\F_q^m \to \F_q$, is
	the set $\polyfunc{m}{q}{d}$ of Definition~\ref{def:polyfunc}.
\end{definition}
...
\begin{remark}\label{rem:schwartz-zippel-restated}
	The chapters on the classical low individual degree test already carry the
	Schwartz--Zippel lemma as Lemma~\ref{lem:schwartz-zippel-total-degree},
	together with its individual-degree consequence
	Lemma~\ref{lem:schwartz-zippel-individual}: distinct elements of
	$\polyfunc{m}{q}{d}$ agree on at most an $md/q$ fraction of points. The
	Pauli basis test chapters invoke those nodes where the individual-degree
	form is needed; Lemma~\ref{lem:schwartz-zippel} only preserves the label of
	\cite{Ji2020MIPStar}.
\end{remark}
...
\begin{lemma}[Qudit-to-qubit isomorphism]\label{lem:pauli-binary}
	...
	For all admissible field sizes $q = 2^k$ and integers $L$, there exists an
	isomorphism $\phi : (\C^q)^{\ot L} \to (\C^2)^{\ot Lk}$ such that
...
\begin{remark}\label{rem:pauli-binary-source}
	...
	$j \in \{1, \dots, k\}$, as the $i$-th qudit is mapped to $k$ qubits.
\end{remark}
