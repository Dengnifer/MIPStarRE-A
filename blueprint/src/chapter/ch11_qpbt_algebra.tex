...
\begin{definition}[Register subspace]\label{def:register-subspace}
	\lean{MIPStarRE.QPBT.registerSubmodule}
	\leanok
...
% The dot product itself is Mathlib's `dotProduct`; the Lean declaration listed here
% is the orthogonal-subspace half of the node, which is the part this development
% defines for itself.
\begin{definition}[Dot product and orthogonal subspace]\label{def:dot-product-orthogonal}
	\lean{MIPStarRE.QPBT.dotOrthogonal}
	\leanok
...
\begin{lemma}\label{lem:perp_perp}
	\lean{MIPStarRE.QPBT.finrank_add_finrank_dotOrthogonal, MIPStarRE.QPBT.dotOrthogonal_dotOrthogonal}
	\leanok
...
\begin{definition}[Deterministic Gaussian elimination]\label{def:gaussian-elimination}
  \lean{MIPStarRE.QPBT.gaussianElimination}
  \lean{MIPStarRE.QPBT.GaussianElimination.eliminateColumnCounted}
  \lean{MIPStarRE.QPBT.GaussianElimination.State.nonpivotIndices}
  \leanok
  This auxiliary construction specifies the Gaussian elimination used in
  Definition~\ref{def:canonical-complement}; it is not a separate labelled
  definition of \cite{Ji2020MIPStar}. Let $K$ be any field with executable
  field operations and decidable equality, and let $A\in K^{m\times n}$ be
  given by its stored entries, where $m,n\geq 0$.

  In this construction, rows $a\in\{1,\ldots,m\}$ and columns
  $b\in\{1,\ldots,n\}$ use the paper's numbering. They correspond to the
  formal indices $i=a-1$ and $j=b-1$, respectively. A pivot in paper row
  $a$ has column number
  $p_{\mathrm{paper}}(a)=p_{\mathrm{Lean}}(a-1)+1$.
  The same conversion applies to every row or column subscript below.
  The number $r$ of completed pivots is a count in both conventions:
  the next paper row is $r+1$, with formal index $r$.

  Start with $B=A$ and $r=0$, and process columns $b=1,\ldots,n$ in order.
  When $r<m$, inspect the rows in increasing order and choose the first
  $s>r$ with $B_{s,b}\ne 0$. If there is no such row, leave $B$ unchanged.
  Otherwise interchange rows $r+1$ and $s$, writing $D$ for the resulting
  matrix, and form the stored row
  \[
    v_d=D_{r+1,d}/D_{r+1,b}\qquad(1\leq d\leq n).
  \]
  Replace $B$ by the stored matrix
  \[
    B'_{a,d}=\begin{cases}
      v_d,&a=r+1,\\
      D_{a,d}-D_{a,b}v_d,&a\ne r+1,
    \end{cases}
  \]
  record $p_{\mathrm{paper}}(r+1)=b$, and increase $r$ by one.
  Once $r=m$, subsequent columns leave the state unchanged. The output
  consists of $B$, the count $r$, the recorded pivot columns, and their
  complement in $\{1,\ldots,n\}$. Each normalized row is stored before
  forming the other rows, so reading it does not repeat its divisions.
\end{definition}

% Issues #676 and #690: the intrinsic correspondence and executable construction
% are both proved. The efficiency assertion uses unit-cost field arithmetic and
% equality tests, as specified in thm:gaussian-elimination-cost.
% See audits/2026-09-22_issue-690-canonical-integration.md; no exemption is granted.
\begin{definition}[Canonical complement]\label{def:canonical-complement}
	\lean{MIPStarRE.QPBT.canonicalComplement}
  \lean{MIPStarRE.QPBT.gaussianElimination}
  \lean{MIPStarRE.QPBT.gaussianElimination_of_linearIndependent}
  \lean{MIPStarRE.QPBT.gaussianElimination_cost_le_poly}
  \lean{MIPStarRE.QPBT.IsReducedRowEchelon.canonicalComplement_eq_nonpivot_indices}
  \lean{MIPStarRE.QPBT.IsReducedRowEchelon.pivot_indices_eq_of_span_eq}
  \lean{MIPStarRE.QPBT.card_canonicalComplement_span_rows}
  \lean{MIPStarRE.QPBT.linearIndependent_canonicalComplement}
  \lean{MIPStarRE.QPBT.canonicalComplement_registerSubmodule}
  \lean{MIPStarRE.QPBT.registerSubmodule_canonicalComplement_eq_dotOrthogonal}
  \leanok
	\uses{def:register-subspace, def:gaussian-elimination}
	Let $\hat{E}$ be the standard basis of $V = \F^n$ and let
	$F = \{v_1, \dots, v_m\} \subset V$ be a set of $m$ linearly independent
	vectors. Write $v_a = \sum_{b=1}^n A_{a,b}\, \hat{e}_b$, and, using a
	canonical Gaussian-elimination algorithm valid over arbitrary fields,
	transform the $m \times n$ matrix $A$ to reduced row echelon form
	$B$. Let $J$ be the set of $m$ column indices containing the leading
	$1$ entry of each row of $B$. The \emph{canonical complement of $F$}
	...
	$\mathrm{span}(F)$ and the ambient standard basis. Indeed, if $\pi_k$ is
	restriction to the first $k$ coordinates, the number of pivots among those
	coordinates is $\dim\pi_k(\mathrm{span}(F))$. Thus $b$ is a nonpivot
	coordinate precisely when
	\[
		\dim\pi_b(\mathrm{span}(F)) = \dim\pi_{b-1}(\mathrm{span}(F))\;,
	\]
	which is independent of the chosen basis and its order. The presentation
  ...
  Definition~\ref{def:gaussian-elimination} gives a deterministic realization
  of this construction. Theorem~\ref{thm:gaussian-elimination-correct}
  identifies its computed nonpivot indices with the intrinsic complement.
  For stored input, Theorem~\ref{thm:gaussian-elimination-cost} gives at most
  $n(m+n+2mn)$ charged field operations and equality tests, with each such
  operation assigned unit cost. This is the arithmetic interpretation of
  efficient construction over an arbitrary field; it makes no assertion
  about the bit complexity of a field representation.
\end{definition}

The row indices $a=1,\ldots,m$ and column indices $b=1,\ldots,n$ here and
in the following decomposition use the one-based numbering of
\cite{Ji2020MIPStar}. Their zero-based counterparts in Lean are $i=a-1$ and
$j=b-1$, respectively; the matrix entry $A_{a,b}$ corresponds to the entry
at $(i,j)$. If $p_a=p_{\mathrm{paper}}(a)$ is the pivot column of row $a$
and $p_{\mathrm{Lean}}$ is the zero-based pivot map, then
\[
  p_{\mathrm{paper}}(a)=p_{\mathrm{Lean}}(a-1)+1\;.
\]
The prefix lengths $b$ and $b-1$ therefore become $j+1$ and $j$; a prefix
length counts coordinates and is not itself a coordinate index. In particular,
$\pi_0$ restricts to the empty prefix. Similarly, a row sum
$\sum_{a=1}^m$ becomes the sum of the reindexed terms over
$i\in\{0,\ldots,m-1\}$.

\begin{theorem}[Correctness of Gaussian elimination]\label{thm:gaussian-elimination-correct}
  \lean{MIPStarRE.QPBT.gaussianEliminationRows}
  \lean{MIPStarRE.QPBT.gaussianEliminationPivots}
  \lean{MIPStarRE.QPBT.gaussianElimination_correct}
  \lean{MIPStarRE.QPBT.gaussianElimination_of_linearIndependent}
  \leanok
  \uses{def:gaussian-elimination, def:canonical-complement}
  This auxiliary theorem verifies the construction in
  Definition~\ref{def:canonical-complement}. For any stored matrix
  $A\in K^{m\times n}$ over a field with the computational data of
  Definition~\ref{def:gaussian-elimination}, let $B,r,p$ be the computed
  matrix, number of pivots, and pivot columns, in the paper's numbering.
  The first $r$ rows of $B$ are in reduced row echelon form:
  $p(1)<\cdots<p(r)$, $B_{a,p(c)}=\delta_{ac}$ for $1\leq a,c\leq r$,
  and $B_{a,b}=0$ for $b<p(a)$. Every remaining row is zero, and
  \[
    \mathrm{span}\{B_{a,*}:1\leq a\leq r\}
      =\mathrm{span}\{A_{a,*}:1\leq a\leq m\}=:S.
  \]
  The computed nonpivot columns are exactly those $b$ for which
  $\dim\pi_b(S)=\dim\pi_{b-1}(S)$, where $\pi_k$ restricts to the first
  $k$ coordinates. Thus paper column $b$ corresponds to formal column
  $j=b-1$, and the two prefix lengths are $j+1$ and $j$.
  If the input rows are independent, then $r=m$, the full output matrix
  is in reduced row echelon form, and its nonpivot standard basis vectors
  are precisely $F^\perp$ for $F=\{A_{1,*},\ldots,A_{m,*}\}$.
\end{theorem}
\begin{proof}
  \leanok
  \uses{def:gaussian-elimination, def:canonical-complement}
  After each inspected column, the completed pivots are ordered unit
  columns, their rows vanish before their pivots, and the remaining rows
  vanish in all inspected columns. A nonzero pivot step preserves the row
  span: its normalization and row subtractions can be reversed. Induction
  over the columns proves these assertions for the output. Removing the
  zero rows preserves its span. The prefix-dimension characterization of
  Definition~\ref{def:canonical-complement} then identifies the nonpivot
  columns. For independent input rows, equality of row spans gives
  $r=\dim S=m$.
\end{proof}

\begin{theorem}[Arithmetic cost of Gaussian elimination]\label{thm:gaussian-elimination-cost}
  \lean{MIPStarRE.QPBT.GaussianElimination.tabulateCounted}
  \lean{MIPStarRE.QPBT.GaussianElimination.findPivot_spec}
  \lean{MIPStarRE.QPBT.GaussianElimination.eliminateColumnCounted_cost}
  \lean{MIPStarRE.QPBT.gaussianElimination_cost_le_poly}
  \leanok
  \uses{def:gaussian-elimination}
  This auxiliary theorem makes precise the arithmetic cost of the
  construction accompanying Definition~\ref{def:canonical-complement}.
  Give each field division, multiplication, subtraction, and equality test
  unit cost. For every stored $m\times n$ matrix, the charged cost $C(A)$ of
  Definition~\ref{def:gaussian-elimination} satisfies
  \[
    C(A)\leq n(m+n+2mn).
  \]
  The counter accompanies the actual stored computations: each tabulation
  evaluates an entry once and then sums its stored charge. Each inspected
  search row is charged one equality test, even if its index already
  excludes it and the field test is omitted. This conservative convention
  makes $C(A)$ an upper bound on the executed field operations and tests.
\end{theorem}
\begin{proof}
  \leanok
  \uses{def:gaussian-elimination}
  In each column, the search inspects at most $m$ rows. A successful pivot
  requires $n$ divisions for its normalized row and at most $mn$
  multiplications and $mn$ subtractions for the other rows. Summing the
  charges over the $n$ column iterations gives the bound, including the
  columns without pivots and those after all rows have been processed.
\end{proof}

Array access introduces no field arithmetic. Per column, tabulation,
summation of charges, row scanning, and pivot storage traverse arrays or
lists of sizes bounded by $m$, $n$, and $mn$. Selecting the nonpivot columns
uses at most $mn$ comparisons of stored pivot indices. These bounds explain
the polynomial administrative work of the construction; the displayed
theorem counts field operations and tests, not machine instructions,
allocations, or bit operations. The input is already materialized: any
computation used to produce its entries lies outside this count.

\begin{lemma}[Explicit RREF decomposition]\label{lem:canonical-complement-rref}
  \lean{MIPStarRE.QPBT.IsReducedRowEchelon}
  \lean{MIPStarRE.QPBT.rowEchelonComponent}
  \lean{MIPStarRE.QPBT.rowEchelonComponent_mem_span}
  \lean{MIPStarRE.QPBT.IsReducedRowEchelon.rowEchelonComponent_pivot}
  \lean{MIPStarRE.QPBT.IsReducedRowEchelon.sub_rowEchelonComponent_mem}
  \lean{MIPStarRE.QPBT.IsReducedRowEchelon.isCompl_span_nonpivot}
  \leanok
  \uses{def:canonical-complement, def:complementary}
  This auxiliary isolates the coordinate calculation in the proof of
  Lemma~\ref{lem:canonical-complement}; it is not a separate labelled result
  of \cite{Ji2020MIPStar}. Let $B$ be in reduced row echelon form, with
  nonzero rows $b_1,\ldots,b_m$ and pivot indices $p_1<\cdots<p_m$.
  Put $S=\mathrm{span}\{b_a:1\le a\le m\}$ and
  $T=\mathrm{span}\{\hat e_b:b\notin\{p_1,\ldots,p_m\}\}$.
  For each $x\in V$, the vector
  \[
    v=\sum_{a=1}^m x_{p_a} b_a
  \]
  belongs to $S$, agrees with $x$ at every pivot coordinate, and satisfies
  $x-v\in T$. Moreover $S\cap T=\{0\}$, so $V=S\oplus T$.
\end{lemma}
\begin{proof}
  \leanok
  \uses{def:register-subspace, def:complementary}
  Membership in $S$ follows from the displayed linear combination.
  Since $b_a(p_c)=\delta_{ac}$, evaluation at $p_c$ gives $v_{p_c}=x_{p_c}$.
  Hence $x-v$ is supported on the nonpivot coordinates and lies in $T$.
  If $y=\sum_{a=1}^m c_a b_a$ also lies in $T$, evaluation at $p_a$ gives $c_a=0$
  for every $a$, whence $y=0$.
\end{proof}

\begin{lemma}\label{lem:canonical-complement}
	\lean{MIPStarRE.QPBT.isCompl_span_rows_canonicalComplement}
	\leanok
	\uses{def:canonical-complement, def:complementary}
...
\begin{proof}
	\leanok
	\uses{def:canonical-complement, lem:canonical-complement-rref}
	Let $A = (A_{a,b})$ be the $m \times n$ matrix whose rows are the $v_a$ and
	choose an RREF matrix $B$ with $A=UB$ for an invertible $U$.
	Such a presentation exists because the input rows are independent.
	The rows of $B$ span $S$, and its nonpivot standard basis vectors are
	exactly $F^\perp$, by the prefix-dimension characterization in
	Definition~\ref{def:canonical-complement}.
	Lemma~\ref{lem:canonical-complement-rref} gives
	$S+T=V$ and $S\cap T=\{0\}$.
\end{proof}
...
\begin{definition}[The map $L^\perp$]\label{def:Lperp}
	\lean{MIPStarRE.QPBT.canonicalProjPerp}
	\leanok
...
\begin{lemma}\label{lem:L_perp_perp}
	\lean{MIPStarRE.QPBT.ker_canonicalProjPerp}
	\leanok
	\uses{def:Lperp}
...
\begin{definition}[Coordinate map and multiplication tables]\label{def:subfields-kappa}
	\lean{MIPStarRE.QPBT.chiOfBasis, MIPStarRE.QPBT.basisCoordVec}
	\leanok
...
\begin{definition}[Admissible field size]\label{def:admissible-size}
	\lean{MIPStarRE.QPBT.IsAdmissibleSize}
	\leanok
...
% Exemption: the node is not marked \leanok. The Lean `binTrace` is the $q = 2$
% specialization of this node, the only one the Pauli basis test uses; the general
% subfield trace $\tr_{q^k \to q}$ has no Lean counterpart. The scope difference is
% recorded in gap:qpbt_characteristic-two-pauli-scope.
...
\begin{definition}[Dual, self-dual, and normal bases]\label{def:dual-self-dual-normal-basis}
	\lean{MIPStarRE.QPBT.IsDualBasisPair, MIPStarRE.QPBT.Basis.IsSelfDual, MIPStarRE.QPBT.Basis.IsNormal, MIPStarRE.QPBT.exists_selfDualNormalBasis}
	\leanok
	...
	The basis coordinates are indexed by $0,\ldots,k-1$; coordinate $r$
	corresponds to the paper's coordinate $r+1$.
\end{definition}

\begin{lemma}[Coordinate maps of self-dual bases]\label{lem:downsize_field}
	\lean{MIPStarRE.QPBT.basisCoord_apply_eq_trace, MIPStarRE.QPBT.trace_mul_eq_dotProduct_basisCoord, MIPStarRE.QPBT.chi_mulVec_basisCoord}
	\leanok
...
\begin{lemma}\label{lem:one}
	\lean{MIPStarRE.QPBT.trace_basis_eq_one_of_selfDual_normal, MIPStarRE.QPBT.basisCoord_one_of_selfDual_normal}
	\leanok
	\uses{def:subfields-kappa, def:subfield-trace, def:dual-self-dual-normal-basis}
...
\begin{definition}[Binary representation of field elements]\label{def:binary-representation}
	\lean{MIPStarRE.QPBT.FixedFieldModel.binaryCoordinates, MIPStarRE.QPBT.multiplicationTable, MIPStarRE.QPBT.chi, MIPStarRE.QPBT.binaryCoordinates_mul}
	\lean{MIPStarRE.QPBT.binaryCoordinates_basis_mul}
	\leanok
	\uses{def:subfields-kappa, def:admissible-size,
	...
	The sums use coordinates $0,\ldots,k-1$; coordinate $r$ corresponds to the
	paper's coordinate $r+1$.
\end{definition}
...
\begin{definition}[Low-degree encoding]\label{def:low-degree-encoding}
	\lean{MIPStarRE.QPBT.lowDegreeEncoding, MIPStarRE.QPBT.lowDegreeEnc}
	\leanok
	\uses{def:polynomials-degree}
...
\begin{definition}[Indicator vector and linearity of the encoding]\label{def:indicator-vector}
	\lean{MIPStarRE.QPBT.indicatorVec, MIPStarRE.QPBT.lowDegreeEnc_eq_dotProduct}
	\leanok
...
\begin{definition}[Decoding maps]\label{def:decoding-map}
	\lean{MIPStarRE.QPBT.decodeAt, MIPStarRE.QPBT.decodeBool, MIPStarRE.QPBT.decodeAt_lowDegreeEnc}
	\leanok
...
\begin{proposition}\label{prop:line-equiv}
	\lean{MIPStarRE.QPBT.linePoints_eq_of_mem}
	\leanok
	\uses{def:line}
...
\begin{lemma}[Formalization support for the canonical representative]\label{lem:line-rep-kernel}
	\lean{MIPStarRE.QPBT.sub_lineRepMap_mem_span}
	\leanok
	\uses{def:line-representative, def:cl-canonical}
	This is not a named statement of the source article; it isolates the defining property of $L^{\mathrm{Ln}}_v$ that the line samplers of Chapter~\ref{chap:qpbt-test} invoke. For all $u, v \in \F_q^m$,
	\[
		u - L^{\mathrm{Ln}}_v(u) \in \mathrm{span}_{\F_q}\{v\}\;.
	\]
\end{lemma}
\begin{proof}
	\leanok
	\uses{def:cl-canonical, def:complementary}
	The map $L^{\mathrm{Ln}}_v$ is the projection onto the canonical complement of $\mathrm{span}_{\F_q}\{v\}$ along $\mathrm{span}_{\F_q}\{v\}$, so $u - L^{\mathrm{Ln}}_v(u)$ is the value at $u$ of the complementary projection, which takes its values in $\mathrm{span}_{\F_q}\{v\}$.
\end{proof}

\begin{lemma}[Formalization support for line incidence]\label{lem:line-rep-incidence}
	\lean{MIPStarRE.QPBT.mem_linePoints_lineRepMap}
	\leanok
	\uses{def:line-representative, def:line, lem:line-rep-kernel}
	This is not a named statement of the source article; it is the incidence half of Definition~\ref{def:line-representative}, in the form used by the line-versus-point samplers of Chapter~\ref{chap:qpbt-test}. For all $u, v \in \F_q^m$,
	\[
		u \in \ell\bigl(L^{\mathrm{Ln}}_v(u),\, v\bigr)\;.
	\]
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:line-rep-kernel, def:line}
	By Lemma~\ref{lem:line-rep-kernel} there is $t \in \F_q$ with $u - L^{\mathrm{Ln}}_v(u) = t\,v$, so $u = L^{\mathrm{Ln}}_v(u) + t\,v$, which lies in $\ell\bigl(L^{\mathrm{Ln}}_v(u), v\bigr)$ by Equation~\eqref{eq:line-set}.
\end{proof}
...
\begin{definition}[Generalized Pauli observables]\label{def:generalized-pauli}
	\lean{MIPStarRE.QPBT.primeTauShift, MIPStarRE.QPBT.primeTauPhase}
	\lean{MIPStarRE.QPBT.primeTauObservable, MIPStarRE.QPBT.primePauliFourier}
	\lean{MIPStarRE.QPBT.primePauliVec, MIPStarRE.QPBT.primePauliProj}
	\lean{MIPStarRE.QPBT.primePauliVec_orthonormal}
	\lean{MIPStarRE.QPBT.primeTauObservable_mulVec_primePauliVec}
	\lean{MIPStarRE.QPBT.primePauliProj_mul_primePauliProj}
	\lean{MIPStarRE.QPBT.primePauliProj_conjTranspose}
	\lean{MIPStarRE.QPBT.posSemidef_primePauliProj}
	\lean{MIPStarRE.QPBT.primePauliProj_isProj, MIPStarRE.QPBT.sum_primePauliProj_eq_one}
	\leanok
	...
	The distinction between the general prime-characteristic definitions and
	the characteristic-two specializations used by the Pauli basis test is
	documented in \cite{gap:qpbt_characteristic-two-pauli-scope}.
\end{definition}

\begin{lemma}[Product and twisted commutation relations]\label{lem:twisted-commutation}
	\lean{MIPStarRE.QPBT.primeTauObservable_mul}
	\lean{MIPStarRE.QPBT.primeTauObservable_pow}
	\lean{MIPStarRE.QPBT.primeTauObservable_pow_char}
	\lean{MIPStarRE.QPBT.primeTauObservable_isUnitary}
	\lean{MIPStarRE.QPBT.primeTauObservable_eigenvalue_pow_char}
	\lean{MIPStarRE.QPBT.primeTauObservable_X_mul_Z}
	\leanok
	...
	\begin{enumerate}
		\item For all $a, a' \in \F_q$ and $b \in \F_p$,
		      ...
		      \end{equation}
		      in particular $\bigl(\tau^W(a)\bigr)^p = \Id$. By the explicit
		      definitions each $\tau^W(a)$ is unitary, and its eigenvalues are
		      $p$-th roots of unity.
...
\begin{lemma}[Fourier cancellation over a subspace]\label{lem:cancellation}
	\lean{MIPStarRE.QPBT.ffChar_dotProduct_submodule_expect_eq_zero}
	\leanok
...
\begin{lemma}[Fourier expansion of Pauli observables]\label{lem:pauli-observable-expansion}
	\lean{MIPStarRE.QPBT.primeTauObservable_eq_sum_primePauliProj}
	\lean{MIPStarRE.QPBT.primePauliProj_eq_avg_primeTauObservable}
	\leanok
	\uses{def:generalized-pauli, def:subfield-trace}
	...
	\begin{equation}\label{eq:pauli-inversion-0}
		...
		= \tau^W_b\;.
	\end{equation}
	The source scope and the binary specialization of these identities are
	recorded in \cite{gap:qpbt_characteristic-two-pauli-scope}.
\end{lemma}
\begin{proof}
	\leanok
	\uses{def:generalized-pauli, lem:fourier-fact-finite-index}
	For $W=Z$, the entrywise expansion follows directly from the computational
	basis projectors. For $W=X$, multiplying the explicit rank-one projector
	entries by the character coefficient reduces each matrix entry to the
	full-space character sum of Lemma~\ref{lem:fourier-fact-finite-index}, which
	vanishes off the required shift and equals one on it. For the inversion,
	substitute the forward expansion and exchange the finite sums. The same
	full-space Fourier orthogonality makes the inner expectation vanish unless
	$b'=b$, where it equals $1$.
\end{proof}
...
\begin{lemma}[Qudit-to-qubit isomorphism]\label{lem:pauli-binary}
	\lean{MIPStarRE.QPBT.qubitPauliProj, MIPStarRE.QPBT.exists_qubitIsometry}
	\leanok
	\uses{def:admissible-size, def:subfields-kappa, def:binary-representation,
		def:generalized-pauli, def:EPR}
	For all admissible field sizes $q = 2^k$ and integers $L \ge 0$, there
	...
	We index the product coordinates from zero: values $(i-1,j-1)$ correspond to
	the paper's pair $(i,j)$.  The corrected upper bound $j\leq k$ is
	documented in \cite{gap:qpbt_pauli-binary-factor-index}.
\end{lemma}
\begin{proof}
	\leanok
	\uses{def:binary-representation, def:subfields-kappa, lem:downsize_field,
		def:generalized-pauli, def:EPR, lem:pauli-observable-expansion}
	Let
	$e : \F_q^L \to \F_2^{L \times k}$ be the bijection that expands each
	coordinate in the fixed self-dual normal basis
	(Definition~\ref{def:binary-representation}), so that $e(u)$ is exactly the
	array $(u_{ij})_{i,j}$ of the statement, and let $\phi$ be the unitary that
	permutes the two computational bases along $e$, that is,
	$\phi \ket{u} = \ket{e(u)}$. Identity \eqref{eq:epr-q-to-epr-2} holds for
	any such relabelling: a bijection of orthonormal bases carries the maximally
	entangled state of the source space to that of the target space. For
	\eqref{eq:pauli-q-to-pauli-2}, conjugation by $\phi^\dagger$ is exactly
	the reindexing of a matrix along $e$, so it suffices to compare matrix
	entries. Both generalized Pauli observables are computed entrywise from
	Definition~\ref{def:generalized-pauli}: $\tau^W(a)$ is, for $W = X$, the
	permutation matrix of the translation $y \mapsto y + a$, and, for $W = Z$,
	the diagonal matrix with entries $(-1)^{\tr(a \cdot y)}$. The relabelling
	$e$ is additive, and item 2 of Lemma~\ref{lem:downsize_field} gives
	$\tr(a \cdot y) = e(a) \cdot e(y)$ by self-duality of the basis; the two
	entry formulas are therefore carried by the reindexing to the corresponding
	formulas over $\F_2$, so that $\tau^W(a)$ becomes $\tau^W(e(a))$ for both
	$W$. Finally, the projectors are recovered from the observables by the
	inversion identity \eqref{eq:pauli-inversion-0} in its characteristic-two
	form $\tau^W_u = \E_{\ba} (-1)^{\tr(\ba \cdot u)}\, \tau^W(\ba)$, used
	over $\F_q^L$ on the left-hand side and over $\F_2^{L \times k}$ on the
	right; averaging the entrywise comparison
	over $\ba$ carries $\tau^W_u$ to the qubit projector at the label
	$e(u) = \kappa(u)$, which is \eqref{eq:pauli-q-to-pauli-2}. This is the
	characteristic-two specialization of
	Lemma~\ref{lem:pauli-observable-expansion}; the corresponding general-prime
	expansion and inversion are discussed in
	\cite{gap:qpbt_characteristic-two-pauli-scope}. The argument uses neither
	the qudit-by-qudit isometry $\theta$ of the source
	proof nor the product and twisted-commutation relations of
	Lemma~\ref{lem:twisted-commutation}.
\end{proof}
...
\begin{remark}\label{rem:pauli-binary-source}
	...
	takes $L$ to be a natural number, including zero, and uses the usual
	empty-product convention for the tensor powers when $L=0$.  The index
	correction is documented in \cite{gap:qpbt_pauli-binary-factor-index}.
\end{remark}
