...
\begin{definition}[The classical low individual degree game: parameters and registers]\label{def:ld-game}
	...
	% The source fixes \N to be the positive integers (04_preliminaries.tex:6) and takes
	% m, d, ldc in \N (08_classical_and_quantum_low_degree_tests.tex:33-35), so positivity is explicit.
...
\begin{proof}
	\leanok
	\uses{def:cl-dist, def:ld-question-distribution, def:line-representative, lem:line-rep-incidence, lem:distribution-map-map, lem:uniform-distribution-map-equivalence, lem:uniform-distribution-map-equi-cardinal-fibers, lem:uniform-product-first-marginal}
...
\begin{proof}
	\leanok
	\uses{def:cl-dist, def:ld-question-distribution, def:line-representative, lem:line-rep-incidence, lem:distribution-map-map, lem:uniform-distribution-map-equivalence, lem:uniform-distribution-map-equi-cardinal-fibers, lem:uniform-product-first-marginal, lem:uniform-product-second-marginal}
...
\begin{definition}[Question distribution of the Pauli basis test]\label{def:pauli-question-distribution}
	...
	\uses{def:admissible, def:ld-question-distribution, lem:ld-aline-level, lem:ld-dline-level, def:ms-game, def:cl-func, def:cl-dist, def:graph-distribution}
	...
	% at every vertex.
	The question distribution of $\mathfrak{G}^{\mathrm{Pauli}}$ is obtained by sampling the type pair $(t_{\mathrm A}, t_{\mathrm B})$ from the graph distribution $\mu_{G^{\mathrm{pauli}}}$ of Definition~\ref{def:graph-distribution}, that is, uniformly at random among all ordered pairs of types whose underlying unordered pair $\{t_{\mathrm A}, t_{\mathrm B}\}$ is an edge of $G^{\mathrm{pauli}}$, self-loops included; sampling $z \in V^{\mathrm{pauli}}$ uniformly at random; and setting $x_w = L_{t_w}(z)$ for $w \in \{\mathrm A,\mathrm B\}$. Explicitly, if $z = (u_X,u_Z,s,v,r_X,r_Z)$, then a player whose type is $(\mathsf{Point},W)$ receives $u_W$; a player whose type is $(\mathsf{ALine},W)$ receives $(u_0, s)$ with $u_0 = L^{\mathrm{Ln}}_{e_i}(u_W)$ and $i = \chi(s)$; a player whose type is $(\mathsf{DLine},W)$ receives $(u_0, s, v')$ with $i = \chi(s)$, $v' = \pi_{i-1}(v)$, and $u_0 = L^{\mathrm{Ln}}_{v'}(u_W)$; a player whose type lies in $\mathcal T^{\mathrm{ms}} \cup \{\mathsf{Pair}, (\mathsf{Pair},X), (\mathsf{Pair},Z)\}$ receives $(u_X, u_Z, r_X, r_Z)$; and a player whose type is $(\mathsf{Pauli},W)$ receives $0$.
	The displayed levels of the individual maps are the content of Lemma~\ref{lem:pauli-cl-levels}. A common-level representation of the whole family and the identification of this sampler with the general typed CL distribution are the two assertions of Lemma~\ref{lem:pauli-question-typed-cl}; both remain open.
\end{definition}

\begin{lemma}[Formalization support for the levels of the Pauli maps]\label{lem:pauli-cl-levels}
	\lean{MIPStarRE.QPBT.isCondLinear_pauliCL}
	\leanok
	\uses{def:pauli-question-distribution, def:cl-func, lem:ld-aline-level, lem:ld-dline-level}
	For every admissible tuple and every $t \in \mathcal T^{\mathrm{pauli}}$, the map $L_t$ is conditionally linear at its displayed level: the point, axis-line, diagonal-line, and Pauli maps have levels $1$, $2$, $3$, and $0$, respectively, while the remaining pair and Magic Square maps have level $1$.
\end{lemma}
\begin{proof}
	\leanok
	\uses{def:cl-func, lem:ld-aline-level, lem:ld-dline-level}
	The point, axis-line, and diagonal-line maps are obtained from their low-degree counterparts by reindexing the relevant registers, which preserves the level. The Pauli map is zero. Every remaining map is the linear projection onto the two point registers and the two scalar registers, and hence has one level.
\end{proof}
...
\begin{lemma}[The Pauli question distribution is a typed CL distribution]\label{lem:pauli-question-typed-cl}
	...
	Fix an admissible tuple $\mathsf{qldparams} = (q,m,d)$ (Definition~\ref{def:admissible}) and let $L = \{L_t\}_{t \in \mathcal T^{\mathrm{pauli}}}$ be the family of Definition~\ref{def:pauli-question-distribution}. Then the question distribution of $\mathfrak{G}^{\mathrm{Pauli}}_{\mathsf{qldparams}}$ is the $(\mathcal T^{\mathrm{pauli}}, G^{\mathrm{pauli}})$-typed conditionally linear distribution $\mu^{G^{\mathrm{pauli}}}_{L,\,L}$ of Definition~\ref{def:typed-cl-distributions}. This assertion consists of an equality between the two-stage question sampler and the distribution obtained from $(L,L)$ and $G^{\mathrm{pauli}}$, together with a representation of all maps $L_t$ at one common level. Lemma~\ref{lem:pauli-cl-levels} gives their individual levels, but neither the common-level representation nor the sampler identity has yet been established. These two assertions are tracked by issue~\#180.
\end{lemma}
\begin{proof}
	\uses{def:typed-cl-distributions, def:graph-distribution, def:cl-dist}
	The type pair is drawn from $\mu_{G^{\mathrm{pauli}}}$ by definition, and for each ordered type pair the content pair is the image of a single uniform $z \in V^{\mathrm{pauli}}$ under the corresponding pair of CL functions, that is, a sample from $\mu_{L_{t_{\mathrm A}},\, L_{t_{\mathrm B}}}$. Completing the argument requires the resulting equality of finite distributions and a common-level representation of the family; both remain open.
\end{proof}
