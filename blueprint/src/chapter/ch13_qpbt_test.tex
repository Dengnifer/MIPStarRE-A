...
\begin{proof}
	...
	The two-level decomposition of $L_{\mathsf{ALine}}$ is exhibited directly: the first stage is the $1$-level CL function projecting $V_I \oplus V_V$ onto $V_I$, and the second stage, on the complementary register $V_X$, is the $1$-level CL function $L^{\mathrm{Ln}}_{e_i}$ with $i = \chi(s)$ read off the first stage. The two stages act on disjoint registers and sum to $L_{\mathsf{ALine}}$, which is therefore $1+1 = 2$-level conditionally linear.
\end{proof}
...
\begin{proof}
	\leanok
	\uses{def:cl-func, def:line-representative}
	The three-level decomposition of $L_{\mathsf{DLine}}$ is exhibited directly: the first stage is the identity on $V_I$; the second, on $V_V$, is the $1$-level CL function $v \mapsto \pi_{i-1}(v)$ with $i = \chi(s)$ read off the first stage; and the third, on $V_X$, is the $1$-level CL function $L^{\mathrm{Ln}}_{v'}$ selected by the pair $(s,v')$ read off the first two. The three stages act on pairwise disjoint registers and sum to $L_{\mathsf{DLine}}$, which is therefore $1+1+1 = 3$-level conditionally linear.
\end{proof}
