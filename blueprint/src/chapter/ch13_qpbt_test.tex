...
\begin{proof}
	\leanok
	\uses{def:cl-func, def:line-representative}
	The first linear contribution retains the coordinate register $V_I$. Its value $s$ determines $i=\chi(s)$, after which the second contribution applies the canonical representative map $L^{\mathrm{Ln}}_{e_i}$ to the point register $V_X$. These two contributions form a $2$-level representation of $L_{\mathsf{ALine}}$ in the sense of Definition~\ref{def:cl-func}.
\end{proof}
...
\begin{proof}
	\leanok
	\uses{def:cl-func, def:line-representative}
	The first linear contribution retains $s$ on the coordinate register $V_I$. Conditioned on $i=\chi(s)$, the second contribution maps the direction register $V_V$ to $v'=\pi_{i-1}(v)$. Conditioned on $v'$, the third applies $L^{\mathrm{Ln}}_{v'}$ to the point register $V_X$. These three contributions form a $3$-level representation of $L_{\mathsf{DLine}}$ in the sense of Definition~\ref{def:cl-func}.
\end{proof}
...
\begin{proof}
	\leanok
	\uses{def:ld-line-description, lem:distribution-map-comp, lem:uniform-map-equivalence, lem:uniform-product-first-marginal, lem:uniform-product-second-marginal, lem:chi-index-uniform, lem:line-representative-idempotent, lem:line-rep-incidence}
...
\begin{proof}
	\leanok
	\uses{def:ld-line-description, lem:distribution-map-comp, lem:uniform-map-equivalence, lem:uniform-product-first-marginal, lem:uniform-product-second-marginal, lem:chi-index-uniform, lem:prefix-projection-idempotent, lem:line-representative-idempotent, lem:line-description-prefix, lem:line-rep-incidence}
...
\begin{proof}
	...
	The type pair is drawn from $\mu_{G^{\mathrm{pauli}}}$ by definition, and for each ordered type pair the content pair is the image of a single uniform $z \in V^{\mathrm{pauli}}$ under the corresponding pair of CL functions, that is, a sample from $\mu_{L_{t_{\mathrm A}},\, L_{t_{\mathrm B}}}$.
\end{proof}
