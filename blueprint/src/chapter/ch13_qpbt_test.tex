...
\begin{remark}[The strategy class in Theorem~\ref{thm:ms-rigidity}]
  \label{rem:ms-rigidity-strategy-class}
  The unrestricted prescribed-answer assertion in
  \cite[Section~7]{Ji2020MIPStar} is false. Fix the canonical Mermin--Peres
  solution on $\mathcal K=\C^2\otimes\C^2$ and its maximally entangled state
  $\ket{\Omega}$. Give each player two copies of $\mathcal K$, with state
  $\ket{\Omega}_{A_1B_1}\otimes\ket{\Omega}_{A_2B_2}$. Alice answers
  constraints on copy~$1$ and variables on copy~$2$; Bob answers constraints
  on copy~$2$ and variables on copy~$1$. Each sampled question pair is a
  constraint--variable pair on a single shared copy, so the value is $1$.
  But the two players' $\mathsf{Variable}_1$ observables act on independent
  maximally mixed factors and have correlation
  \[
    \langle O_{A,1}\otimes O_{B,1}\rangle_\psi
      =\frac{\operatorname{tr}(\sigma^X\otimes I)}4
       \frac{\operatorname{tr}(\sigma^X\otimes I)}4=0.
  \]
  The printed conclusion at $\eps=0$ forces this correlation to equal $1$:
  isometries preserve it, and the two ideal $X$ observables agree on the
  extracted EPR state. The same contradiction holds for
  $\mathsf{Variable}_5$ and $Z$.

  Add flag qubits in $(\ket{01}+\ket{10})/\sqrt2$ and use one common
  measurement family: flag~$0$ selects the first assignment of roles above,
  and flag~$1$ selects the second. The state is invariant under exchanging
  the players, each flag branch still wins perfectly, and the variable
  correlations remain zero. Thus symmetry does not repair the assertion.
  Both examples use only prescribed answers and have
  $\Delta_1=\Delta_5=1$ in Definition~\ref{def:ms-variable-agreement}.

  The proved theorem above assumes, for $\eps,\delta\ge0$,
  \[
    \operatorname{val}(\mathcal S)\ge1-\eps,\qquad
    \Delta_1(\mathcal S)\le\delta,\qquad
    \Delta_5(\mathcal S)\le\delta.
  \]
  Its Euclidean state error and its six squared operator distances are
  bounded by $C(\sqrt\eps+\sqrt\delta)$, with $C=2\cdot10^{12}$; the
  operator distances are evaluated on the ideal extracted state.
  The bit effects are completed by assigning wrong-form answers to zero.
  For a symmetric projective strategy consistent on its state,
  Corollary~\ref{cor:ms-rigidity-symmetric-consistent} sets $\delta=0$.
  This recovers the printed error scale on that restricted domain.
  It does not identify completed effects with prescribed effects, since
  zero agreement does not eliminate wrong-form answers.

  The soundness argument needs only same-player anticommutation.
  Lemma~\ref{lem:qld-ms-anticommutator-original-state} bounds each player's
  squared anticommutator norm for the completed variable observables on the
  original state by $1183680\eps$, using only value at least $1-\eps$.
  The two observable orientations and the headline soundness theorems use
  these bounds, without invoking the agreement-dependent extraction.
  Point--variable consistency then transfers the anticommutation to the
  point observables. This establishes the consequence used in the sequel,
  without asserting the printed simultaneous extraction.

  A separate one-way extraction and its prescribed-answer refinement are
  retained in the closed, unmerged PR~\#706, not in the present formalization.
  They give state error $O(\sqrt\eps)$, squared Bob measurement and both
  anticommutator errors $O(\eps)$, and squared Alice measurement errors
  $O(\eps+\Delta_j)$ for $j=1,5$. The one-way witness alone compares Bob's
  variables, with Alice's isometry built from constraints; exchanging roles
  need not preserve the witness. These results neither prove the printed
  assertion nor certify global optimality of an error bound.
  The discrepancy is recorded as a documented deviation in
  \cite{gap:qpbt_ms-rigidity-symmetric-strategies}, with the separate errors
  and the retained branch evidence. Definition~\ref{def:ms-rigidity-printed-claim}
  continues to record the unrestricted prescribed-answer proposition without
  asserting it. The completion marks on the theorem above certify only its
  displayed agreement-dependent statement and proof.
\end{remark}
