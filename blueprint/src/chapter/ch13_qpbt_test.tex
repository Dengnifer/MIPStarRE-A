...
\begin{lemma}[Formalization support for Remark~\ref{rem:ld-win-zero-direction}: zero-direction mass]\label{lem:line-point-zero-direction-mass}
	...
	This lemma is not a named statement of the paper; it isolates, for the Lean proof, the mass estimate that Remark~\ref{rem:ld-win-zero-direction} appeals to when it bounds the probability that a $\mathsf{DLine}$ question carries a zero direction. Sample a pair $(\ell,u)$ from one of the seed-bearing line-point laws of Definition~\ref{def:line-point-dist} and let $v'$ denote the projected direction recorded by $\ell$. Under the axis law the event $v' = 0$ has probability $0$, and under the diagonal law it has probability at most $1/q$. No claim is made here about the mixture $D_{\mathsf{Line}}$, and neither bound is the constant $2/(mq)$ quoted in the remark, which also accounts for the $\mathsf{ALine}$ and $\mathsf{DLine}$ question weights.
\end{lemma}
\begin{proof}
	\leanok
	\uses{def:cl-dist, def:ld-line-description, lem:distribution-map-comp, lem:uniform-map-equivalence, lem:uniform-product-first-marginal, lem:prefix-projection-idempotent}
	Both laws are push-forwards of the uniform distribution on $V$ along a coordinate of the CL map of Definition~\ref{def:cl-dist}, so in each case the event may be pulled back to an event on the raw seed $z$. For the axis law the recorded direction is the coordinate vector $e_{\chi(s)}$, which is nonzero, so the pulled-back event is empty. For the diagonal law the recorded direction is $v' = \pi_{i-1}(v)$ with $i = \chi(s)$, and $\pi_{i-1}$ is idempotent (Lemma~\ref{lem:prefix-projection-idempotent}), so the projected direction of the sampled description equals $\pi_{i-1}(v)$ itself. Since $i - 1 < m$, the last coordinate $v_{m}$ is never suppressed by $\pi_{i-1}$, hence $v' = 0$ forces $v_{m} = 0$. That single raw coordinate is uniform on $\F_q$, being a marginal of the uniform distribution on $V$ (Lemma~\ref{lem:uniform-map-equivalence} and Lemma~\ref{lem:uniform-product-first-marginal}, composed by Lemma~\ref{lem:distribution-map-comp}), so it vanishes with probability exactly $1/q$. Monotonicity of the average over the pointwise inequality of indicators gives the bound.
\end{proof}
