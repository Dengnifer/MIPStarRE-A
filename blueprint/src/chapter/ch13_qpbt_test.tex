...
\begin{proof}
	...
	% The Lean declaration exists_ld_soundness is proved (axioms: propext,
	% Classical.choice, Quot.sound) by the direct route described in
	% rem:ld-soundness-provider, not by the printed tensor-code proof
	% transcribed below, whose two verification obligations are still open.
	% A proof-level \leanok here would certify the printed route, so it is
	% deliberately withheld pending an editorial decision on this node.
	The formal proof uses the direct reduction and seed compression described in
	Remark~\ref{rem:ld-soundness-provider}; it establishes all three conclusions
	for every $k$. The following account records the printed tensor-code route
	and its separate verification obligations, which that formal proof avoids
