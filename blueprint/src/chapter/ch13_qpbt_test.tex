...
\begin{lemma}[Formalization support for Lemma lem:delta-bound]
...
\[
  x^{s}\, 2^{-t x} \le \frac{\lceil s \rceil_{+}!}{(t \log 2)^{\lceil s \rceil_{+}}} .
\]
This is the explicit envelope used for the third term in the proof of Lemma~\ref{lem:delta-bound}, with $x = m$, $s = a + b$ and $t = b$.
\end{lemma}
\begin{proof}
\leanok
Since $x \ge 1$, $x^{s} \le x^{\lceil s \rceil_{+}}$, and the Taylor lower bound $y^{n}/n! \le e^{y}$ at $y = t x \log 2$ and $n = \lceil s \rceil_{+}$ gives $x^{n} (t \log 2)^{n} \le n!\, 2^{t x}$.
\end{proof}
