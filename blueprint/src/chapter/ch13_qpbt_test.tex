...
\begin{proposition}[Simultaneous polynomial measurements for general $k$]
	...
	\lean{MIPStarRE.QPBT.exists_directSimultaneousPolynomialMeasurements_combinedError, MIPStarRE.QPBT.exists_direct_ld_soundness}
	\leanok
	...
	Theorem~\ref{lem:ld-soundness}, with the error of
	...
	loss $(m+k)d/q$ for the two point/polynomial relations, and taken alone for
	the polynomial/polynomial relation.  For $0 < \eps \le 1$ that error is at
	most $\delta_{\mathrm{ld}}(\eps,q,m,d,k)$ at the universal constants
	$a = 10^{23}$ and $b = 1/80000$, and for $\eps \ge 1$ the latter is at least
	one while a consistency defect on a unit state is at most one; so for every
	$\eps > 0$ the three relations hold with the error
	$\delta_{\mathrm{ld}}(\eps,q,m,d,k)$ at those constants as well.
...
\begin{proof}
	...
	\uses{lem:ld-combined-value, lem:ld-combining-recovery,
		...
		thm:main-formal, lem:ld-soundness}
	...
	$1 - 10\eps$.  That game has simultaneity parameter $1$, so
