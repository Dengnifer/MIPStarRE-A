...
\begin{proof}
	\leanok
	The support and weight fields determine a finite distribution; the remaining
...
\begin{lemma}[Uniform first marginal after an equivalence]
	...
	\leanok
	Let $E\colon A\simeq B\times C$, and suppose $f(a)$ is the first component of
	$E(a)$ for every $a\in A$. Then
...
\begin{lemma}[Exact fibers of the coordinate-index map]\label{lem:chi-seed-fibers}
	...
	\leanok
	\uses{def:ld-game, def:binary-representation}
	With the zero-based coordinate convention, every scalar seed has a unique
	representation as a pair
	\[
		(i,r)\in\{0,\ldots,m-1\}\times\{0,\ldots,q/m-1\},
	\]
	and the first component is $\chi(s)$. Conversely, every such pair determines
...
\begin{lemma}[Uniformity of the coordinate index]\label{lem:chi-index-uniform}
	...
	For a uniformly random $s\in\F_q$, the coordinate index $\chi(s)$ is uniform
	on $\{0,\ldots,m-1\}$.
\end{lemma}
\begin{proof}
	\leanok
	\uses{lem:chi-seed-fibers, lem:uniform-first-marginal-after-equivalence}
	Under the equivalence of Lemma~\ref{lem:chi-seed-fibers}, $\chi$ is the first
