...
\begin{proof}
	\leanok
	\uses{def:ld-question-distribution, def:typed-cl-distributions, def:graph-distribution, def:cl-dist}
...
\begin{lemma}[Common level of the Pauli question maps]\label{lem:pauli-question-cl-family-obligation}
...
\begin{lemma}[Pauli question-distribution identity]\label{lem:pauli-question-typed-equality-obligation}
...
\begin{proof}
	...
	Both distributions first use the uniform law on ordered pairs whose unordered pair is an edge of $G^{\mathrm{pauli}}$, and then push forward a common uniform seed in $V^{\mathrm{pauli}}$ by the same pair of maps. The derivation of these assertions is tracked by issue~\#180.
\end{proof}
