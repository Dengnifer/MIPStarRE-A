...
\begin{definition}[Variable-measurement agreement distance]\label{def:ms-variable-agreement}
	...
	\]
	the squared state-dependent distance of Definition~\ref{def:povm-distance} between the two players' binary $\mathsf{Variable}_j$ effects on $\ket{\psi}$, under the one-point distribution and with the constant inside the $O(\cdot)$ taken to be $1$. It is not the consistency defect of Definition~\ref{def:consistency}, which is an off-diagonal outcome weight; the Lean name records that the quantity measures the failure of the two players' $\mathsf{Variable}_j$ measurements to be consistent with one another.
\end{definition}
...
\begin{proof}
	\leanok
	\uses{def:ms-variable-agreement, def:consistent-measurement}
