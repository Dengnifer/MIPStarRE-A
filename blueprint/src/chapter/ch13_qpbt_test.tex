...
\begin{theorem}[Quantum soundness of the simultaneous classical low individual degree test]\label{lem:ld-soundness}
	...
	% The linked theorem uses completed point POVMs. The prescribed-effect
	% statement stays unmarked; see the C4 exemption and the provider remark.
...
\begin{proof}
	...
	The route transcribed below is the one the paper prints. The linked
	formal theorem instead establishes the three bounds for completed point
	POVMs, as stated in Theorem~\ref{lem:ld-soundness-formalized}; it does not
	state the conclusion for the prescribed-answer effects displayed above.
	The positivity comparison in Remark~\ref{rem:ld-soundness-provider}
	explains their relation, but is not a formalized implication here.
	The source-labelled statement and this printed proof therefore remain
	unmarked. The completed theorem uses neither of the two steps of the
	printed route --- the identification of $\mathfrak{G}^{\mathrm{ld}}$ with the
	...
	of~\cite{Ji2021Quantum}. These remain open source gaps, tracked as issue
	527 of this repository and enumerated below. The following account records
	that printed route and its separate verification obligations.
...
% This node states the completed point-POVM variant and its formalized route.
% Its marks do not certify the prescribed-effect statement or printed proof.
\begin{theorem}[Formalization support for Theorem~\ref{lem:ld-soundness}:
	completed point measurements in the seed-indexed game]
	...
	\leanok
	\uses{def:ld-game, def:ld-question-distribution, def:ld-win-predicate, def:ld-meas, def:projective-strategy-general, def:tensor-product-value, def:consistency, def:bracket}
	This formalization-only statement uses completed point measurements.
	For a point question $u$, write $A^u_{\mathbf a}$ for the strategy's
	effect at the prescribed answer $\mathbf a\in\F_q^k$, and put
	\[
		R_A^u=I_{\mathrm A}-\sum_{\mathbf a\in\F_q^k}A^u_{\mathbf a},
		\qquad
		\widetilde A^u_{\mathbf a}
		=A^u_{\mathbf a}+\mathbf 1_{\mathbf a=\mathbf 0}R_A^u.
	\]
	Here $R_A^u$ is the sum of the effects of all answers of the wrong
	form. Define $R_B^u$ and $\widetilde B^u$ in the same way. Thus
	$\widetilde A^u$ and $\widetilde B^u$ are POVMs obtained by sending
	wrong-form answers to the zero tuple; the hypotheses do not require
	$R_A^u$ or $R_B^u$ to vanish.

	There exists a function
	\[
		\delta_{\mathrm{ld}}(\eps, q, m, d, k) = a\,(dmk)^a \bigl(\eps^b + q^{-b} + 2^{-bmd}\bigr)
	\]
	for universal constants $a\ge1$ and $0<b\le1$ such that, for every
	parameter tuple $(q,m,d,k)$ as in Definition~\ref{def:ld-game}, every
	$\eps>0$, and every projective strategy $(\psi,A,B)$ of value at least
	$1-\eps$ in $\mathfrak{G}^{\mathrm{ld}}_{\mathsf{ldparams}}$, there are
	polynomial-tuple POVMs $G^{\mathrm A}$ and $G^{\mathrm B}$ on the original
	players' spaces satisfying
	\begin{align*}
		\widetilde A^u_{\mathbf a}\ot I_{\mathrm B}
		&\simeq_{\delta_{\mathrm{ld}}}
		I_{\mathrm A}\ot G^{\mathrm B}_{[\mathbf g(u)=\mathbf a]},\\
		G^{\mathrm A}_{[\mathbf g(u)=\mathbf a]}\ot I_{\mathrm B}
		&\simeq_{\delta_{\mathrm{ld}}}
		I_{\mathrm A}\ot\widetilde B^u_{\mathbf a},\\
		G^{\mathrm A}_{\mathbf g}\ot I_{\mathrm B}
		&\simeq_{\delta_{\mathrm{ld}}}
		I_{\mathrm A}\ot G^{\mathrm B}_{\mathbf g}.
	\end{align*}
	Here $\mathbf g=(g_1,\ldots,g_k)$ is a tuple of polynomial
	representatives of individual degree at most $d$, and
	$\mathbf g(u)=(g_1(u),\ldots,g_k(u))$. All three relations use
	$\ket\psi$; the first two average over uniform $u\in\F_q^m$, and the
	third has no question distribution. More precisely, each off-diagonal
	consistency defect is at most $\delta_{\mathrm{ld}}$.
	The auxiliary declarations linked here are the tuple collision bound
	and the two scalar absorption estimates used in the proof.
\end{theorem}
\begin{proof}
	\leanok
...
\begin{remark}[Relation to the low individual degree test of Chapter~\ref{chap:test}]\label{rem:ld-soundness-provider}
	Theorem~\ref{lem:ld-soundness-formalized} proves the three consistency
	bounds for completed point POVMs for every positive simultaneity
	parameter $k$ on the original seed-indexed domain $m\mid q$. Its proof
	uses the low individual degree theorem of~\cite{Ji2020LowIndividualDegree},
	formalized as Theorem~\ref{thm:main-formal}, together with the directly
	indexed strategy reduction and the combining construction of
	Proposition~\ref{prop:ld-simultaneous-general-k}. At combined dimension
	$M=m+k$ it uses the auxiliary integer $N=2560000M^3d$ and verifies the
	condition $400Md\le N$. Correlated seed dilation preserves projectivity
	and game value; the two point/polynomial relations compress exactly,
	and point agreement and Schwartz--Zippel recover global consistency on
	the original spaces. Replacing the direct constants $(a,b)$ by
	$(10a,b/2)$ absorbs the resulting error.

	The prescribed-answer effects in Theorem~\ref{lem:ld-soundness}
	differ from these completed effects at the zero tuple by the positive
	remainders $R_A^u$ and $R_B^u$. For opposite tensor placements, each
	off-diagonal summand contributed by a remainder is nonnegative.
	Consequently the completed bounds imply the corresponding bounds for
	the prescribed effects, with the same error; the polynomial/polynomial
	relation is unchanged. This is a mathematical comparison, not an
	implication stated by the linked formal theorem. The source-labelled
	entry remains unmarked, with the documentary exemption recorded in
	\cite{gap:qpbt_ld-dimension-divisibility}; only the separate completed
	statement and its proof carry formalization marks.

	The printed proof in~\cite{Ji2020MIPStar} takes a different route through
	Theorem~4.7 of~\cite{Ji2021Quantum}. Its claimed game identification
	requires a reduction from diagonal-line questions to hidden-subcube
	pair questions, with the seed information and branch probabilities
	accounted for. Its choice $K=m^3d$ must also meet that theorem's distinct
	condition $K\ge12m(d+1)$, which fails at the small dimensions enumerated
	in the proof above. Neither assertion is a hypothesis or a proved step
	of the formalized soundness theorem.

	The documented deviation in~\cite{gap:qpbt_ld-dimension-divisibility}
	therefore distinguishes the established completed-POVM soundness statement
	from the prescribed-effect statement and the unverified source derivation.
	It also distinguishes these from the
	use of a directly indexed auxiliary game at dimension $2m+2$, where
	the original seed-indexed game is generally undefined
	(Remark~\ref{rem:qld-4-7-divisibility}). Documentary closure of that
	comparison supplies no proof of the printed import or of the
	intermediate source-law assertions.
\end{remark}
